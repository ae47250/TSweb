...
const fixture = JSON.parse(readFileSync("tests/fixtures/alpha-master-80-cases.json", "utf8"));
const MAX_FOLLOW_UP_ROUNDS = fixture.follow_up_round_limit || 3;
...
test("master 80 parse-ready cases preserve expected parser fields on the first pass", () => {
  const parseCases = fixture.cases.filter((testCase) => testCase.decision === "parse");
...
test("master 80 warning behavior keeps non-blocking issues visible to Tree Dude", () => {
  const overFourCases = fixture.cases.filter((testCase) => testCase.category === "more_than_four_options");
  ...
  const safetyCases = fixture.cases.filter((testCase) =>
