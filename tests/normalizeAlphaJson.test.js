...
function assertNormalizedCase(testCase) {
  ...
  if (testCase.expectedWarning) assert.match(validation.warnings.join(" "), testCase.expectedWarning);
...
test("follow-up answers recover missing customer fields without leaking contact notes to customer summary", () => {
  ...
  for (const smokeCase of smokeCases) {
    ...
    if (smokeCase.expected.warning) {
      assert.match(validation.warnings.join(" "), smokeCase.expected.warning, smokeCase.input);
    }
