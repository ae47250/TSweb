...
test("review and confirm screens separate AI review from final estimate approval", () => {
  ...
  assert.match(reviewSource, /NEW ESTIMATE/);
  ...
  assert.match(reviewSource, /Estimate Can Be Confirmed Below/);
  assert.match(reviewSource, /More info is needed to complete Estimate/);
  assert.match(reviewSource, /review-status-needs-info/);
  assert.match(reviewSource, /canConfirmWithOverrides \? "Estimate Can Be Confirmed Below" : "More info is needed to complete Estimate"/);
  assert.match(reviewSource, /review-status-action-font/);
  assert.doesNotMatch(reviewSource, /td2-required-warning/);
  ...
  assert.match(cssSource, /\.review-status-action-font\s*\{[^}]*font-size:\s*18px/s);
  assert.match(cssSource, /\.review-status-action-font\s*\{[^}]*font-weight:\s*800/s);
  ...
  assert.match(reviewSource, /function optionDisplayTitle/);
  assert.match(reviewSource, /function optionAdvisoryBaseTitle/);
  assert.match(reviewSource, /optionDisplayTitle\(option, jobNotes\)/);
  assert.match(reviewSource, /optionTitleDetailScore\(description\) > optionTitleDetailScore\(title\)/);
  assert.match(reviewSource, /formatOptionDisplayText\(optionDisplayTitle\(option, jobNotes\)\)/);
  ...
  assert.match(reviewSource, /CustomerOverrideCheckbox/);
  ...
  assert.match(reviewSource, /Phone number needed to SMS Estimate to Customer/);
  ...
  assert.match(reviewSource, /CustomerSummaryField/);
  assert.match(reviewSource, /field="name"/);
  assert.match(reviewSource, /field="address"/);
  assert.match(reviewSource, /field="email"/);
  assert.match(reviewSource, /placeholder="E-mail needed to email Estimate to customer"/);
  assert.match(reviewSource, /warnWhenMissing=\{false\}/);
  assert.match(reviewSource, /overrideKey="missingName"/);
  assert.match(reviewSource, /Create Estimate without customer name/);
  assert.match(reviewSource, /overrideKey="missingAddress"/);
  assert.match(reviewSource, /Create Estimate without exact address/);
  assert.match(reviewOverridesSource, /missingName/);
  assert.match(reviewSource, /CustomerPhoneSummaryField/);
  assert.match(reviewSource, /onChange\?\.\("phone", nextValue\)/);
  assert.match(reviewSource, /const \[draftText, setDraftText\]/);
  assert.match(reviewSource, /const fieldMissing = missing \|\| draftMissing/);
  assert.match(reviewSource, /onDraftChange=\{setDraftText\}/);
  assert.match(reviewSource, /placeholder=\{phoneOverrideAccepted \? "" : phoneNeedsAttention \? "Phone number needed to SMS Estimate to Customer" : "Phone not available"\}/);
  assert.match(reviewSource, /Phone number needed to SMS Estimate to Customer/);
  assert.match(reviewSource, /const \[draftPhoneText, setDraftPhoneText\]/);
  assert.match(reviewSource, /const phoneNeedsAttention = !String\(draftPhoneText \|\| ""\)\.trim\(\) \|\| !draftPhoneIsValid/);
  assert.match(reviewSource, /const currentDigits = phoneDigits\(value\)/);
  assert.match(reviewSource, /onChange\?\.\("phone", ""\)/);
  assert.match(reviewSource, /if \(isValid && nextDigits !== currentDigits\) \{/);
  assert.match(reviewSource, /const showPhoneOverride = phoneNeedsAttention && contactOverrideStatus/);
  assert.match(reviewSource, /editorClassName=\{!phoneOverrideAccepted && phoneNeedsAttention \? "td2-inline-editor-warning" : "td2-inline-editor-quiet"\}/);
  assert.match(reviewSource, /td2-customer-field-with-override/);
  assert.match(reviewSource, /needsPhone=\{needsInlinePhoneEditor\}/);
  assert.match(reviewSource, /const needsInlinePhoneEditor = !customerPhoneAvailable/);
  assert.match(reviewSource, /const customerPhoneRequired = !customerPhoneAvailable && !phoneOverrideAccepted/);
  assert.match(reviewSource, /const canConfirmWithOverrides = overrideStatus\.canProceed && !customerPhoneRequired/);
  assert.match(reviewSource, /const fallbackPhoneOverrideStatus =/);
  assert.match(reviewSource, /contactWarning: \{ key: "missingPhone" \}/);
  assert.match(reviewSource, /contactOverrideStatus=\{customerPhoneOverrideStatus\}/);
  assert.match(reviewSource, /inlineMissingPhoneOverrideStatus/);
  assert.match(reviewSource, /showPhoneEditor=\{false\}/);
  assert.match(reviewSource, /td2-inline-editor-quiet/);
  assert.match(reviewSource, /minWidthCh=\{44\}/);
  assert.match(reviewSource, /Math\.max\(minWidthCh, widthText\.length \+ 2\)/);
  assert.match(pageSource, /field === "name"/);
  ...
  assert.match(cssSource, /\.customer-summary-card\s*\{[^}]*width:\s*100%/s);
  ...
  assert.match(cssSource, /min-width:\s*44ch/);
...
test("review overrides are narrow and recorded separately from normal validation", () => {
  ...
  assert.match(pageSource, /const nextPhone = phoneDigitCount\(nextValue\) === REQUIRED_PHONE_DIGITS \? nextValue : ""/);
  assert.match(pageSource, /nextAlphaJson\.customer\.phone_display = nextPhone/);
  assert.match(pageSource, /nextContact\.phone = nextPhone/);
