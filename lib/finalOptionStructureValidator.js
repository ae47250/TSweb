...
function optionHasKind(option, kind) {
  ...
  if (metaKind) return metaKind === kind;
  return inferServiceKindFromText(optionText(option)) === kind;
}

function optionServiceKind(option = {}) {
  return option.canonical_option?.service_kind ||
    option.canonical_service_item?.service_kind ||
    inferServiceKindFromText(optionText(option));
}
...
function activeOptionComparisonErrors(options = [], finalOptionModel = {}) {
  ...
  for (const option of options) {
    const kind = optionServiceKind(option);
  ...
  if (
    ...
    (options[0] && DEPENDENT_ADD_ON_KINDS.has(optionServiceKind(options[0])))
  ) {
    errors.push(structuralError(
...
function unresolvedAddOnInterpretationErrors(alphaJson = {}, options = [], finalOptionModel = {}) {
  ...
  for (const interpretation of interpretations) {
    ...
    const activeAddOn = options.find((option) => {
      ...
      return DEPENDENT_ADD_ON_KINDS.has(optionServiceKind(option));
    });
