...
function parseNumericPrice(value) {
  ...
  const match = asString(value).match(/\$?\s*([0-9][0-9,]*)\b/);
...
function hasRawOptionLabel(option) {
  return /^(?:Option\s*)?[A-E]\b/i.test(asString(option.raw_label));
}

function hasSoftNumericPriceLanguage(option) {
  const text = [option.price_raw, option.raw_text, option.scope, option.evidence].map(asString).join(" ");
  return /\b(?:maybe|around|about|roughly|probably)\s+\$?\s*[0-9][0-9,]*(?:k|000)?\b/i.test(text);
}

function isLabeledSoftNumericOption(option) {
  return option.price_status === "explicit_numeric_with_soft_language" ||
    (hasRawOptionLabel(option) && hasSoftNumericPriceLanguage(option));
}
  ...
  return {
    ...
    service_options: {
      items: options.map((option) => {
        const labeledSoftNumeric = isLabeledSoftNumericOption(option);
        const resolvedPrice = resolvePrice({
          rawPrice: option.price_raw,
          optionText: option.raw_text || option.scope,
          rawText: labeledSoftNumeric ? "" : rawInput,
        });
        const amount = option.price_status === "firm" || labeledSoftNumeric
          ? option.price_amount ?? parseNumericPrice(option.price_raw) ?? resolvedPrice.amount
          : null;
        const priceStatus = labeledSoftNumeric ? "explicit_numeric_with_soft_language" : option.price_status || resolvedPrice.priceStatus;
        return {
          ...
          price: {
            amount,
            display: amount ? labeledSoftNumeric ? money(amount) : resolvedPrice.display || money(amount) : "",
            raw: asString(option.price_raw),
            status: priceStatus,
            is_unclear: !amount || (!labeledSoftNumeric && priceStatus !== "firm"),
            review_warning: Boolean(labeledSoftNumeric && amount),
          },
          price_status: priceStatus,
          price_review_warning: Boolean(labeledSoftNumeric && amount),
          scope_unclear: Boolean(option.scope_unclear),
    ...
    normalization: {
      ...
      uncertainties: [
        ...
        ...options.flatMap((option) => {
          if (isLabeledSoftNumericOption(option)) return [];
          const resolvedPrice = resolvePrice({
            rawPrice: option.price_raw,
            optionText: option.raw_text || option.scope,
            rawText: rawInput,
      ...
      low_confidence_spans: Array.isArray(draft?.low_confidence_spans) ? draft.low_confidence_spans : [],
      number_trace: Array.isArray(draft?.number_trace) ? draft.number_trace : [],
      field_evidence: {
        ...
        phone_source: asString(draft?.normalization?.field_evidence?.phone_source) || "openai_draft",
        email: asString(draft?.normalization?.field_evidence?.email) || asString(draft?.contact?.email),
        email_source: asString(draft?.normalization?.field_evidence?.email_source) || "openai_draft",
        service_address: address.value || asString(draft?.normalization?.field_evidence?.service_address),
        service_address_source: address.source || asString(draft?.normalization?.field_evidence?.service_address_source) || "openai_draft",
