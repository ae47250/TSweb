...
function customerLocationForScope(value) {
  const qualifier = materialQualifierForScope(value);
...
function materialQualifierForScope(value) {
  const text = normalizeCanonicalText(value);
  if (!text) return "";
  const patterns = [
  ...
  for (const pattern of patterns) {
    ...
    if (match) return normalizedQualifierPhrase(match[0]);
  }
  return "";
}
...
function appendQualifier(text, qualifier) {
  ...
  const phrase = normalizedQualifierPhrase(qualifier);
