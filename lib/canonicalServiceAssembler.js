...
function customerLocationForScope(value) {
  const text = stripSafetyAccessText(value);
...
function normalizedJobFactsFromAlpha(alphaJson = {}) {
  ...
  return {
    ...
    work_location: customerLocationForScope(evidenceText) || extractLocation(stripSafetyAccessText(`${description} ${corrected}`)),
...
function scopeFactsForEvidence(record, normalizedJobFacts = {}) {
  const localText = record.local_text || "";
  const safeEvidenceText = stripSafetyAccessText(localText);
  const count = extractCountPhrase(safeEvidenceText);
  const species = extractSpecies(safeEvidenceText);
  const location = customerLocationForScope(safeEvidenceText) || extractLocation(safeEvidenceText);
  return {
...
export function buildCanonicalServiceItems(normalizedJobFacts, typedPriceEvidence, extractedRelationships) {
  ...
  for (const record of usableEvidence) {
    ...
    const item = {
      ...
      scope: {
        ...scopeFactsForEvidence(record, normalizedJobFacts),
        inclusions: [],
        exclusions: [],
      },
...
function finalOptionTextForItem(item) {
  ...
  if (item.service_kind === "tree_trim") {
    if (textHasMapleLimbs(evidenceText)) {
      return {
        ...
        description: "Trim the maple limbs.",
      };
    }
    return {
      title: "Tree trimming",
      description: "Trim the tree branches.",
    };
  }
  ...
  if (item.service_kind === "limb_removal") {
    return {
      title: "Limb removal",
      description: textHasLargeLimb(evidenceText) ? "Cut up the large limb." : "Cut up the limb.",
    };
  }
  ...
  if (item.service_kind === "tree_removal") {
    if (textHasThreeOrnamentalPears(evidenceText) || (count === 3 && /ornamental\s+pear/i.test(species))) {
      return withBaseDisposition({
        ...
        description: "Remove the three small ornamental pear trees.",
      }, localScopeText);
    }
    ...
    return withBaseDisposition({
      title: "Tree removal",
      description: count && count > 1 ? `Remove the ${numberWord(count)} trees.` : "Remove the tree.",
    }, localScopeText);
  }
...
function addOnTitlePhraseForKind(kind) {
  return {
    stump_grinding: "stump grinding",
...
function addOnDescriptionPhraseForKind(kind, baseItem, addOnItem = {}) {
  ...
  if (kind === "stump_grinding") {
    ...
    if (textHasThreeOrnamentalPears(evidenceText)) return "grind the stumps";
    return count && count > 1 ? "grind the stumps" : "grind the stump";
  }
  const cleanup = cleanupLocationPhrase(addOnEvidenceText);
  if (kind === "haul_away" && cleanup) return `haul away the resulting debris and ${cleanup}`;
...
function cumulativeOptionFromItems({ baseItem, addOnItem, relationship, targetBinding }) {
  ...
  const addOnTitle = addOnTitlePhraseForKind(addOnItem.service_kind);
