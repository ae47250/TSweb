...
function optionLocalEvidence(value) {
  const text = compact(value);
  return text
    ? compact(text.split(/\s+(?:and\s+)?(?:option|opt)?\s*[A-E]\b/i)[0])
    : "";
}
...
export function inferServiceKindDetails(value) {
  ...
  if (/\blimbs?\b|\bbranch(?:es)?\b/.test(text)) {
    if (/\bremove|removal|cut|cut up|clear|cleanup|clean up|take\b/.test(text)) return result("limb_removal", "explicit_limb_removal_words", /\b(?:limbs?|branch(?:es)?|remove|removal|cut up|cut|clear|take)[a-z\s/-]{0,80}/i);
    if (/\btrim|prune|pruning|crown\b/.test(text)) return result("tree_trim", "explicit_limb_trim_words", /\b(?:limbs?|branch(?:es)?|trim|prune|pruning|crown)[a-z\s/-]{0,80}/i);
  }
...
function explicitBaseServiceKind(value) {
  const text = normalizeCanonicalText(value);
  if (!text) return "";
  if (/\b(?:storm|wind|downed)\b/.test(text) && /\b(?:cleanup|clean up|debris|limbs?|branches?)\b/.test(text)) {
    return "storm_cleanup";
  }
  if (/\b(?:trim(?:ming)?|prun(?:e|ing)|crown)\b/.test(text)) return "tree_trim";
  if (/\b(?:limbs?|branches?)\b/.test(text) && /\b(?:remove|removal|cut|clear|cleanup|clean up|take)\b/.test(text)) {
    return "limb_removal";
  }
  if (/\b(?:remove|removal|take down|tree down|tree out|drop|rmv)\b/.test(text) ||
    /\bcut\b.{0,30}\b(?:down|only|stack|leave)\b/.test(text)) {
    return "tree_removal";
  }
  return "";
}

function serviceKindDetailsForRelationshipRole(record, details, relationshipRole) {
  if (relationshipRole !== "base") return details;
  const explicitKind = explicitBaseServiceKind(record.local_text);
  if (!explicitKind || explicitKind === details.service_kind) return details;
  return {
    ...details,
    service_kind: explicitKind,
    reason_code: "relationship_base_service_kind",
    evidence_text: record.local_text,
  };
}
...
function extractSourceTreeCount(value) {
  const text = compact(value).toLowerCase();
  const count = "(one|two|three|four|five|six|seven|eight|nine|ten|[1-9]|10)";
  const descriptor = "(?:small|large|big|dead|fallen|ornamental)\\s+";
  const species = "(?:ash|cedar|cherry|locust|maple|oak|ornamental\\s+pear|pear|pine|spruce|walnut)";
  const match = text.match(new RegExp(
    `\\b${count}\\s+(?:${descriptor})?(?:${species}\\s+)?trees?\\b|` +
    `\\b${count}\\s+(?:${descriptor})?(?:ashes|cedars|cherries|locusts|maples|oaks|ornamental\\s+pears|pears|pines|spruces|walnuts)\\b`,
    "i",
  ));
  if (!match) return "";
  const token = match[0].match(new RegExp(`\\b${count}\\b`, "i"))?.[1] || "";
  const number = numberWord(token);
  return number ? `${number} trees` : "";
}
...
function normalizedJobFactsFromAlpha(alphaJson = {}) {
  ...
  const explicitRawCount = extractSourceTreeCount(`${corrected} ${rawText}`) || extractExplicitTreeCount(`${corrected} ${rawText}`);
  const treeCount = explicitRawCount || (treeCountStatus && treeCountStatus !== "missing" ? compact(tree.tree_count) : "");
  const evidenceText = `${corrected} ${rawText}`;
  return {
    ...
    work_location: customerLocationForScope(corrected) || extractLocation(corrected),
...
function relationshipPriceRoleSets(extractedRelationships = {}) {
  return {
    basePriceIds: new Set((extractedRelationships.add_on_interpretations || [])
      .map((interpretation) => interpretation.base_price_id)
      .filter(Boolean)),
    addOnPriceIds: new Set((extractedRelationships.add_on_interpretations || [])
      .map((interpretation) => interpretation.add_on_price_id)
      .filter(Boolean)),
  };
}

function scopeFactsForEvidence(record, normalizedJobFacts = {}, serviceKind = "", useGlobalFallbackOverride = false) {
  ...
  const useGlobalFallback = useGlobalFallbackOverride || BASE_SERVICE_KINDS.has(serviceKind);
  const count = extractCountPhrase(evidenceText) ||
    (useGlobalFallback ? extractCountPhrase(globalEvidenceText) || compact(normalizedJobFacts.tree_count) : "");
  const species = extractSpecies(evidenceText, useGlobalFallback ? normalizedJobFacts.tree_species || globalEvidenceText : "");
  const location = customerLocationForScope(evidenceText) || extractLocation(evidenceText);
  return {
...
export function buildCanonicalServiceItems(normalizedJobFacts, typedPriceEvidence, extractedRelationships) {
  ...
  const alternativePriceIds = new Set(
    (extractedRelationships.alternatives || []).flatMap((relationship) => relationship.option_price_ids || []),
  );
  const { basePriceIds, addOnPriceIds } = relationshipPriceRoleSets(extractedRelationships);
  ...
  for (const record of usableEvidence) {
    ...
    const relationshipRole = basePriceIds.has(record.price_id)
      ? "base"
      : addOnPriceIds.has(record.price_id)
        ? "add_on"
        : "";
    const inferredServiceKindDetails = record.service_kind
      ? {
    ...
    const serviceKindDetails = serviceKindDetailsForRelationshipRole(record, inferredServiceKindDetails, relationshipRole);
    ...
    const item = {
      ...
      scope: {
        ...scopeFactsForEvidence(record, normalizedJobFacts, serviceKind, alternativePriceIds.has(record.price_id)),
        inclusions: [],
        exclusions: [],
      },
...
function customerScopeEvidenceTextForItem(item) {
  return allEvidenceText(
    optionLocalEvidence(item.source?.local_text),
    optionLocalEvidence(item.scope?.evidence_text),
    optionLocalEvidence(item.source?.service_kind_evidence_text),
  ) || evidenceTextForItem(item);
}
...
function sourceFaithfulOptionWording(item) {
  const rawText = compact(item.source?.local_text);
  if (!rawText || hasContactOrAddressLeak(rawText) || classifiedSafetyAccessWarnings(rawText).length) return "";

  const wording = rawText
    .replace(/^\s*(?:option|opt)\s*[A-E1-5]\s*[:.)-]?\s*/i, "")
    .replace(/\s+(?:for|at)\s*$/i, "")
    .replace(/[.;,:]+$/g, "")
    .trim();
  const normalized = normalizeCanonicalText(wording);
  if (!wording || !/^(?:remove|removal|take\s+down|cut\s+down|cut|drop)\b/i.test(wording)) return "";
  if (!/\b(?:tree|trees|maple|oak|cedar|ash|beech|birch|cherry|elm|hickory|locust|pear|pine|poplar|spruce|sycamore|walnut|willow)\b/i.test(normalized)) return "";
  if (/\b(?:rmv|brsh|debrs?|stmp)\b/i.test(wording)) return "";
  if (!/\b(?:leave|stack|haul|grind|stump|cleanup|clean|brush|wood|debris|logs?|chips?|no\s+cleanup)\b/i.test(normalized)) return "";
  if (/\bboth\b/i.test(normalized)) return "";
  if (!/(?:\bon\s+site\b|\b(?:wood\s+and\s+brush|brush\s+and\s+wood)\b|\bno\s+(?:cleanup|stump)\b|\b(?:haul\s+away|clean\s+up\s+the|clean\s+the|grind\s+the)\b)/i.test(normalized)) return "";
  return wording;
}

function finalOptionTextForItem(item) {
  const evidenceText = evidenceTextForItem(item);
  const localScopeText = optionLocalEvidence(item.source?.local_text) || evidenceText;
  ...
  const sourceWording = item.service_kind === "tree_removal" ? sourceFaithfulOptionWording(item) : "";

  if (sourceWording) {
    return {
      title: sourceWording,
      description: sourceWording,
    };
  }
...
function descriptionForItem(item) {
  ...
  switch (item.service_kind) {
    ...
    case "haul_away":
      return sentence(`Haul away debris from ${subject || "the tree work"}`);
    case "brush_cleanup":
      return sentence(`Clean up brush and debris ${subject ? `from ${subject}` : "as described in the job notes"}`);
    case "storm_cleanup":
...
function sourceHasBrushChip(value) {
  const text = normalizeCanonicalText(value);
  return /\bchip(?:ping)?\b.{0,24}\b(?:brush|limbs?|branches?)\b|\b(?:brush|limbs?|branches?)\b.{0,24}\bchip(?:ping)?\b|\bchip\s+brush\b/.test(text);
}

function sourceHasBrushHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)(?:ed|ing)?(?:\s+away)?\s+(?:the\s+)?brush\b|\bbrush\s+(?:haul|hual|hawl)(?:-away)?\b/.test(text);
}

function sourceHasLogHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)(?:ed|ing)?(?:\s+away)?\s+(?:the\s+)?logs?\b|\blogs?\b.{0,24}\b(?:haul|hual|hawl)\b|\blog\s+hauling\b/.test(text);
}

function sourceHasDebrisHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)\b(?:\s+away)?\b|\b(?:haul|hual|hawl)\b.{0,24}\b(?:debris|logs?|wood)\b|\b(?:debris|logs?|wood)\b.{0,24}\b(?:haul|hual|hawl)\b|\bdebris\s+haul(?:-away)?\b|\bhaul\s+debris\b|\blog\s+hauling\b|\bwood\s+removal\b/.test(text);
}

function sourceHasCutRounds(value) {
  const text = normalizeCanonicalText(value);
  return /\bcut\s+(?:the\s+)?rounds?\b/.test(text);
}

function sourceHasWoodLeft(value) {
  const text = normalizeCanonicalText(value);
  return /\bleave\b.{0,24}\b(?:wood|logs?|firewood)\b|\b(?:wood|logs?|firewood)\b.{0,24}\bleave\b|\bstack\s+(?:wood|logs?|firewood)\b|\bleave\s+wood\s+on\s+site\b/.test(text);
}

function sourceHasChipAndLeave(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:mulch\s+)?chip(?:ping)?\b.{0,24}\bleave\b|\bleave\b.{0,24}\bchips?\b|\bchips?\s+left\s+on\s+site\b|\bchip\s+and\s+leave\b/.test(text);
}

function sourceHasRootGrinding(value) {
  const text = normalizeCanonicalText(value);
  return /\broot\s+grind(?:ing)?\b|\bgrind(?:ing)?\s+(?:the\s+)?roots?\b/.test(text);
}

function sourceHasCleanupAction(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:cleanup|clean up|clean (?:the )?(?:debris|brush|yard|work area)|final cleanup|site cleanup|yard protection cleanup|access cleanup|yard cleanup|rake(?:[-\s]?up)?)\b/.test(text);
}
...
function cleanupActionPhrase(value) {
  ...
  const text = normalizeCanonicalText(value);
  if (/\bclean\b.{0,12}\bdebris\b|\bdebris\s+cleanup\b/.test(text)) return "clean up the debris";
  if (/\bclean\b.{0,12}\bbrush\b|\bbrush\s+cleanup\b/.test(text)) return "clean up the brush";
  if (/\bclean\b.{0,12}\byard\b/.test(text)) return "clean up the yard";
  if (/\brake(?:[-\s]?up)?\b/.test(text)) return "rake up the brush";
  if (/\b(?:site|final)\s+cleanup\b/.test(text)) return "site cleanup";
  if (/\byard\s+protection\s+cleanup\b/.test(text)) return "yard protection cleanup";
  if (/\baccess\s+cleanup\b/.test(text)) return "access cleanup";
...
function supplementalCleanupPhrases(value) {
  ...
  if (sourceHasLogHaul(value)) phrases.push("haul away the logs");
  else if (sourceHasBrushHaul(value)) phrases.push("haul away the brush");
  else if (sourceHasDebrisHaul(value)) phrases.push("haul away the debris");
  if (sourceHasCutRounds(value)) phrases.push("cut the rounds");
  if (sourceHasWoodLeft(value)) phrases.push("leave the wood on site");
  if (sourceHasChipAndLeave(value)) phrases.push("leave the chips on site");
...
function addOnTitlePhraseForItem(item = {}) {
  ...
  const evidenceText = customerScopeEvidenceTextForItem(item);
  const hasCleanup = sourceHasCleanupAction(evidenceText);
  const hasRootGrinding = sourceHasRootGrinding(evidenceText);
  if (kind === "stump_grinding") {
    const phrases = [hasRootGrinding ? "root grinding" : "stump grinding"];
    if (sourceHasBrushChip(evidenceText)) phrases.push("brush chipping");
    if (sourceHasBrushHaul(evidenceText)) phrases.push("brush haul-away");
    else if (sourceHasDebrisHaul(evidenceText)) phrases.push("debris haul-away");
    if (hasCleanup) phrases.push("cleanup");
    return joinActionPhrases(phrases);
  }
  if (kind === "brush_cleanup" && sourceHasBrushHaul(evidenceText)) return "brush haul-away";
  if (kind === "haul_away" && sourceHasWoodLeft(evidenceText)) return "wood left on site";
  if (kind === "haul_away" && sourceHasChipAndLeave(evidenceText)) return "chips left on site";
  return {
    stump_grinding: "stump grinding",
...
function addOnDescriptionPhraseForKind(kind, baseItem, addOnItem = {}) {
  const addOnEvidenceText = customerScopeEvidenceTextForItem(addOnItem);
  if (kind === "stump_grinding") {
    ...
    if (sourceHasRootGrinding(addOnEvidenceText)) {
      return joinActionPhrases(["grind the roots", ...supplementalCleanupPhrases(addOnEvidenceText)]);
    }
  ...
  if (kind === "haul_away" && sourceHasWoodLeft(addOnEvidenceText)) return "leave the wood on site";
  if (kind === "haul_away" && sourceHasChipAndLeave(addOnEvidenceText)) return "chip the brush and leave the chips on site";
  if (kind === "haul_away" && haulCleanup) return `haul away the resulting debris and ${haulCleanup}`;
  if (kind === "brush_cleanup") {
    if (sourceHasChipAndLeave(addOnEvidenceText) && !sourceHasBrushChip(addOnEvidenceText)) {
      return "chip the brush and leave the chips on site";
    }
    const baseEvidenceText = customerScopeEvidenceTextForItem(baseItem);
    const phrases = supplementalCleanupPhrases(addOnEvidenceText).filter((phrase) => {
      if (phrase === "leave the wood on site" && sourceHasWoodLeft(baseEvidenceText)) return false;
      if (phrase === "leave the chips on site" && sourceHasChipAndLeave(baseEvidenceText)) return false;
      return true;
    });
...
function independentAlternativeTextForItem(item) {
  if (!DEPENDENT_ADD_ON_KINDS.has(item.service_kind)) return finalOptionTextForItem(item);
  const baseKind = explicitBaseServiceKind(evidenceTextForItem(item));
  if (!baseKind) return finalOptionTextForItem(item);
  const baseText = finalOptionTextForItem({ ...item, service_kind: baseKind });
  const addOnTitle = addOnTitlePhraseForItem(item);
  const addOnPhrase = addOnDescriptionPhraseForKind(item.service_kind, { ...item, service_kind: baseKind }, item);
  const actionConnector = /\band\b/.test(addOnPhrase) ? ", " : " and ";
  return {
    title: `${baseText.title} with ${addOnTitle}`,
    description: sentence(`${stripFinalPunctuation(baseText.description)}${actionConnector}${addOnPhrase}`),
  };
}
...
function sourceQuantityConflictError(baseItem, addOnItem, relationshipId) {
  ...
  if (!addOnStumpCount) return null;
  if (!baseCount) {
    return structuralError(
      "SCOPE_NUMBER_AGREEMENT_MISMATCH",
      "The source gives a stump quantity but does not provide a source-supported base tree quantity; TD must clarify the scope before final option construction.",
      {
  ...
  if (baseCount === addOnStumpCount) return null;
  return structuralError(
    "SCOPE_NUMBER_AGREEMENT_MISMATCH",
...
function optionFromCanonicalItem(item, index, extraCanonical = {}) {
  const text = extraCanonical.option_kind === "independent_alternative"
    ? independentAlternativeTextForItem(item)
    : finalOptionTextForItem(item);
  return {
    label: `Option ${String.fromCharCode(65 + index)}`,
    raw_label: "",
    sort_order: index + 1,
...
function cumulativeOptionFromItems({ baseItem, addOnItem, relationship, targetBinding }) {
  const sourceWording = relationship.price_role === EXPLICIT_OPTION_TOTAL
    ? sourceFaithfulOptionWording(addOnItem)
    : "";
  const baseText = sourceWording
    ? { title: sourceWording, description: sourceWording }
    : finalOptionTextForItem(baseItem);
  ...
  return {
    ...
    raw_label: "",
    sort_order: 2,
    title: sourceWording ? sourceWording : `${baseText.title} with ${addOnTitle}`,
    description: sourceWording ? sourceWording : sentence(`${baseDescription}${actionConnector}${addOnPhrase}`),
    price: {
      price_type: "fixed",
      currency: "USD",
      amount: totalAmount,
...
export function buildCanonicalFinalOptionModel({
  ...
  const alternativeRelationships = (extractedRelationships.alternatives || [])
    .filter((relationship) =>
      relationship.type === "alternative_to" &&
      (relationship.option_price_ids || []).length === 2,
    );

  if (!relationships.length && alternativeRelationships.length) {
    for (const relationship of alternativeRelationships) {
      const priceIds = new Set(relationship.option_price_ids);
      const alternativeItems = items.filter((item) => priceIds.has(item.price_occurrence_id));
      if (alternativeItems.length !== 2) {
        errors.push(structuralError(
          "MISSING_BASE_CHOICE",
          "An independent alternative relationship must resolve to two priced customer choices.",
          { item_ids: alternativeItems.map((item) => item.stable_id), evidence_ids: [relationship.relationship_id].filter(Boolean) },
        ));
        continue;
      }
      alternativeItems.forEach((item, index) => {
        finalOptions.push(optionFromCanonicalItem(item, index, {
          option_kind: "independent_alternative",
          service_role: "independent_alternative",
          price_relationship: "standalone",
          selectability: "selectable",
          target_id: item.target_id,
      ...
      relationshipModels.push({
        relationship_id: relationship.relationship_id || "independent_alternative",
        type: "independent_alternative",
        option_price_ids: relationship.option_price_ids,
        confidence: relationship.confidence || "medium",
        provenance: relationship.source || "option_price_sidecar_independent_alternatives",
      });
    }
