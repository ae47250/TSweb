...
function sourceHasDebrisHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)\b.{0,24}\b(?:debris|logs?|wood)\b|\b(?:debris|logs?|wood)\b.{0,24}\b(?:haul|hual|hawl)\b|\bdebris\s+haul(?:-away)?\b|\bhaul\s+debris\b|\blog\s+hauling\b|\bwood\s+removal\b/.test(text);
}
...
function sourceHasCleanupAction(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:cleanup|clean up|clean yard|clean the yard|clean work area|clean the work area|final cleanup|site cleanup|yard protection cleanup|access cleanup|yard cleanup|rake(?:[-\s]?up)?)\b/.test(text);
}
...
function addOnTitlePhraseForItem(item = {}) {
  ...
  if (kind === "stump_grinding") {
    if (hasRootGrinding) return "root grinding";
...
function addOnDescriptionPhraseForKind(kind, baseItem, addOnItem = {}) {
  ...
  if (kind === "brush_cleanup" && sourceHasChipAndLeave(addOnEvidenceText)) return "chip the brush and leave the chips on site";
