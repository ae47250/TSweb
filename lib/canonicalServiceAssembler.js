...
function customerScopeEvidenceTextForItem(item) {
  return allEvidenceText(
    item.source?.local_text,
    item.scope?.evidence_text,
    item.source?.service_kind_evidence_text,
...
function sourceHasBrushHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)\b.{0,24}\bbrush\b|\bbrush\b.{0,24}\b(?:haul|hual|hawl)\b|\bbrush\s+haul(?:-away)?\b/.test(text);
}

function sourceHasDebrisHaul(value) {
  const text = normalizeCanonicalText(value);
  return /\b(?:haul|hual|hawl)\s+away\b|\b(?:haul|hual|hawl)\b.{0,24}\b(?:debris|logs?|wood)\b|\b(?:debris|logs?|wood)\b.{0,24}\b(?:haul|hual|hawl)\b|\bdebris\s+haul(?:-away)?\b|\bhaul\s+debris\b|\blog\s+hauling\b|\bwood\s+removal\b/.test(text);
}
...
function cumulativeOptionFromItems({ baseItem, addOnItem, relationship, targetBinding }) {
  ...
  return {
    ...
    description: sentence(`${baseDescription}${actionConnector}${addOnPhrase}`),
    price: {
      price_type: "fixed",
      currency: "USD",
      amount: totalAmount,
