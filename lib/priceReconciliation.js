...
function serviceCategory(text) {
  ...
  if (/\bhaul(?:\s+away|\s+off)?\b|\bcleanup\b|\bclean\s+up\b|\bdebris\b|\bbrush\b/.test(value)) return "haul_cleanup";
...
export function reconcileSidecarPrices(alphaJson = {}, optionPriceCandidateView = null) {
  ...
  const addOnPriceIdsNeedingReview = new Set(addOnInterpretations
    .filter((interpretation) => interpretation.needs_review && !initiallyAcceptedComponentPriceIds.has(interpretation.add_on_price_id))
