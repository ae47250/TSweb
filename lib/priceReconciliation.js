...
const SERVICE_SCOPE_START_PATTERN = new RegExp(`\\b(?:cut\\s+up|${TREE_SERVICE_PATTERN_SOURCES.workScope})\\b`, "i");
...
function cleanScope(value) {
  let text = asString(value)
    ...
    .replace(/^[\s:;.,=-]+|[\s:;.,=-]+$/g, "")
    .trim();
  const workStart = text.search(SERVICE_SCOPE_START_PATTERN);
...
function basicCustomerFacingScope(value) {
  let text = asString(value)
    .replace(/\boptions?\s+include\b.*$/gi, " ")
    .replace(/\b(?:prices?|priced|quote|quoted|estimate|est|bid|cost|total|option|opt)\b/gi, " ")
    .replace(/\b\d[\d,]*\b/g, " ")
    .replace(/\s+/g, " ")
    .replace(/^[\s:;.,=-]+|[\s:;.,=-]+$/g, "")
  ...
  if (
    workStart > 0 &&
    /\b(?:not\s+addr|addr|address|cell|phone|e\s*mail|email|mail|customer|estimate)\b|@/i.test(text.slice(0, workStart))
  ) {
    text = text.slice(workStart).trim();
  }
  return text
    .replace(/\s+/g, " ")
    .replace(/^[\s:;.,=-]+|[\s:;.,=-]+$/g, "")
...
function finishCustomerFacingScope(value) {
  return finishScopePhrase(basicCustomerFacingScope(value));
}

function finishScopePhrase(value) {
...
function stormCleanupBaseScope(interpretation = {}) {
  const evidence = normalizeScopeText([
    interpretation.base_description,
    interpretation.context,
  ].filter(Boolean).join(" "));
  if (!/\bstorm|wind|downed\b/.test(evidence)) return "";
  if (!/\bcleanup|clean up|clear|damage|debris|limbs?\b/.test(evidence)) return "";
  return /\bback\s+yard\b/.test(evidence)
    ? "storm-damage cleanup in back yard"
    : "storm-damage cleanup";
}

function explicitMultiOrnamentalPearScope(interpretation = {}, alphaJson = {}) {
  const evidence = normalizeScopeText([
    interpretation.base_description,
    interpretation.context,
  ...
  const match = evidence.match(/\b(?:remove\s+)?(two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\s+(small\s+)?ornamental\s+pears?\b/);
  if (!match) return "";
  return `remove ${match[1]} ${match[2] || ""}ornamental pears`.replace(/\s+/g, " ").trim();
}
...
function normalizeScopeText(value) {
  return customerFacingServiceScope(value)
    .toLowerCase()
    .replace(/[^a-z0-9\s/-]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function isGenericCustomerScope(value = "") {
  const text = normalizeScopeText(value);
  if (!text) return true;
  if (/^(?:tree service|tree service tree service|tree work|tree removal|tree trim|service|work|cleanup|cleanup cleanup)$/.test(text)) return true;
  if (/^(?:remove|trim|take down|cut down)\s+one\s+tree$/.test(text)) return true;
  return /^(?:tree service\s*)+$/.test(text);
}

function removeAddOnScopeFromBase(value = "", addOnDescription = "") {
  let text = basicCustomerFacingScope(value);
  const addOn = normalizeScopeText(addOnDescription);
  if (/\bstump\b|\bstumping\b|\bgrind/.test(addOn)) {
    text = text.replace(/\b(?:and\s+)?(?:stump\s+grind(?:ing)?|grind(?:ing)?\s+(?:the\s+)?stumps?)\b.*$/i, " ");
  }
  if (/\bhaul\b|\bdebris\b|\bwood\b/.test(addOn)) {
    text = text
      .replace(/\b(?:and\s+)?haul(?:\s+away|\s+off)?(?:\s+(?:debris|wood|brush))?\b.*$/i, " ")
      .replace(/\b(?:and\s+)?(?:haul|remove)\s+debris\b.*$/i, " ");
  }
  if (/\bbrush\b|\bcleanup\b|\bclean/.test(addOn)) {
    text = text.replace(/\b(?:and\s+)?(?:clean(?:\s+up)?|cleanup|clear)\s+(?:brush|debris|area)\b.*$/i, " ");
  }
  return basicCustomerFacingScope(text);
}

function conciseBaseScope(value = "") {
  let text = basicCustomerFacingScope(value).toLowerCase();
  if (!text) return "";
  text = text
    .replace(/\b(?:tree\s+)?job\s+(?=trim|trimming|prune|pruning|remove|removal|take\s+down|cut\s+down|cut\s+up|drop|limb|limbs|branch|branches)\b/g, "")
    .replace(/\bremove\s+(two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\s+small\s+ornamental\s+pears?\s+trees?\b/g, "remove $1 small ornamental pears")
    .replace(/\b(two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\s+small\s+ornamental\s+pears?\s+trees?\b/g, "$1 small ornamental pears")
    .replace(/\b(take\s+down\s+.+?)\s+tree\s+removal\b/g, "$1")
    .replace(/\b(remove\s+.+?)\s+tree\s+removal\b/g, "$1")
    .replace(/\bremove\s+one\s+([a-z]+)\s+tree\b/g, "remove $1")
    .replace(/\bremove\s+(ash|beech|birch|cedar|cherry|elm|hickory|locust|maple|oak|pear|pine|poplar|sycamore|walnut|willow)\s+tree\b/g, "remove $1")
    .replace(/\b(remove\s+(?:ash|beech|birch|cedar|cherry|elm|hickory|locust|maple|oak|pear|pine|poplar|sycamore|walnut|willow))[\s.;:-]+tree\s+removal\b/g, "$1")
    .replace(/\b(remove\s+(?:two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\s+small\s+ornamental\s+pears)[\s.;:-]+tree\s+removal\b/g, "$1")
    .replace(/\b(trim\s+.+?\s+(?:limbs?|branches?))[\s.;:-]+tree\s+trim\b/g, "$1")
    .replace(/\b(trim\s+(?:branches?|limbs?))[\s.;:-]+tree\s+trim\b/g, "$1")
    .replace(/\btake\s+down\s+one\s+([a-z]+)\s+tree\b/g, "take down $1")
    .replace(/\bone\s+([a-z]+)\s+tree\b/g, "$1 tree")
    .replace(/\s+[.;:-]+\s*/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return text;
...
function resolvedTreeFactBaseScope(alphaJson = {}) {
  ...
  const action = evidence.match(/\b(remove|removal|take\s+down|cut\s+down|drop|trim|prune)\b/i)?.[1] || "";
  const species = asString(tree.tree_type).toLowerCase().trim();
  if (!action || !species || !TREE_SPECIES_SCOPE_RE.test(species)) return "";
  const countText = asString(tree.tree_count).toLowerCase();
  const countMatch = countText.match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|[1-9]|10)\b/);
  if (!countMatch) return "";
  const count = countMatch[1];
  const numeric = Number(count);
  const quantity = Number.isFinite(numeric)
    ? ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten"][numeric] || count
    : count;
  const noun = numeric === 1 || count === "one" ? "tree" : "trees";
  const normalizedAction = action === "removal" ? "remove" : action;
  return `${normalizedAction} ${quantity} ${species} ${noun}`;
}

function baseScopeFromJobFacts(alphaJson = {}, interpretation = {}) {
  const job = alphaJson.job || {};
  const evidence = [
    resolvedTreeFactBaseScope(alphaJson),
    alphaJson.normalization?.corrected_interpretation,
    alphaJson.normalization?.field_evidence?.work_scope,
    job.description,
  ];
  const baseDescription = normalizeScopeText(interpretation.base_description || "");
  const candidates = [];
  for (const source of evidence) {
    const stripped = removeAddOnScopeFromBase(source, interpretation.add_on_description);
    const concise = conciseBaseScope(stripped);
    if (!concise || isGenericCustomerScope(concise)) continue;
    if (baseDescription && !serviceCategory(`${concise} ${baseDescription}`)) continue;
    candidates.push(concise);
  }
  return candidates.sort((left, right) => scopeSpecificityScore(right) - scopeSpecificityScore(left))[0] || "";
}

function scopeSpecificityScore(value = "") {
  const text = normalizeScopeText(value);
  if (!text) return -100;
  if (isGenericCustomerScope(text)) return -50;
  let score = Math.min(text.length, 60);
  if (TREE_SPECIES_SCOPE_RE.test(text)) score += 25;
  if (/\b(?:limb|limbs|branch|branches|storm|damage|pear|pears)\b/.test(text)) score += 25;
  if (/\b(?:one|two|three|four|five|six|seven|eight|nine|ten|[1-9]|10)\b/.test(text)) score += 20;
  if (/\b(?:remove|removal|take down|cut up|cut down|trim|prune|clear|cleanup)\b/.test(text)) score += 15;
  return score;
}

function optionWithCustomerFacingScope(option = {}, scope = "", action = "confirmed_customer_facing_scope") {
  const clean = finishScopePhrase(scope);
...
function optionWithFinishedPhrases(option = {}) {
  const title = finishScopePhrase(option.title || "");
  const description = finishScopePhrase(option.description || "");
  return {
    ...option,
...
function serviceCategory(text) {
  ...
  if (/\bstumps?\b|\bstumping\b|\bgrind(?:ing)?\b/.test(value)) return "stump";
...
function hasRepeatedPriceSentence(rawText) {
  const sentences = String(rawText || "")
    .split(/(?<=[.!?])\s+/)
    .map((sentence) => sentence.toLowerCase().replace(/\s+/g, " ").trim())
    .filter((sentence) => (sentence.match(/\$\s*[0-9][0-9,]*/g) || []).length >= 2)
    .map((sentence) => {
      const start = sentence.search(/\b(?:quote|quoted|price|estimate|option|options)\b|\$/i);
...
function confirmedPriceCorrections(alphaJson = {}) {
  const claimsById = new Map((alphaJson.normalization?.claim_graph?.claims || []).map((claim) => [claim.id, claim]));
  return (alphaJson.normalization?.claim_graph?.relationships || [])
    .filter((relationship) => relationship.type === "supersedes" && relationship.confidence === "confirmed")
    .map((relationship) => {
      const previous = claimsById.get(relationship.to);
      const corrected = claimsById.get(relationship.from);
      if (previous?.field !== "job.price" || corrected?.field !== "job.price") return null;
      const previousAmount = Number(previous.value?.amount);
      const correctedAmount = Number(corrected.value?.amount);
      if (!Number.isFinite(previousAmount) || !Number.isFinite(correctedAmount)) return null;
      return {
        previousClaimId: previous.id,
        correctedClaimId: corrected.id,
        previousAmount: Math.round(previousAmount),
        correctedAmount: Math.round(correctedAmount),
      };
    })
    .filter(Boolean);
}

function interpretationIsExplicitCorrection(interpretation, corrections = []) {
  return corrections.some((correction) =>
    Number(interpretation?.base_price_value) === correction.previousAmount &&
    Number(interpretation?.add_on_price_value) === correction.correctedAmount,
  );
}
...
function shouldCollapseSameAmountOptions(group, rawText, prices) {
  ...
  const repeatedPriceSentence = hasRepeatedPriceSentence(rawText);

  if (duplicateScopes) return true;
  if (repeatedPriceSentence && hasRepeatedEvidence) return true;
...
function optionForSidecarPrice(price, index) {
  const scope = finishCustomerFacingScope(price.description_raw || price.context || "") || "work scope unclear";
...
function sequentialOptionLabel(index) {
  return `Option ${String.fromCharCode(65 + index)}`;
}

function normalizeComputedAddOnOptionSequence(options = [], interpretations = []) {
  if (!interpretations.length || options.length < 2) return options;
  let ordered = [...options];
  for (const interpretation of interpretations) {
    const baseAmount = Math.round(Number(interpretation.base_price_value));
    const totalAmount = Math.round(Number(interpretation.combined_price_value));
    if (!Number.isFinite(baseAmount) || !Number.isFinite(totalAmount)) continue;
    const baseIndex = ordered.findIndex((option) => amountFromOption(option) === baseAmount);
    const totalIndex = ordered.findIndex((option) => amountFromOption(option) === totalAmount);
    if (baseIndex < 0 || totalIndex < 0 || baseIndex < totalIndex) continue;
    const baseOption = ordered[baseIndex];
    const totalOption = ordered[totalIndex];
    ordered = ordered.filter((_, index) => index !== baseIndex && index !== totalIndex);
    ordered.splice(Math.min(baseIndex, totalIndex), 0, baseOption, totalOption);
  }
  return ordered.map((option, index) => ({
    ...option,
    label: sequentialOptionLabel(index),
    raw_label: "",
    sort_order: index + 1,
    preserve_order: true,
  }));
...
function customerFacingBaseScope(interpretation = {}, options = [], alphaJson = {}) {
  const stormScope = stormCleanupBaseScope(interpretation);
  if (stormScope) return stormScope;
  const explicitPearScope = explicitMultiOrnamentalPearScope(interpretation, alphaJson);
  if (explicitPearScope) return explicitPearScope;
  const optionScope = specificBaseScopeFromOption(baseOptionForInterpretation(options, interpretation) || {});
  const jobScope = baseScopeFromJobFacts(alphaJson, interpretation);
  const sidecarScope = customerFacingServiceScope(interpretation.base_description || "");
  if (interpretationReferencesPriorOption(interpretation) && sidecarScope && !isGenericCustomerScope(sidecarScope)) {
    return conciseBaseScope(sidecarScope);
  }
  if (jobScope && !isGenericCustomerScope(jobScope)) return jobScope;
  const candidates = [jobScope, sidecarScope, optionScope, cleanScope(interpretation.base_description || "")]
    .map(conciseBaseScope)
    .filter(Boolean);
  return candidates.sort((left, right) => scopeSpecificityScore(right) - scopeSpecificityScore(left))[0] || "";
}

function withCustomerFacingBaseScope(interpretation = {}, options = [], alphaJson = {}) {
  const baseScope = customerFacingBaseScope(interpretation, options, alphaJson);
  return baseScope ? { ...interpretation, base_description: baseScope } : interpretation;
}

function interpretationReferencesPriorOption(interpretation = {}) {
  const evidence = [
    interpretation.context,
    interpretation.add_on_description,
  ].filter(Boolean).join(" ");
  return /\b(?:includes?|including|included|same\s+as|based\s+on)\s+(?:(?:option|opt)\s*)?[a-e1-5]\b/i.test(evidence);
}

function baseScopeWithoutExcludedCleanup(baseScope = "", addOnScope = "") {
  if (!/\b(?:haul|hauling|cleanup|clean\s+up|debris|brush)\b/i.test(addOnScope)) return baseScope;
  return compact(baseScope)
    .replace(/\b(?:and\s+)?(?:leave|leaves|leaving|stack|stacks|stacking)\s+(?:the\s+)?(?:debris|brush|wood|logs?|limbs?|branches?)(?:\s+on\s+site|\s+there|\s+behind)?\b.*$/i, " ")
    .replace(/\b(?:debris|brush|wood|logs?)\s+(?:stays?|left|remain(?:s)?)\b.*$/i, " ")
    .replace(/\s+/g, " ")
    .replace(/^[\s:;.,=-]+|[\s:;.,=-]+$/g, "")
    .trim();
}
...
function combinedAddOnScope(interpretation) {
  const directTotalScope = explicitTotalCompleteOptionScope(interpretation);
  if (directTotalScope) return directTotalScope;
  let baseScope = conciseBaseScope(interpretation.base_description || "") || "base tree work";
  let addOnScope = (finishCustomerFacingScope(interpretation.add_on_description || "")
    .replace(/^(?:option\s*)?[a-e1-5]\s+(?:plus\s+)?/i, "")
    .replace(/^\+\s*/i, "")
    .replace(/^plus\s+/i, "")
    .replace(/\bhauling\b/i, "haul away")
    .trim()) || "add-on work";
  if (interpretationReferencesPriorOption(interpretation)) {
    baseScope = baseScopeWithoutExcludedCleanup(baseScope, addOnScope) || baseScope;
  }
  if (/\b(?:stump|stumping|grind)/i.test(addOnScope) &&
      /\b(?:two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\b/i.test(baseScope)) {
    addOnScope = addOnScope
      .replace(/\bgrind(?:ing)?\s+(?:the\s+)?stump\b/i, "grind stumps")
      .replace(/\bstump\s+grinding\b/i, "grind stumps");
  }
  if (/\bstorm[-\s]+damage\b|\bstorm\s+cleanup\b/i.test(baseScope) && /\bhaul|debris\b/i.test(addOnScope)) {
    return `${baseScope} with debris haul-away`;
  }
...
function explicitTotalCompleteOptionScope(interpretation = {}) {
  if (
  ...
  const addOnScope = finishCustomerFacingScope(interpretation.add_on_description || "");
  if (!addOnScope) return "";
  const normalized = normalizeScopeText(addOnScope);
  const describesPrimaryWork = /\b(?:remove|removal|take\s+down|cut\s+down|drop|trim|prune|pruning|cut\s+up|limb\s+removal|branch\s+removal|tree\s+removal|storm[-\s]+damage\s+cleanup|storm\s+cleanup)\b/i.test(normalized);
  if (!describesPrimaryWork) return "";
  return addOnScope;
}
...
function incrementalAddOnEvidence(interpretation = {}) {
  ...
  if (/\b(?:if|optional|separate|separately|either|or|unless|included|package|all[-\s]?in|bundle|bundled|discount|discounted|deal)\b/i.test(evidence)) return false;
  return Boolean(
...
function candidateDecision({
  ...
  explicitCorrectionPriceIds = new Set(),
  supersededCorrectionPriceIds = new Set(),
  preferBundledStatus = false,
}) {
  const priceKey = priceId || "";
  if (supersededCorrectionPriceIds.has(priceKey)) {
    return {
      candidate_status: "rejected",
      reason_code: "superseded_by_explicit_correction",
      reason: "This earlier price was superseded by a confirmed later correction.",
    };
  }
  if (explicitCorrectionPriceIds.has(priceKey)) {
    return {
      candidate_status: "accepted",
      reason_code: "accepted_explicit_correction",
      reason: "This price is the confirmed later correction for the earlier quote amount.",
    };
  }
...
function sidecarPriceById(prices = []) {
  return new Map(prices.map((price) => [price.price_id, price]).filter(([id]) => id));
}

function normalizedSidecarPriceScope(price = {}) {
  return basicCustomerFacingScope(price.description_raw || price.context || sidecarPriceScope(price))
    .toLowerCase()
    .replace(/[^a-z0-9\s/-]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function isStumpAddOnScope(value = "") {
  return /\bstumps?\b|\bstumping\b|\bgrind(?:ing)?\b/.test(value);
}

function isHaulCleanupAddOnScope(value = "") {
  return /\bhaul(?:ing|\s+away|\s+off)?\b|\bdebris\b|\bbrush\b|\bcleanup\b|\bclean\s+up\b|\brake\b|\bchip(?:ping)?\b/.test(value);
}

function isTreeRemovalBaseScope(value = "") {
  return /\bremove\b|\bremoval\b|\brmv\b|\btake\s+down\b|\bcut\s+down\b|\bdrop\b|\btree\s+removal\b/.test(value);
}

function isStormCleanupBaseScope(value = "") {
  return /\b(?:storm|wind|downed)\b/.test(value) &&
    /\b(?:clear|cleanup|clean\s+up|damage|debris|limbs?)\b/.test(value);
}

function isFallenTreeCleanupBaseScope(value = "") {
  return /\b(?:fallen|downed|on\s+ground)\b/.test(value) && isTreeRemovalBaseScope(value);
}

function isBrushCleanupAddOnScope(value = "") {
  return /\bbrush\b/.test(value) && /\b(?:cleanup|clean\s+up|clear|haul(?:ing|\s+away|\s+off)?)\b/.test(value);
}

function isTrimLimbBaseScope(value = "") {
  return /\btrim(?:ming)?\b|\bprun(?:e|ing)\b|\blimbs?\b|\bbranches?\b|\bcrown\b|\btree\s+trim\b|\blimb\s+removal\b/.test(value);
}

function totalComponentBundleMatch(base = {}, addOn = {}) {
  const baseScope = normalizedSidecarPriceScope(base);
  const addOnScope = normalizedSidecarPriceScope(addOn);
  if (isStumpAddOnScope(addOnScope) && isTreeRemovalBaseScope(baseScope)) {
    return {
      reason: "A high-confidence total/component relationship ties the early total to the base service plus stump grinding.",
    };
  }
  if (isHaulCleanupAddOnScope(addOnScope) && isTrimLimbBaseScope(baseScope)) {
    return {
      reason: "A high-confidence total/component relationship ties the early total to the base trim/limb service plus haul-away.",
    };
  }
  if (isHaulCleanupAddOnScope(addOnScope) && isStormCleanupBaseScope(baseScope)) {
    return {
      reason: "A high-confidence total/component relationship ties the early total to the base storm cleanup plus haul-away.",
    };
  }
  if (isBrushCleanupAddOnScope(addOnScope) && isFallenTreeCleanupBaseScope(baseScope)) {
    return {
      reason: "A high-confidence total/component relationship ties the early total to the fallen-tree removal plus brush cleanup.",
    };
  }
  return null;
}

function highConfidenceTotalComponentAddOnBundles(relationships = [], prices = []) {
  const pricesById = sidecarPriceById(prices);
  return relationships.map((relationship) => {
    if (relationship.confidence !== "high") return null;
    const componentIds = asArray(relationship.component_price_ids);
    if (componentIds.length !== 2) return null;
    const totalPrice = pricesById.get(relationship.total_price_id);
    const components = componentIds.map((id) => pricesById.get(id)).filter(Boolean);
    if (!totalPrice || components.length !== 2) return null;
    const totalAmount = Math.round(Number(relationship.total_amount));
    if (!Number.isFinite(totalAmount) || totalPrice.price_value !== totalAmount) return null;
    const pair = components
      .flatMap((first, index) => components.slice(index + 1).flatMap((second) => [
        { base: first, addOn: second, match: totalComponentBundleMatch(first, second) },
        { base: second, addOn: first, match: totalComponentBundleMatch(second, first) },
      ]))
      .find((candidate) => candidate.match);
    const base = pair?.base;
    const addOn = pair?.addOn;
    if (!base || !addOn) return null;
    if (base.price_value + addOn.price_value !== totalAmount) return null;
    if (totalPrice.amount_confidence !== "high") return null;
    if (confidenceRank(base.pairing_confidence) < confidenceRank("medium")) return null;
    if (confidenceRank(addOn.pairing_confidence) < confidenceRank("medium")) return null;
    if (!/\b(?:estimate|total|quote|all[-\s]?in|package|not\s+addr)\b/i.test(totalPrice.context || relationship.reason || "")) {
      return null;
    }
    return {
      interpretation_id: `total_component_${relationship.relationship_id}`,
      interpretation: "bundled_total",
      base_price_id: base.price_id,
      base_price_value: base.price_value,
      base_price_display: base.price_display || money(base.price_value),
      base_description: base.description_raw || base.context,
      add_on_price_id: totalPrice.price_id,
      add_on_price_value: totalAmount,
      add_on_price_display: totalPrice.price_display || money(totalAmount),
      add_on_component_price_id: addOn.price_id,
      add_on_component_price_value: addOn.price_value,
      add_on_description: addOn.description_raw || addOn.context,
      combined_price_value: totalAmount,
      combined_price_display: totalPrice.price_display || money(totalAmount),
      amount_confidence: "high",
      pairing_confidence: addOn.pairing_confidence,
      addon_interpretation_confidence: "high",
      price_role: EXPLICIT_OPTION_TOTAL,
      price_role_confidence: "high",
      price_role_evidence_text: pair.match.reason,
      price_role_ambiguity_reason: "",
      incremental_add_on_amount: addOn.price_value,
      needs_review: false,
      context: compact([totalPrice.context, base.context, addOn.context].filter(Boolean).join(" ")),
...
export function reconcileSidecarPrices(alphaJson = {}, optionPriceCandidateView = null) {
  ...
  const priceCorrections = confirmedPriceCorrections(reconciled);
  const rawPrices = sidecarPrices(optionPriceCandidateView);
  const prices = hasRepeatedPriceSentence(rawText)
    ? rawPrices.filter((price, index, allPrices) => allPrices.findIndex((candidate) => candidate.price_value === price.price_value) === index)
    : rawPrices;
  const explicitCorrectionAmounts = new Set(priceCorrections.map((correction) => correction.correctedAmount));
  const supersededCorrectionAmounts = new Set(priceCorrections.map((correction) => correction.previousAmount));
  const explicitCorrectionPriceIds = new Set(
    prices.filter((price) => explicitCorrectionAmounts.has(price.price_value)).map((price) => price.price_id),
  );
  const supersededCorrectionPriceIds = new Set(
    prices.filter((price) => supersededCorrectionAmounts.has(price.price_value)).map((price) => price.price_id),
  );
  const retainedPriceIds = new Set(prices.map((price) => price.price_id));
  const repeatedSentenceDuplicatePriceIds = hasRepeatedPriceSentence(rawText)
    ? new Set(rawPrices.filter((price) => !retainedPriceIds.has(price.price_id)).map((price) => price.price_id))
    : new Set();
  const addOnInterpretations = sidecarAddOnInterpretations(optionPriceCandidateView)
    .filter((interpretation) => !interpretationIsExplicitCorrection(interpretation, priceCorrections));
  const monetaryRelationships = sidecarMonetaryRelationships(optionPriceCandidateView);
  const independentAlternativeRelationships = asArray(
    optionPriceCandidateView?.pre_ai_option_price_candidate_clues?.alternatives,
  );
  ...
  const highConfidenceComputedAddOns = addOnInterpretations
    ...
    .map((interpretation) => withCustomerFacingBaseScope(interpretation, options, reconciled));
  const highConfidenceTotalComponentAddOns = highConfidenceTotalComponentAddOnBundles(monetaryRelationships, prices)
    .map((interpretation) => withCustomerFacingBaseScope(interpretation, options, reconciled));
  const highConfidenceBundledTotalInterpretations = [
    ...
    .map((interpretation) => withCustomerFacingBaseScope(interpretation, options, reconciled)),
    ...highConfidenceTotalComponentAddOns,
  ];
  const highConfidenceBundledTotalsByAmount = new Map(highConfidenceBundledTotalInterpretations
    .map((interpretation) => [interpretation.add_on_price_value, interpretation]));
  const highConfidenceBundledTotalPriceIds = new Set(highConfidenceBundledTotalInterpretations
  ...
  const addOnPriceIdsToCompute = new Set([
    ...highConfidenceComputedAddOns.map((interpretation) => interpretation.add_on_price_id),
    ...highConfidenceTotalComponentAddOns.map((interpretation) => interpretation.add_on_component_price_id),
  ].filter(Boolean));
  ...
  const addOnAmountsToReplace = new Set([
    ...highConfidenceComputedAddOns.map((interpretation) => interpretation.add_on_price_value),
    ...highConfidenceTotalComponentAddOns.map((interpretation) => interpretation.add_on_component_price_value),
  ].filter(Boolean));
  ...
  const distinctSidecarAmounts = new Set(prices.map((price) => Number(price.price_value)).filter((amount) => Number.isFinite(amount)));
  const explicitAlternativePair = prices.length === 2 && distinctSidecarAmounts.size === 2 &&
    /\b(?:without|with)\b.{0,80}\b(?:without|with)\b|\bor\b/i.test(rawText);
  const acceptedSidecarAmounts = new Set([
    ...explicitCorrectionAmounts,
    ...prices
      .filter((price) =>
        price.amount_confidence === "high" &&
        (price.pairing_confidence === "high" || explicitAlternativePair) &&
        !addOnPriceIdsToCompute.has(price.price_id) &&
  ...
  const relationshipBackedBundledAddOns = [
    ...highConfidenceComputedAddOns,
    ...highConfidenceTotalComponentAddOns,
  ];
  ...
  let updatedOptions = options.flatMap((option) => {
    ...
    if (!amount) {
      if (relationshipBackedBundledAddOns.length && optionScopeQuality(option) === "generic") return [];
      return [option];
    }
  ...
  const computedAddOnByBaseAmount = new Map(relationshipBackedBundledAddOns
    .map((interpretation) => [Math.round(Number(interpretation.base_price_value)), interpretation]));
  const computedAddOnByTotalAmount = new Map(relationshipBackedBundledAddOns
    .map((interpretation) => [Math.round(Number(interpretation.combined_price_value)), interpretation]));
  updatedOptions = updatedOptions.map((option) => {
    const amount = amountFromOption(option);
    const baseInterpretation = computedAddOnByBaseAmount.get(amount);
    if (baseInterpretation) {
      return optionWithCustomerFacingScope(
        option,
        customerFacingBaseScope(baseInterpretation, [option], reconciled),
        "confirmed_base_scope_for_computed_add_on",
      );
    }
    const totalInterpretation = computedAddOnByTotalAmount.get(amount);
    if (!totalInterpretation) return option;
    return optionWithCustomerFacingScope(
      option,
      combinedAddOnScope(totalInterpretation),
      "confirmed_expanded_scope_for_computed_add_on",
    );
  });
  ...
  for (const price of prices) {
    ...
    if (explicitCorrectionPriceIds.has(price.price_id)) {
      const option = optionForSidecarPrice(price, updatedOptions.length + addedOptions.length);
      addedOptions.push(option);
      existingAmounts.add(price.price_value);
      continue;
    }
    if (
      !highConfidenceBundledTotalPriceIds.has(price.price_id) &&
      acceptedTotalRelationshipsForAmounts(monetaryRelationships, existingAmounts).some((relationship) => relationship.total_price_id === price.price_id)
    ) {
      continue;
    }
  ...
  const collapsedOptions = normalizeComputedAddOnOptionSequence(duplicatePriceCollapse.items, relationshipBackedBundledAddOns)
    .map(optionWithFinishedPhrases);
  const finalOptions = duplicatePriceCollapse.collapses.length
  ...
  const acceptedBundlePriceIds = new Set([
    ...
    ...highConfidenceTotalComponentAddOns.map((interpretation) => interpretation.add_on_component_price_id).filter(Boolean),
  ]);
  const totalComponentAddOnByPair = new Map(highConfidenceTotalComponentAddOns
    .map((interpretation) => [`${interpretation.base_price_id}:${interpretation.add_on_component_price_id}`, interpretation]));
  const resolvedAddOnInterpretations = addOnInterpretations.map((interpretation) => {
    const resolved = totalComponentAddOnByPair.get(`${interpretation.base_price_id}:${interpretation.add_on_price_id}`);
    if (!resolved) return interpretation;
    return {
      ...interpretation,
      interpretation: "additive_amount",
      combined_price_value: resolved.combined_price_value,
      combined_price_display: resolved.combined_price_display,
      incremental_add_on_amount: resolved.incremental_add_on_amount,
      price_role: INCREMENTAL_ADDON_PRICE,
      price_role_confidence: "high",
      price_role_evidence_text: "A high-confidence total/component relationship confirms this amount is the incremental add-on component.",
      price_role_ambiguity_reason: "",
      addon_interpretation_confidence: "high",
      needs_review: false,
      review_reason: "",
  ...
  const finalAddOnInterpretations = resolvedAddOnInterpretations.filter((interpretation) =>
    !repeatedSentenceDuplicatePriceIds.has(interpretation.base_price_id) &&
    !repeatedSentenceDuplicatePriceIds.has(interpretation.add_on_price_id),
  );
  ...
  const duplicatePriceAmounts = new Set(duplicatePriceCollapse.collapses.map((collapse) => Number(collapse.amount)));
  const finalNeedsReview = hasRepeatedPriceSentence(rawText)
    ? needsReview.filter((entry) => !duplicatePriceAmounts.has(Number(entry.amount)))
    : needsReview;
    ...
    sidecar_price_reconciliation: {
      sidecar_prices: prices.map((price) => ({
        ...
        ...candidateDecision({
          ...
          explicitCorrectionPriceIds,
          supersededCorrectionPriceIds,
        }),
      })),
      add_on_interpretations: finalAddOnInterpretations.map((interpretation) => ({
        interpretation_id: interpretation.interpretation_id,
      ...
      alternatives: independentAlternativeRelationships,
      ...
      final_price_gate: {
        accepted_amounts: [...acceptedSidecarAmounts]
          .filter((amount) => !supersededCorrectionAmounts.has(Number(amount)))
  ...
  const blockingErrors = [
    ...finalNeedsReview.map((item) => item.kind === "add_on_interpretation"
