...
function combinedAddOnScope(interpretation) {
  ...
  if (/\b(?:stump|grind)/i.test(addOnScope) &&
...
function isStumpAddOnScope(value = "") {
  return /\bstumps?\b|\bgrind(?:ing)?\b/.test(value);
}
...
export function reconcileSidecarPrices(alphaJson = {}, optionPriceCandidateView = null) {
  ...
  const finalOptions = normalizeComputedAddOnOptionSequence(duplicatePriceCollapse.items, relationshipBackedBundledAddOns)
  ...
  reconciled.normalization = {
    ...
    sidecar_price_reconciliation: {
      ...
      add_on_interpretations: resolvedAddOnInterpretations.map((interpretation) => ({
        interpretation_id: interpretation.interpretation_id,
