...
function deploymentStage(env) {
  ...
  return ["development", "staging", "production"].includes(stage) ? stage : "development";
}
...
export function customerPipelinePolicy({ documentId = "", env = process.env } = {}) {
  ...
  const deliveryEnabled = stage !== "production" || enabledFlag(env[CUSTOMER_DELIVERY_ENABLED_FLAG]);
  ...
  const activeMode = assemblerEnabled
    ...
    : resolutionEnabled
      ...
      : "off";
  const deliveryBlocked = stage === "production" && (
    !deliveryEnabled ||
