...
export const READINESS_SAFETY_BLOCK_RE = /^Readiness check:/i;
...
export function normalizeReviewOverrides(value = {}) {
  return {
    ...
    acknowledgedReadinessRisk: Boolean(value.acknowledgedReadinessRisk),
  };
}
...
function overrideKeyForBlockingError(error) {
  ...
  if (READINESS_SAFETY_BLOCK_RE.test(text)) return "acknowledgedReadinessRisk";
  return "";
}

function readinessWarningForBlockingErrors(blockingErrors = []) {
  const readinessMessages = blockingErrors.filter((error) => READINESS_SAFETY_BLOCK_RE.test(String(error || "").trim()));
  if (!readinessMessages.length) return null;
  return {
    key: "acknowledgedReadinessRisk",
    title: "Readiness safety flags",
    message: "One or more readiness safety checks found something that needs explicit reviewer confirmation before this estimate can be sent.",
    findings: readinessMessages.map((error) => String(error).replace(READINESS_SAFETY_BLOCK_RE, "").trim()),
  };
}
...
export function getBlockingOverrideStatus(validation = {}, overrides = {}, alphaJson = {}) {
  ...
  const readinessWarning = readinessWarningForBlockingErrors(blockingErrors);
  ...
  if (readinessWarning && overrideAccepted("acknowledgedReadinessRisk", normalized)) {
    acceptedOverrideWarnings.push(readinessWarning);
  }
  ...
  return {
    ...
    needsReadinessOverride: requiredKeys.includes("acknowledgedReadinessRisk"),
