...
- If sidecar evidence says a lower later stump/haul/cleanup price is an add-on amount, account for the computed bundled option instead of treating the add-on amount as a standalone full option.
- Preserve every option's raw wording before cleanup.
- Correct obvious typos only when business meaning does not change.
- Correct obvious email domain typos only for email-shaped contact candidates, using a small common-provider and common-TLD allowlist; do not apply broad fuzzy matching outside email context.
