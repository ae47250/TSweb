...
function sidecarAddressFromExcludedNumbers(clues = {}) {
  const excluded = Array.isArray(clues.excluded_numbers) ? clues.excluded_numbers : [];
  const address = excluded.find((item) => item.kind === "address" && /\d+\s+\S+/.test(item.raw || ""));
  return cleanupExtractedValue(address?.raw || "");
}
...
export function buildEvidenceBackedCoherentNote({
  ...
  const incompleteEmail = !sidecarEmail
    ...
    : "";
  const sidecarAddress = sidecarAddressFromExcludedNumbers(clues);
  ...
  const serviceAddress = fallbackEvidence.service_address?.value || sidecarAddress;
  ...
  if (sidecarAddress && sidecarAddress !== fallbackEvidence.service_address?.value) {
    addRewriteTrace(rewriteTrace, {
      field: "service_address",
      type: "sidecar_excluded_number_evidence",
      before: fallbackEvidence.service_address?.value || "missing",
      after: sidecarAddress,
      reason: "Used address-shaped excluded-number evidence from option/price normalizer.",
