...
export const REVIEWER_DECISION_REASON_CODES = Object.freeze([
  "app_wrong",
  "customer_update",
  "formatting",
]);
...
export function createReviewerDecision({
  ...
  const candidate = asString(candidateId);
  if (candidate) {
    decision.candidateId = candidate;
    decision.candidateIds = [...new Set([...(decision.candidateIds || []), candidate])];
  }
  const option = asString(optionId);
  if (option) decision.optionId = option;
  const finding = asString(findingId);
  if (finding) decision.findingId = finding;
...
export function appendReviewerDecision(alphaJson = {}, decision) {
  const existing = Array.isArray(alphaJson?.reviewer_decisions)
    ...
    : [];
  if (!decision || typeof decision !== "object") return { ...alphaJson, reviewer_decisions: existing.slice() };
  const fingerprint = reviewerDecisionFingerprint(decision);
  if (existing.some((entry) => reviewerDecisionFingerprint(entry) === fingerprint)) {
    return { ...alphaJson, reviewer_decisions: existing.slice() };
  }
  return {
    ...alphaJson,
...
export function reviewerDecisionFingerprint(decision = {}) {
  return createHash("sha256").update(JSON.stringify({
    ...
    candidateIds: Array.isArray(decision.candidateIds) ? [...decision.candidateIds].sort() : [],
    ...
    extractionVersion: decision.extractionVersion || "",
    resolutionPolicyVersion: decision.resolutionPolicyVersion || "",
  })).digest("hex");
}
