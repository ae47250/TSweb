 ...
 * @returns {object|null}
 */
export function buildPhoneException(envelope = null) {
  ...
  const candidates = (Array.isArray(envelope.candidates) ? envelope.candidates : [])
    .filter((candidate) => candidate && candidate.status !== "rejected" && candidate.status !== "quarantined");
 ...
 * @returns {{ phone: object|null, treeScope: object|null, priceAlternatives: object|null }}
 */
export function getReviewDecisionExceptions(alphaJson = {}) {
  const decisions = decisionsBag(alphaJson);
  return {
    phone: buildPhoneException(decisions.phone || null),
