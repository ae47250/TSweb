...
export const SOURCE_FINAL_FACT_COVERAGE_VERSION = "source-final-fact-coverage-v0.3";
...
function contactEqualityResult({
  fact,
  label,
  code,
  sourceValue,
  ...
  if (reviewerOverride && !matches) {
    return {
      ...
      code,
      message: `${label} differs from the source candidate by recorded reviewer decision.`,
    };
  }
  return {
    option_label: "",
...
function selectedDecisionCandidate(decision = {}) {
  const selectedId = decision?.resolution?.selectedCandidateId;
  return (decision?.candidates || []).find((candidate) => candidate.id === selectedId) || null;
}

function sourceCorrectionSelected(decision, finalValue, normalize) {
  if (decision?.resolution?.reasonCode !== "explicit_correction") return false;
  const selected = selectedDecisionCandidate(decision);
  if (!selected) return false;
  const normalizedFinal = normalize(finalValue);
  const normalizedSelected = normalize(selected.value);
  return Boolean(normalizedFinal) && normalizedFinal === normalizedSelected;
}

function contactEqualityCoverage({ rawText, finalCustomer, finalJob, decisions = {} }) {
  ...
  const results = CONTACT_FACT_DEFINITIONS
    ...
    .map(([fact, label, code]) => contactEqualityResult({
      ...
      reviewerOverride: reviewerFields[fact]?.resolution?.resolvedBy === "reviewer",
      sourceCorrection: sourceCorrectionSelected(
        reviewerFields[fact],
        final[fact],
        normalizers[fact],
      ),
...
function treeQuantityFromText(value) {
  ...
  const countedSpeciesMatches = [...text.matchAll(new RegExp(`\\b${count}\\s+(?:dead\\s+|large\\s+|big\\s+|small\\s+|leaning\\s+|fallen\\s+)?(?:${speciesPattern})\\b`, "gi"))]
    .map((match) => ({ phrase: normalizeFactText(match[0]), amount: countTokenToNumber(match[1]) }))
    .filter((item) => Number.isFinite(item.amount) && item.amount > 0)
    .filter((item, index, items) => items.findIndex((candidate) => candidate.phrase === item.phrase) === index)
...
function workActionsFromText(value) {
  ...
  if (/\bfull\s+(?:tree\s+)?remov(?:e|al)\b/.test(text)) actions.push("full_removal");
  if (/\bcut\s+(?:the\s+)?rounds?\b/.test(text)) actions.push("cut_rounds");
  if (/\b(?:cut(?:\s+(?:only|down|tree|trees))?|drop(?:\s+(?:only|tree|trees))?)\b/.test(text) &&
      /\b(?:tree|trees|wood|logs?|leave|stack)\b/.test(text)) {
    actions.push("remove_tree");
  }
  if (/\bcut\b/.test(text) && /\b(?:haul|leave|stack|wood|logs?|debris)\b/.test(text)) actions.push("remove_tree");
  if (/(?:\bleave\b|\bstack\b).{0,30}\b(?:wood|logs?|brush|debris|chips?)\b/.test(text) ||
      /\b(?:wood|logs?|brush|debris|chips?)\b.{0,30}(?:\bleave\b|\bstack\b)/.test(text)) {
    actions.push("remove_tree");
  }
...
function sharedFactsForRawText(rawText = "") {
  ...
  return {
    ...
    price: null,
    stump_quantity: null,
    base_work_actions: facts.work_actions.filter((value) => value === "remove_tree"),
    work_actions: [],
    debris_disposition: [],
    stump_treatment: [],
  };
...
function inheritedFactsForSourceOption(sourceOption, sharedFacts = {}, localFactsByToken = new Map()) {
  ...
  return {
    ...
    work_actions: unique([
      ...(sharedFacts.base_work_actions || []),
      ...referencedFacts.flatMap((facts) => facts.work_actions || []),
    ]),
...
function factsWithFinalContext(finalFacts = {}, finalContextFacts = {}) {
  const finalText = compact(finalFacts.final_text);
...
function canonicalFactValue(factKey, value) {
  const normalized = normalizeFactText(value);
  if (factKey === "work_actions") {
    if (/^haul_(?:brush|debris)$/.test(normalized)) return "haul_away";
  }
  return normalized;
}

function valuesEqual(left, right, factKey = "") {
  return canonicalFactValue(factKey, left) === canonicalFactValue(factKey, right);
}

function valuePresentInFinal(factKey, sourceValue, finalFacts) {
  const finalValue = finalFacts[factKey];
  if (Array.isArray(finalValue)) {
    return finalValue.some((value) => valuesEqual(value, sourceValue, factKey));
  }
  return valuesEqual(finalValue, sourceValue, factKey);
}
...
function numericTreeQuantity(value) {
  const text = normalizeFactText(value);
  const match = text.match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\b/);
  if (!match) return null;
  return countTokenToNumber(match[1]);
}

function claimValueForCorrection(claim = {}) {
  if (claim?.field === "job.target_trees") {
    if (Array.isArray(claim.value)) return claim.value.length || null;
    return numericTreeQuantity(claim.evidence);
  }
  if (claim?.field === "job.price") return Number(claim.value?.amount);
  return null;
}

function explicitCorrectionForFact({ factKey, sourceValue, finalValue, decisions = {}, claimGraph = {} } = {}) {
  const field = factKey === "tree_quantity" ? "tree_count" : factKey === "price" ? "prices" : "";
  if (!field) return null;
  const decision = decisions[field];
  if (decision?.resolution?.reasonCode !== "explicit_correction") return null;
  const correction = decision.correction;
  const selected = selectedDecisionCandidate(decision);
  if (!correction || !selected) return null;
  const claimsById = new Map((claimGraph?.claims || []).map((claim) => [claim.id, claim]));
  const previousClaim = claimsById.get(correction.previousClaimId);
  const previousValue = claimValueForCorrection(previousClaim);
  const selectedValue = factKey === "tree_quantity"
    ? numericTreeQuantity(selected.value)
    : Number(selected.value?.amount);
  const normalizedSourceValue = factKey === "tree_quantity"
    ? numericTreeQuantity(sourceValue)
    : Number(sourceValue);
  const normalizedFinalValue = factKey === "tree_quantity"
    ? numericTreeQuantity(finalValue)
    : Number(finalValue);
  if (!Number.isFinite(previousValue) || !Number.isFinite(selectedValue)) return null;
  if (normalizedSourceValue !== previousValue || normalizedFinalValue !== selectedValue) return null;
  return {
...
function compareFact({
  ...
  const explicitCorrection = explicitCorrectionForFact({
    factKey,
    sourceValue: sourceFacts[factKey],
    finalValue: finalFacts[factKey],
    decisions,
    claimGraph,
  });
  if (explicitCorrection) {
    return {
      ...
      status: "resolved_correction",
      correction_recorded: true,
      previous_source_value: displayValue(factKey, explicitCorrection.previous_value),
      final_value: displayValue(factKey, explicitCorrection.resolved_value),
      claim_id: explicitCorrection.claim_id,
      previous_claim_id: explicitCorrection.previous_claim_id,
    };
  }
...
export function buildSourceFinalFactCoverage({
  ...
  const contactCoverage = contactEqualityCoverage({ rawText, finalCustomer, finalJob, decisions });
