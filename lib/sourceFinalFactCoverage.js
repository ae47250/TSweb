export const SOURCE_FINAL_FACT_COVERAGE_VERSION = "source-final-fact-coverage-v0.1";
...
function labelMatchesInText(text) {
  ...
  return [...text.matchAll(/(^|[\s,.;])([A-E])\s*[:.)-]?\s+(?=(?:remove|rmv|drop|cut|tree|trim|stump|grind|haul|chip|clean|cleanup|leave|stack)\b)/gi)]
    .map((match) => {
...
function workActionsFromText(value) {
  ...
  if (/\bchip(?:ping)?\b.{0,30}\b(?:brush|limbs?|branches?)\b|\b(?:brush|limbs?|branches?)\b.{0,30}\bchip(?:ping)?\b/.test(text)) actions.push("chip_brush");
  if (/\b(?:haul|hual|hawl)\b.{0,30}\bbrush\b|\bbrush\b.{0,30}\b(?:haul|hual|hawl)\b/.test(text)) actions.push("haul_brush");
  if (/\b(?:haul|hual|hawl)\b.{0,30}\bdebris\b|\bdebris\b.{0,30}\b(?:haul|hual|hawl)\b/.test(text)) actions.push("haul_debris");
  ...
  if (/\b(?:cleanup|clean up|clean the work area|clean work area|clean yard|clean the yard|rake(?: up)?)\b/.test(text)) actions.push("cleanup");
...
function debrisDispositionFromText(value) {
  ...
  if (/\bleave\b.{0,30}\bwood\b|\bwood\s+stays?\b/.test(text)) values.push("leave_wood");
  if (/\bleave\b.{0,30}\blogs?\b|\blogs?\s+stays?\b/.test(text)) values.push("leave_logs");
  if (/\bstack\b.{0,30}\bwood\b|\bwood\b.{0,30}\bstack\b/.test(text)) values.push("stack_wood");
  ...
  if (/\b(?:haul|hual|hawl)\b.{0,30}\bdebris\b|\bdebris\b.{0,30}\b(?:haul|hual|hawl)\b/.test(text)) values.push("haul_debris");
  ...
  if (/\bchip(?:ping)?\b.{0,30}\b(?:brush|limbs?|branches?)\b|\b(?:brush|limbs?|branches?)\b.{0,30}\bchip(?:ping)?\b/.test(text)) values.push("chip_brush");
  if (/\b(?:cleanup|clean up|clean the work area|clean work area|clean yard|clean the yard|rake(?: up)?)\b/.test(text)) values.push("cleanup");
...
function targetQualifiersFromText(value) {
  const text = normalizeFactText(value);
  return unique([
    phraseIfPresent(text, /\btouching\b.{0,20}\b(?:service|power|utility)\s+line\b|\b(?:service|power|utility)\s+line\b.{0,20}\btouching\b/, "touching service line"),
    phraseIfPresent(text, /\b(?:near|against|on)\b.{0,20}\b(?:service|power|utility)\s+line\b|\b(?:service|power|utility)\s+line\b/, "near service line"),
    ...
    phraseIfPresent(text, /\bby\b.{0,10}\bshed\b/, "by shed"),
    ...
    phraseIfPresent(text, /\bin\b.{0,10}\bback\s+yard\b|\bback\s+yard\b/, "back yard"),
...
function factsFromText({ label = "", price = null, text = "" } = {}) {
  return {
    ...
    tree_quantity: treeQuantityFromText(text),
    stump_quantity: stumpQuantityFromText(text),
...
export function buildSourceFinalFactCoverage({
  ...
  const sharedSourceFacts = sharedFactsForRawText(rawText);
  ...
  for (const sourceOption of sourceOptions) {
    ...
    const finalFacts = finalFactsForOption(finalEntry.option, finalEntry.index);
  ...
  return {
    version: SOURCE_FINAL_FACT_COVERAGE_VERSION,
    applicable: true,
    source_options: sourceFactRows,
    final_options: finalFactRows,
    results,
