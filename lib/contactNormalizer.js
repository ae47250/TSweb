...
const ADDRESS_BASE_PATTERN = new RegExp(
  `(?<![\\d./()\\-])(?!(?:\\d{3}[ \\t]+){2}\\d{4}\\b)\\b\\d{1,5}[ \\t]+(?:[A-Za-z0-9'-]+[ \\t]+){0,5}${ADDRESS_SUFFIX}\\b(?:[ \\t]+\\d+\\s*[NSEW]\\b)?`,
  "gi",
);
const LOCAL_TOWN_AFTER_ADDRESS_PATTERN = new RegExp(
  `^\\s*(?:(?:,|[-–—]+)\\s*|in\\s+)?(${LOCAL_TOWN_PATTERN})\\b(?:\\s*,?\\s*(Indiana|IN))?(?:\\s+(\\d{5}(?:-\\d{4})?))?`,
  "i",
);
const INDIANA_CITY_AFTER_ADDRESS_PATTERN =
  /^\s*(?:(?:,|[-–—]+)\s*|in\s+)?([A-Za-z][A-Za-z.'-]*(?:\s+[A-Za-z][A-Za-z.'-]*){0,2})\s*,?\s+(Indiana|IN)\b(?:\s+(\d{5}(?:-\d{4})?))?/i;
...
function addressCandidateOverlapsPhone(input, start, end) {
  const windowStart = Math.max(0, start - 24);
  const windowEnd = Math.min(input.length, end + 24);
  const window = input.slice(windowStart, windowEnd);
  const phoneMatch = window.match(PHONE_VALUE_PATTERN);
  if (!phoneMatch) return false;
  const phoneStart = windowStart + (phoneMatch.index || 0);
  const phoneEnd = phoneStart + phoneMatch[0].length;
  return phoneStart < end && start < phoneEnd;
}
...
function scanAddressCandidates(text, { source = "raw_unlabeled", includeSpans = true } = {}) {
  ...
  for (const match of input.matchAll(ADDRESS_BASE_PATTERN)) {
    ...
    if (addressCandidateOverlapsPhone(input, start, baseEnd)) continue;
    if (/\b(?:tree|trees|limb|limbs|branch|branches|brush|stump|stumps)\b/i.test(street)) continue;
