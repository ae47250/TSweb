...
import { attachPipelineDecisionEnvelopes } from "./attachPipelineDecisionEnvelopes.js";
import { normalizeContactFields } from "./contactNormalizer.js";
import { clearClientClaimGraph } from "./claimGraph.js";
import { clearClientDecisionEnvelopes } from "./decisionEnvelope.js";
...
function protectedContactSpans(contactNormalizationResult = {}) {
  return [
    ...
    ...asArray(contactNormalizationResult?.address?.candidates)
      .filter((candidate) => candidate.accepted && candidate.span)
      .map((candidate) => ({
        start: candidate.span.start,
        end: candidate.span.end,
...
function clearClientRouteEvidence(normalized = {}) {
  ...
  // Permanent reviewer ledger entries are server-owned. The request may carry
  // the simple action log, but it cannot submit durable ledger history.
  delete normalized.reviewer_decisions;
  delete normalized.review?.reviewer_decision_log;
  delete normalized.normalization.sidecar_price_reconciliation;
  delete normalized.normalization.route_validation_evidence;
  clearClientDecisionEnvelopes(normalized);
  clearClientClaimGraph(normalized);
  ...
  delete normalized.review.readiness_decisions;
  delete normalized.review.readiness_decision_errors;
}
...
function copyTrustedPriceReconciliationEvidence(target = {}, source = {}) {
  ...
  if (source?.service_options && typeof source.service_options === "object") {
    target.service_options = structuredClone(source.service_options);
  }
  if (sidecar) {
...
function copyTrustedDecisionEnvelopes(
  target = {},
  source = {},
  ...
  const sourceDecisions = source?.normalization?.decisions;
  if (sourceDecisions && typeof sourceDecisions === "object") {
    target.normalization = {
      ...(target.normalization || {}),
      decisions: {
        ...(target.normalization?.decisions || {}),
        ...structuredClone(sourceDecisions),
      },
    };
  }
  const resolved = attachPipelineDecisionEnvelopes(
    target,
    contactNormalizationResult,
    target?.raw_input?.customer_text || "",
    reviewerDecisionLog,
    reviewContext,
  );
  if (resolved !== target) {
...
export function preserveRouteValidationEvidence(normalized = {}, sourceAlphaJson = {}, context = {}) {
  return applyTrustedRouteValidationEvidence(normalized, {
    ...
    reviewerDecisionLog: context.reviewerDecisionLog || [],
...
export function validateAlphaJsonRoutePayload(body = {}, {
  ...
  const decisionLog = [
    ...(Array.isArray(body.decisionLog) ? body.decisionLog : []),
    ...(Array.isArray(body.readinessDecisions) ? body.readinessDecisions : []),
    ...(Array.isArray(body.reviewOverrides?.readinessDecisions) ? body.reviewOverrides.readinessDecisions : []),
  ];
  applyTrustedRouteValidationEvidence(normalized, {
    rawInput,
    intake,
    reviewerDecisionLog: decisionLog,
    reviewContext,
  });
