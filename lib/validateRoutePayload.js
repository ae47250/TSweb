...
function clearClientRouteEvidence(normalized = {}) {
  const trustedTreeCountDecision = normalized.normalization?.decisions?.tree_count
    ? structuredClone(normalized.normalization.decisions.tree_count)
    : null;
  ...
  if (trustedTreeCountDecision) {
    normalized.normalization.decisions = {
      ...(normalized.normalization.decisions || {}),
      tree_count: trustedTreeCountDecision,
    };
  }
...
function copyTrustedDecisionEnvelopes(target = {}, source = {}, contactNormalizationResult = null) {
  ...
  attachPipelineDecisionEnvelopes(
    target,
...
export function applyTrustedRouteValidationEvidence(normalized = {}, {
  ...
  try {
    ...
    copyTrustedDecisionEnvelopes(normalized, evidenceSource, contactNormalizationResult);
