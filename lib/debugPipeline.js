export function buildDebugPipelinePayload({
  ...
  const sourceFinalFactCoverage = validation?.alphaJson?.validation?.source_final_fact_coverage
  ...
  const optionPriceStage = optionPriceCandidateView
    ...
    : [];
  const sourceFinalCoverageStage = sourceFinalFactCoverage
    ? [{
        label: "Source-final coverage",
        meaning: "A lexicon-backed audit compared source facts against final TD2 output.",
        status: sourceFinalFactCoverage.applicable
          ? `${sourceFinalFactCoverage.blocking_codes?.length || 0} blockers, ${sourceFinalFactCoverage.warning_results?.length || 0} warnings`
          : "not applicable",
      }]
    : [];
  ...
  return {
    debugPipeline: {
      ...
      ...(contactNormalizationResult
        ...
        : {}),
      ...(alphaJson?.normalization?.decisions
        ? { decisionEnvelopes: alphaJson.normalization.decisions }
        : {}),
      ...(alphaJson?.normalization?.claim_graph
        ? { claimGraph: alphaJson.normalization.claim_graph }
        : {}),
      ...(optionPriceCandidateView
        ? { td1OptionPriceCandidateView: optionPriceCandidateView }
        : {}),
      ...(sourceFinalFactCoverage
        ? { sourceFinalFactCoverage }
        : {}),
      rawOpenAiDraftJson,
