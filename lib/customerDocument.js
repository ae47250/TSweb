...
import { buildCustomerEstimateView, CUSTOMER_ESTIMATE_VIEW_VERSION } from "./customerEstimateView.js";
import { estimateSemanticHash } from "./estimateSemantics.js";
...
function optionDescriptionAddsDetail(option = {}) {
  const title = compactOptionText(option.title || "");
  const description = compactOptionText(option.description || "");
  return Boolean(description && description !== title);
}

export function buildCustomerRenderFacts(alphaJson = {}) {
  const view = alphaJson?.version === CUSTOMER_ESTIMATE_VIEW_VERSION
    ? alphaJson
    : buildCustomerEstimateView(alphaJson);
  return {
    document: {
      number: view.document?.number || "",
      title: view.document?.title || "",
      date_display: view.document?.dateDisplay || "",
    },
    customer: {
      name: view.customer?.name || "Customer",
      phone: view.customer?.phone || "",
    },
    serviceAddress: view.serviceAddress || "",
    workDescription: view.workDescription || "",
    notes: view.notes || "No additional customer-visible notes.",
    options: view.options || [],
    semanticHash: view.document?.semanticHash || "",
  };
}
...
export function renderCustomerDocument(alphaJson, { mobile = false, signature = "", selectedOption = "", signedAtDisplay = "" } = {}) {
  ...
  const semanticHash = facts.semanticHash;
  const optionHtml = options
    .map((option) => {
      const selected = selectedOption === option.label ? " selected" : "";
      return `<div class="option-button${selected}" data-option="${esc(option.label)}">
        <strong>${esc(option.label)}:</strong> ${esc(option.title)}
        <span>${esc(option.priceDisplay)}</span>
        ${optionDescriptionAddsDetail(option) ? `<small>${esc(option.description)}</small>` : ""}
      </div>`;
    })
    .join("");
...
<head>
  ...
  <title>${esc(facts.document.number)} ${mobile ? "Mobile" : "Full"} Estimate</title>
  <style>
...
<body>
  <main class="page" data-estimate-semantic-hash="${esc(semanticHash)}">
    <header>
      <h1>Alpha Tree Service</h1>
      <div>${esc(facts.document.title)} | ${esc(facts.document.number)} | ${esc(facts.document.date_display)}</div>
    </header>
    <section class="meta">
      <div class="box"><strong>Customer</strong><br>${esc(facts.customer.name)}<br>${esc(facts.customer.phone)}</div>
      <div class="box"><strong>Service Address</strong><br>${esc(facts.serviceAddress)}</div>
    </section>
    <section>
      <h2>Work Description</h2>
      <div class="box">${esc(facts.workDescription)}</div>
    </section>
    <section>
      <h2>Options</h2>
    ...
    <section>
      <h2>Notes</h2>
      <div class="box notes">${esc(facts.notes)}</div>
    </section>
    <section>
      <h2>Authorization</h2>
...
export function renderTreeDudeDocument(alphaJson, { warnings = [] } = {}) {
  ...
  const semanticHash = alphaJson.validation?.estimate_semantic_hash || estimateSemanticHash(alphaJson);
  ...
  const warningHtml = warningItems.length
    ? warningItems
      .map((warning) => {
        const title = typeof warning === "string" ? "Review note" : warning.title;
        const message = typeof warning === "string" ? warning : warning.message;
        return `<li><strong>${esc(title)}:</strong> ${esc(message)}</li>`;
      })
      .join("")
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${esc(alphaJson.document?.number)} Contractor Copy</title>
  <style>
    body { font-family: Arial, sans-serif; color: #222; margin: 0; background: #f5f5f5; }
...
<body>
  <main class="page" data-estimate-semantic-hash="${esc(semanticHash)}">
    <header>
      <h1>Alpha Tree Service - Contractor Copy</h1>
    ...
    <section class="warning-box">
      ...
      <ul>${warningHtml}</ul>
      <p>These warnings are for the contractor copy only. They are not printed on the customer estimate.</p>
    </section>
    <section class="meta">
