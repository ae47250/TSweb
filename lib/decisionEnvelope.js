 ...
 * @typedef {"single_explicit_candidate"|"explicit_correction"|"structured_intake_preferred"|"duplicate_equivalent_candidates"|"stronger_scope_pairing"|"conflicting_supported_candidates"|"insufficient_support"|"reviewer_override"} ResolutionReasonCode
 ...
 * @property {string} [suggestedCandidateId]
 * @property {string[]} [selectedCandidateIds]
...
export const RESOLUTION_REASON_TEXT = Object.freeze({
  ...
  return {
    ...
    resolution: {
      ...
      ...(resolution.selectedCandidateId
        ...
        : {}),
      ...(resolution.suggestedCandidateId
        ? { suggestedCandidateId: asString(resolution.suggestedCandidateId) }
        : {}),
      ...(Array.isArray(resolution.selectedCandidateIds)
        ? { selectedCandidateIds: resolution.selectedCandidateIds.map(asString).filter(Boolean) }
        : {}),
      reasonCode: resolution.reasonCode || "insufficient_support",
      ...(resolution.reasonText
        ? { reasonText: asString(resolution.reasonText) }
        : { reasonText: RESOLUTION_REASON_TEXT[resolution.reasonCode || "insufficient_support"] }),
...
export function validateDecisionEnvelope(envelope) {
  ...
  if (!envelope.resolution || typeof envelope.resolution !== "object") {
    ...
    if (
      envelope.resolution.suggestedCandidateId &&
      !envelope.candidates.some((candidate) => candidate.id === envelope.resolution.suggestedCandidateId)
    ) {
      errors.push("suggestedCandidateId must match a candidate id");
    }
    if (Array.isArray(envelope.resolution.selectedCandidateIds)) {
      for (const candidateId of envelope.resolution.selectedCandidateIds) {
        if (!envelope.candidates.some((candidate) => candidate.id === candidateId)) {
          errors.push(`selectedCandidateIds must match candidate id: ${candidateId}`);
        }
      }
    }
  }
