...
export async function notifyContractor({ documentId, customerName, address, selectedOption, price }) {
  const payload = {
    documentId,
    sms: {
      to: TREE_DUDE_PHONE,
      provider: "pingram",
      message: `Signed estimate received: ${customerName || "Customer"} ${address || ""} ${selectedOption || ""} ${price || ""}`.trim(),
    },
    email: {
      to: TREE_DUDE_EMAIL,
