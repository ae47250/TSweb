  ...
  if (channel === "email") {
    return {
      ...
      email: {
        ...
        html: escapeHtml(compact(message)).replace(/\n/g, "<br>"),
...
function pingramEndpoint(channel) {
  const base = compact(PINGRAM_API_URL).replace(/\/(?:send|sms|email)\/?$/i, "");
  if (channel === "sms") return `${base}/sms`;
  return `${base}/send`;
}

export function formatPingramErrorDetail(value) {
  if (typeof value === "string") return compact(value);
  if (value == null) return "";
  if (typeof value !== "object") return compact(value);

  for (const key of ["message", "error", "detail", "description"]) {
    const nested = formatPingramErrorDetail(value[key]);
  ...
  try {
    ...
    return "Pingram returned an unreadable error object.";
  }
}

function pingramErrorDetail(data, response) {
  const detail = formatPingramErrorDetail(data?.error)
    || formatPingramErrorDetail(data?.message)
    || formatPingramErrorDetail(data?.detail)
    || formatPingramErrorDetail(data?.errors)
    || formatPingramErrorDetail(data)
    || response.statusText
    || "Pingram request failed.";
  return `Pingram ${response.status} ${detail}`.trim();
}
...
async function sendPingram({ channel, to, subject = "", message }) {
  ...
  if (MOCK_NOTIFICATIONS) {
    if (isStrictDeliveryStage()) {
      throw new Error("Real Pingram notifications are required in staging and production.");
    }
    return mockResult({ channel, to: recipient, subject, message, payload });
  }
  ...
  const response = await fetch(pingramEndpoint(channel), {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Pingram ${channel} failed: ${pingramErrorDetail(data, response)}`);
  }
