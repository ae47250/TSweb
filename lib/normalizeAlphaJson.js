...
import { attachDecisionEnvelopes } from "./decisionEnvelope.js";
...
import { buildTreeCountDecisionEnvelope } from "./treeCountDecisionEnvelope.js";
...
const CORRECTION_RULES = [
  ...
  { pattern: /\bhawlin\b/gi, original: "hawlin", corrected: "hauling", reason: "Corrected haul-away typo." },
  ...
  { pattern: /\bstmp\b/gi, original: "stmp", corrected: "stump", reason: "Corrected stump shorthand typo." },
  { pattern: /\bhaull\b/gi, original: "haull", corrected: "haul", reason: "Corrected haul-away typo." },
  { pattern: /\bbrsh\b/gi, original: "brsh", corrected: "brush", reason: "Corrected brush typo." },
  ...
  { pattern: /\bclnup\b/gi, original: "clnup", corrected: "cleanup", reason: "Corrected cleanup typo." },
...
function targetScopePhrases(alphaJson = {}) {
  const text = normalizeTreeServiceText([
    alphaJson.raw_input?.customer_text,
    alphaJson.normalization?.field_evidence?.work_scope,
    alphaJson.job?.description,
  ].filter(Boolean).join(" ")).toLowerCase();
  const phrases = [];
  const add = (pattern, phrase) => {
    if (pattern.test(text) && !phrases.includes(phrase)) phrases.push(phrase);
  };

  add(/\bbehind\s+(?:the\s+)?garage\b/i, "behind the garage");
  add(/\b(?:by|close to)\s+(?:the\s+)?fence\b/i, "by the fence");
  add(/\bnear\s+(?:the\s+)?driveway\b/i, "near the driveway");
  add(/\b(?:by|near)\s+(?:the\s+)?shed\b/i, "by the shed");
  add(/\bnear\s+(?:the\s+)?alley\b/i, "near the alley");
  add(/\b(?:by|near)\s+(?:the\s+)?creek\b/i, "by the creek");
  add(/\b(?:by|near)\s+(?:the\s+)?porch\b/i, "by the porch");
  add(/\b(?:by|near|next to)\s+(?:the\s+)?barn\b/i, "by the barn");
  add(/\b(?:near|by)\s+(?:the\s+)?(?:22|twenty[-\s]+two)[-\s]+foot\s+drive\s+opening\b/i, "near the 22-foot drive opening");
  add(/\b(?:in|near)\s+(?:the\s+)?back\s+yard\b/i, "in the back yard");
  return phrases;
}
...
export function buildCustomerJobSummary(alphaJson = {}) {
  ...
  const resolvedCountNumber = Number(tree.tree_count?.match?.(/\d+/)?.[0] || 0);
  const countedSpeciesMatchesResolvedCount = !resolvedCountNumber || Number(countedSpecies?.count) === resolvedCountNumber;
  if (countedSpecies?.phrase && Number(countedSpecies.count) > 1 && countedSpeciesMatchesResolvedCount) {
...
function sourceWorkScopeFromFirstWorkCue(value) {
  const text = normalizeTreeServiceText(stripEmails(stripStructuredContactLines(stripContactOnlyFollowUpLines(value))))
    .replace(PHONE_PATTERN, " ")
    .trim();
  const match = text.match(
    /\b(?:remove|removed|removal|trim|trimming|cut|drop|take\s+down|haul|cleanup|clean|grind|(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(?:[a-z]+\s+){0,4}(?:trees?|limbs?|branches?|stumps?|brush|oaks?|maples?|pines?|elms?|ashes?|cedars?|sycamores?|hickories?|locusts?|birches?))\b(?!\s+(?:Street|St|Road|Rd|Ave|Avenue|Drive|Dr|Lane|Ln|Court|Ct|Way|Blvd|Boulevard|Highway|Hwy|Route|Pike|Circle|Cir|Place|Pl|Terrace|Ter|Trail|Trl|Parkway|Pkwy|Bend|Main)\b)/i,
  );
  if (!match) return rawTextWithoutLeadContact(value);

  let workText = text.slice(match.index).trim();
  const boundary = workText.search(new RegExp(
    `\\b(?:option|opt)\\s*[A-E1-5]\\b|\\b(?:quote|quoted|prices?|estimate|cost|total)\\b|\\b(?:service\\s+address|address)\\b|\\b\\d{1,5}\\s+(?:[A-Za-z0-9.]+\\s+){0,5}${ADDRESS_SUFFIX}\\b`,
    "i",
  ));
  if (boundary >= 0) {
    const beforeBoundary = workText.slice(0, boundary).trim();
    const boundaryText = workText.slice(boundary);
    const addressBoundary = /\b(?:service\s+address|address)\b/i.test(boundaryText) ||
      /\b\d{1,5}\s+(?:[A-Za-z0-9.]+\s+){0,5}(?:Street|St|Road|Rd|Ave|Avenue|Drive|Dr|Lane|Ln|Court|Ct|Way|Blvd|Boulevard|Highway|Hwy|Route|Pike|Circle|Cir|Place|Pl|Terrace|Ter|Trail|Trl|Parkway|Pkwy|Bend|Main)\b/i.test(boundaryText);
    workText = addressBoundary
      ? beforeBoundary.replace(/,\\s*[A-Za-z][A-Za-z.'-]*(?:\\s+[A-Za-z][A-Za-z.'-]*){0,3}\\s*$/i, "").trim()
      : beforeBoundary;
  }
  return workText.replace(/\s+(?:at|by|near|on)\s*$/i, "").replace(/[\s,;:.-]+$/, "").trim();
}
...
function buildEvidenceViews(rawInput) {
  ...
  return {
    ...
    sourceWorkScopeEvidence: sourceWorkScopeFromFirstWorkCue(contactOnlyFollowUpsRemoved),
  };
}
...
function buildBasicNormalizationFromRawInput(rawInput, alphaJson) {
  ...
  const priceClues = buildOptionPriceCandidateView(rawInput)
    ?.pre_ai_option_price_candidate_clues?.money_like_numbers || [];
  ...
  normalized.field_evidence = {
    customer_name: extractNameFromRaw(rawInput),
    phone: extractPhoneFromRaw(rawInput),
    email: extractEmailFromRaw(rawInput),
    service_address: extractAddressFromRaw(rawInput),
    tree_count: extractTreeCountFromRaw(rawInput),
    tree_type: extractTreeTypeFromRaw(rawInput),
    work_scope: evidenceViews.sourceWorkScopeEvidence,
    price: priceClues.map((clue) => clue.price_display || clue.raw).filter(Boolean),
    // Option parsing has its own raw-input resolver. Keeping this legacy
    // field empty prevents source prose from being re-parsed as final scope.
    options: [],
  };
...
function normalizeUncertaintyField(value) {
  return asString(value)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
function hasNormalizationUncertainty(normalization, fields) {
  const normalizedFields = fields.map(normalizeUncertaintyField);
  const items = [
  ...
  return items.some((item) => {
    const normalizedItemField = normalizeUncertaintyField(item?.field);
    if (!normalizedItemField) return false;
    return normalizedFields.some((field) =>
      normalizedItemField === field ||
      normalizedItemField.endsWith(`_${field}`) ||
      field.endsWith(`_${normalizedItemField}`),
    );
  });
}
...
function baseJobScopePhrase(alphaJson) {
  ...
  if (quantity || treeType) {
    ...
    const base = normalizeTreeServiceText(`${action} ${[quantity, treeType, noun].filter(Boolean).join(" ")}`).toLowerCase();
    return [base, ...targetScopePhrases(alphaJson)].join(" ");
  }
  ...
  if (/\b(options? include|haul away|cleanup|stump grinding|root grinding|site cleanup|final cleanup|yard protection cleanup|access cleanup|chip brush|leave wood on site|stack firewood|mulch chip and leave)\b/i.test(jobCandidate)) return "";
...
function optionAddOns(option) {
  ...
  const addOns = [];
  if (/\b(?:root\s+grind(?:ing)?|grind\s+roots?|root\s+grinding)\b/i.test(text)) {
    addOns.push("stump grinding");
  }
  const hasHaulAway =
    /\b(?:haul(?:[\s-]+away|[\s-]+off)?\s+debris|debris\s+haul|haul\s+debris|remove\s+(?:all\s+)?debris|haul\s+everything|wood\s+removal|log\s+hauling|haul\s+logs?|haul\s+wood|haul(?:[\s-]+away|[\s-]+off)?\s+the\s+(?:debris|wood|logs?)|debris\s+removal)\b/i.test(text) ||
    (/\bhaul(?:[\s-]+away|[\s-]+off)?\b/i.test(text) && !/\b(?:brush|debris|wood|logs?|everything)\b/i.test(text));
  if (hasHaulAway) {
    addOns.push("haul away");
  }
  if (/\bhaul(?:[\s-]+away|[\s-]+off)?\b.{0,30}\bbrush\b|\bbrush\b.{0,30}\bhaul(?:[\s-]+away|[\s-]+off)?\b/i.test(text)) {
    addOns.push(/\bhaul\s+off\s+brush\b/i.test(text) ? "haul off brush" : "haul away the brush");
  }
  if (/\b(?:chip(?:ping)?\s+brush|brush\s+chipping|mulch\s+chip(?:\s+and\s+leave)?|chip(?:\s+and\s+leave))\b/i.test(text)) {
    addOns.push(/\band\s+leave\b/i.test(text) ? "chip brush and leave chips on site" : "chip brush");
  }
  if (/\b(?:leave|leaves|leaving)\b.{0,30}\b(?:brush|debrs?)\b|\b(?:brush|debrs?)\s+(?:stays?|left|remain(?:s)?)\b/i.test(text)) {
    addOns.push(/\bdebrs?\b/i.test(text) ? "leave the debris on site" : "leave the brush on site");
  }
  if (/\b(?:leave\s+(?:wood|logs?)\s+(?:on\s+site|stacked)?|stack\s+(?:firewood|wood|logs?))\b/i.test(text)) {
    addOns.push("leave wood on site");
  }
  if (/\b(?:cleanup|clean\s+up|clean\s+yard|site\s+cleanup|final\s+cleanup|yard\s+protection\s+cleanup|access\s+cleanup|sweep|rake(?:[-\s]+up)?|yard\s+cleanup)\b/i.test(text)) addOns.push("cleanup");
  if (/\bstump\s+(?:grind|grinding)|grind\s+(?:stumps?|the\s+stump)\b|\bflush\s+cut\s+stump\b|\bcut\s+stump\s+low\b/i.test(text)) addOns.push("stump grinding");
  return [...new Set(addOns)];
}

function sourceOptionForConstruction(alphaJson, option) {
  const flags = option?.review_flags || {};
  if (optionDescriptionWasEditedByTd(option)) return null;
  if (flags.scope_approved_by_reviewer || flags.source_to_final_verified_override) return null;
  const token = rawOptionLabelToken(option);
  if (!token) return null;
  return extractLabeledOptionsFromRaw(alphaJson.raw_input?.customer_text || "")
    .find((candidate) => rawOptionLabelToken(candidate) === token) || null;
}

function rawSourceOptionForConstruction(alphaJson, option) {
  const token = rawOptionLabelToken(option);
  if (!token || optionDescriptionWasEditedByTd(option)) return null;
  const flags = option?.review_flags || {};
  if (flags.scope_approved_by_reviewer || flags.source_to_final_verified_override) return null;
  const raw = asString(alphaJson.raw_input?.customer_text);
  const directBody = raw.match(new RegExp(
    `\\b(?:option|opt)\\s*${token}\\b\\s*[:.)-]?\\s*(.*?)(?=\\b(?:option|opt)\\s*[A-E1-5]\\b|\\bFollow-up\\s+\\d+\\s*:|$)`,
    "is",
  ))?.[1] || "";
  if (optionAddOns({ description: directBody }).length ||
      /\b(?:remove|removal|take\s+down|cut|drop|trim|tree|trees|limb|branch|stump)\b/i.test(directBody)) {
    return { description: directBody };
  }
  const addOnBeforeLabel = raw.match(new RegExp(
    `([^.!?\\n]+?)\\s+(?:is|are)\\s+included\\s+in\\s+(?:option|opt)\\s*${token}\\b`,
    "i",
  ))?.[1]?.trim() || "";
  return addOnBeforeLabel ? { description: addOnBeforeLabel } : null;
}

function sourceOptionForInference(alphaJson, option) {
  if (!option?.review_flags?.inferred_base_scope) return null;
  return sourceOptionForConstruction(alphaJson, option);
}

function sourceOptionHasExplicitCompleteScope(option) {
  const text = optionText(option);
  return /\bclean\s+yard\b/i.test(text) &&
    /\b(?:remove|removal|take\s+down|cut\s+down|drop|trim)\b/i.test(text) &&
    /\b(?:haul|debris|brush|cleanup|clean)\b/i.test(text);
}

function optionDispositionKind(option) {
  const text = optionText(option);
  if (/\b(?:leave|leaves|leaving|stack|stacked)\b.{0,30}\b(?:wood|logs?|brush|debris)\b|\b(?:wood|logs?|brush|debris)\s+(?:stays?|left|remain(?:s)?)\b/i.test(text)) {
    return "leave_on_site";
  }
  if (/\b(?:haul|remove)\b.{0,30}\b(?:away|off|debris|brush|wood|logs?)\b|\bdebris\s+(?:haul|removal)\b/i.test(text)) {
    return "haul_away";
  }
  return "";
}

function sourceOptionHasDispositionChange(sourceOption, option) {
  const sourceDisposition = optionDispositionKind(sourceOption);
  return Boolean(sourceDisposition && sourceDisposition !== optionDispositionKind(option));
}

function sourceOptionIsAddOnOnly(option) {
  const text = optionText(option);
  return optionAddOns(option).length > 0 &&
    !/\b(?:remove|removal|take\s+down|cut\s+down|cut|drop|trim|tree|trees|limb|limbs|branch|branches|stump|stumps)\b/i.test(text);
}

function scopeWithoutInheritedTargets(alphaJson, baseScope) {
  return targetScopePhrases(alphaJson)
    .reduce((scope, target) => scope.replace(new RegExp(`\\s*${escapeRegExp(target)}`, "ig"), ""), baseScope)
    .replace(/\s+/g, " ")
    .trim();
}

function sourceOptionDescription(option) {
  return cleanOptionDisplayPhrase(option?.description || option?.title || "").toLowerCase();
}

function rawSourceOptionHasExplicitCompleteScope(alphaJson, option) {
  const token = rawOptionLabelToken(option);
  ...
  return Boolean(body && sourceOptionHasExplicitCompleteScope({ description: body }));
}

function optionNeedsBaseScope(option, baseScope = "") {
  const text = optionText(option);
  const addOns = optionAddOns(option);
  const hasSpecificTreeFact = /\b(?:trees?|oak|pine|maple|elm|ash|cedar|sycamore|hickory|locust|birch|spruce|walnut|cherry|limbs?|branches?)\b/i.test(text);
  if (!addOns.length && !baseScope) return false;
  if (!baseScope) return !hasSpecificTreeFact;

  const baseText = normalizeTreeServiceText(baseScope).toLowerCase();
  const baseHasSpecies = /\b(?:oak|pine|maple|elm|ash|cedar|sycamore|hickory|locust|birch|spruce|walnut|cherry)\b/i.test(baseText);
  const baseHasQuantity = /\b(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(?:[a-z]+\s+){0,2}trees?\b/i.test(baseText);
  const baseTargets = targetScopePhrases({ raw_input: { customer_text: baseText } });
  const missingSpecies = baseHasSpecies && !hasSpecificTreeFact;
  const missingQuantity = baseHasQuantity && !/\b(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(?:[a-z]+\s+){0,2}trees?\b/i.test(text);
  const missingTarget = baseTargets.some((target) => !normalizeTreeServiceText(text).toLowerCase().includes(target));
  return addOns.length > 0 || missingSpecies || missingQuantity || missingTarget;
}
...
function resolvedTreeCountKey(value) {
  ...
  const match = text.match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+\+?)\s+(?:[a-z]+\s+){0,3}(?:trees?|oaks?|maples?|pines?|elms?|ashes?|cedars?|birches?|spruces?|walnuts?|hickories?|locusts?|sycamores?|cherries?)\b/i);
  if (!match) return "";
...
function recoverExplicitSourceOptionScopes(alphaJson) {
  if (!/\bclean\s+yard\b/i.test(asString(alphaJson.raw_input?.customer_text))) return;
  const options = alphaJson.service_options?.items || [];
  alphaJson.service_options.items = options.map((option) => {
    const sourceOption = sourceOptionForInference(alphaJson, option);
    if (!sourceOption || !rawSourceOptionHasExplicitCompleteScope(alphaJson, option)) return option;
    return withInferredOptionScope(option, sourceOptionDescription(sourceOption), {
      inferred_base_scope: true,
      inferred_from_source_option: true,
    });
  });
}

export function applyOptionScopeInference(alphaJson) {
  const options = alphaJson.service_options?.items || [];
  ...
  alphaJson.service_options.items = options.map((option, index, allOptions) => {
    // Revalidation may rewrite an inferred option from generated job wording.
    // Recover add-ons only from the matching raw option, never from that wording.
    const sourceOption = sourceOptionForInference(alphaJson, option);
    const constructionSourceOption = rawSourceOptionForConstruction(alphaJson, option) ||
      sourceOptionForConstruction(alphaJson, option);
    if (sourceOption && sourceOptionHasExplicitCompleteScope(sourceOption)) {
      return withInferredOptionScope(option, sourceOptionDescription(sourceOption), {
        inferred_base_scope: true,
        inferred_from_source_option: true,
      });
    }
    const inferenceOption = sourceOption && sourceOptionHasDispositionChange(sourceOption, option)
      ? sourceOption
      : option;
    const addOns = optionAddOns(inferenceOption);
    const inferenceBaseScope = constructionSourceOption && sourceOptionIsAddOnOnly(constructionSourceOption)
      ? scopeWithoutInheritedTargets(alphaJson, baseScope)
      : baseScope;
    const baseOnly = optionIsBaseOnly(option);
    const inferredDescription = addOns.length
      ? combineBaseScopeAndAddOns(inferenceBaseScope, addOns)
      : baseOnly
        ? combineBaseScopeAndOnly(inferenceBaseScope)
        : inferenceBaseScope;
    ...
    if (optionTreeCountDisagreesWithBaseScope(option, baseScope) && baseScopeHasResolvedTreeCount(baseScope)) {
      return withInferredOptionScope(option, inferredDescription, {
        inferred_base_scope: true,
        inferred_from_job_scope: baseScope,
        refreshed_from_tree_count_correction: true,
        ...(baseOnly ? { inferred_base_only_scope: true } : {}),
      });
    }
    ...
    if (!addOns.length || !optionNeedsBaseScope(option, inferenceBaseScope) || !hasEarlierLowerPricedOption(allOptions, index)) {
      return option;
    }
    return withInferredOptionScope(option, combineBaseScopeAndAddOns(inferenceBaseScope, addOns), {
      inferred_base_scope: true,
      inferred_from_job_scope: baseScope,
    });
  });
...
function selectedDecisionCandidate(envelope = {}) {
  if (envelope?.resolution?.status && envelope.resolution.status !== "selected") return null;
...
function lockedFact(value, { source = "source_notes", envelope = null, evidence = [] } = {}) {
  const candidate = selectedDecisionCandidate(envelope);
  return {
    value: candidate?.value ?? value ?? "",
    source: candidate?.source || source,
    support: candidate?.support || (candidate ? "explicit" : "unresolved"),
...
/**
 * Preserve the boundary between source-backed facts, reviewer/business
 * decisions, and customer-facing wording after all field winners are locked.
 */
export function lockFactsAndCustomerWording(alphaJson = {}, reviewerDecisionLog = [], { trustedResolution = false } = {}) {
  const normalization = alphaJson.normalization || {};
  const decisions = normalization.decisions || {};
  const evidence = normalization.field_evidence || {};
  const priceCandidates = decisions.prices?.resolution?.status === "selected"
    ? (decisions.prices?.candidates || [])
      .filter((candidate) => candidate.status === "selected")
      .map((candidate) => lockedFact(candidate.value, { envelope: { candidates: [candidate], resolution: { selectedCandidateId: candidate.id } } }))
    : [];
  const resolutionOverrides = Object.fromEntries(
    Object.entries(decisions)
      .filter(([, envelope]) => envelope?.resolution?.resolvedBy === "reviewer")
      .map(([field, envelope]) => [field, {
        selected_candidate_id: envelope.resolution.selectedCandidateId || "",
        reason_code: envelope.resolution.reasonCode || "reviewer_override",
      }]),
  );

  normalization.verified_facts = {
    version: "verified-facts-v1",
    customer: {
      name: lockedFact(evidence.customer_name || alphaJson.customer?.name || ""),
      phone: lockedFact("", { envelope: decisions.phone, evidence: evidence.phone ? [{ quote: evidence.phone }] : [] }),
      email: lockedFact("", { envelope: decisions.email, evidence: evidence.email ? [{ quote: evidence.email }] : [] }),
    },
    job: {
      service_address: lockedFact("", { envelope: decisions.service_address, evidence: evidence.service_address ? [{ quote: evidence.service_address }] : [] }),
      tree_count: lockedFact("", { envelope: decisions.tree_count, evidence: evidence.tree_count ? [{ quote: evidence.tree_count }] : [] }),
      tree_type: lockedFact(alphaJson.job?.tree_details?.tree_type || "", {
        evidence: evidence.tree_type ? [{ quote: evidence.tree_type }] : [],
      }),
      source_work_scope: lockedFact(evidence.work_scope || "", {
        evidence: evidence.work_scope ? [{ quote: evidence.work_scope }] : [],
      }),
    },
    service_options: priceCandidates.map((price, index) => ({
      label: alphaJson.service_options?.items?.[index]?.label || `Option ${String.fromCharCode(65 + index)}`,
      price,
    })),
  };
  normalization.business_decisions = {
    version: "business-decisions-v1",
    reviewer_decisions: Array.isArray(alphaJson.reviewer_decisions) ? alphaJson.reviewer_decisions : [],
    reviewer_decision_log: Array.isArray(reviewerDecisionLog) ? reviewerDecisionLog : [],
    review_overrides: alphaJson.review?.overrides || {},
    resolution_overrides: resolutionOverrides,
  };
  normalization.customer_wording = {
    version: "customer-wording-v1",
    locked_after_facts: true,
    job_description: asString(alphaJson.job?.description),
    option_descriptions: (alphaJson.service_options?.items || []).map((option) => ({
      label: option.label || "",
      title: option.title || "",
      description: option.description || "",
    })),
    display_notes: asString(alphaJson.notes?.display_notes),
  };
  if (trustedResolution) {
    normalization.fact_resolution_locked = true;
  }
  alphaJson.normalization = normalization;
  return alphaJson;
}
...
function genericCityStateAfterStreet(text) {
  const match = asString(text).match(new RegExp(`^\\s*(?:(?:,|[-–—]+)\\s*|in\\s+)?${GENERIC_CITY_STATE_PATTERN}\\b`, "i"));
  if (!match) return null;
...
function extractExplicitCityStateAddress(rawInput) {
  ...
  const pattern = new RegExp(
    `\\b(\\d+\\s+(?:[A-Za-z0-9.]+\\s+){0,5}${ADDRESS_SUFFIX}\\b(?:\\s+\\d+(?:\\s*[NSEW]\\b)?)?)(?:\\s*(?:,|[-–—]+)\\s*|\\s+in\\s+|\\s+)${GENERIC_CITY_STATE_PATTERN}\\b`,
    "gi",
  );
...
function extractTreeCountFromRaw(rawInput) {
  ...
  const singleSpeciesWork = text.match(
    new RegExp(`\\b(?:${TREE_SPECIES_MATCH_PATTERN})s?\\s+(?:removed|removal|taken?\\s+down|cut\\s+down|dropped|trimmed|cut)\\b`, "i"),
  );
  if (singleSpeciesWork) return "1 tree";
...
function extractTreeTypeFromRaw(rawInput) {
  const text = treeTextBeforeOptions(rawInput);
  const fullText = stripStreetNameSpeciesEvidence(treeTextForCount(rawInput));
  ...
  for (const species of TREE_SPECIES) {
    ...
    const speciesBeforeWorkInPreamble = new RegExp(
      `\\b${escaped}s?\\b.{0,180}\\b(?:remove|removed|removal|take\\s+down|cut\\s+down|drop|trim|cut|tree\\s+out|tree\\s+down|wants?\\s+(?:it\\s+)?(?:gone|out))\\b`,
      "i",
    );
    if (
      speciesNearTree.test(text) ||
      speciesAfterWork.test(text) ||
      speciesBeforeWork.test(text) ||
      speciesBeforeCount.test(text) ||
      speciesBeforeWorkInPreamble.test(fullText)
    ) return species;
  }
  return "";
}
...
function rawTextWithoutLeadContact(rawInput) {
  return stripEmails(stripStructuredContactLines(rawInput))
    ...
    .replace(/(?:phone\s*)?(?:\+?1[-./\s]?)?\(?\d{3}\)?[-./\s]?\d{3}[-./\s]?\d{4}/gi, "")
    .trim();
}
...
function cleanOptionPhrase(value) {
  let text = stripInternalOperationalNotes(asString(value))
    ...
    .replace(
      /\b(?:service\s+address|service\s+addr|svc\s+addr|service\s+at|job\s+at|address|addr)\b[\s\S]*?(?=\b(?:remove|removal|take\s+down|cut|drop|trim|haul|cleanup|clean|stump|grind|leave|stack)\b)/i,
      " ",
    )
    .replace(
      /\b(?:tree\s+dude\s+looked\s+already(?:\s+maybe)?|tree\s+dude\s+already\s+looked(?:\s+maybe)?)\b[\s\S]*?(?=\b(?:remove|removal|take\s+down|cut|drop|trim|haul|cleanup|clean|stump|grind|leave|stack|option|opt)\b|$)/i,
      " ",
    )
...
function textForOptionExtraction(rawInput) {
  ...
  return stripAddressLikePhrasesForOptionText(text)
    .replace(
      new RegExp(
        `\\b(?:service\\s+address|address|addr)\\b\\s*(?:is|at|:)?\\s*.*?\\b${LOCAL_CITY_PATTERN}\\b\\s*,?\\s*(?:Indiana|IN)\\b`,
        "gi",
      ),
      " ",
    )
    .replace(/\b(?:service\s+address|address|addr)\b\s*(?=(?:[-–—]+|;|\.|$))/gi, " ")
...
function extractThreePriceStumpOptions(text) {
  const priceMatches = Array.from(text.matchAll(/\$*\s*([0-9][0-9,]{2,})/g))
    .filter((match) => isLikelyPriceMatch(text, match));
  if (priceMatches.length !== 3) return [];

  const [firstPrice, secondPrice, stumpPrice] = priceMatches;
  const firstSegment = text
    .slice(Math.max(0, firstPrice.index - 120), firstPrice.index)
    .split(/[.;\n]/)
    .at(-1) || "";
  const secondSegment = text.slice(firstPrice.index + firstPrice[0].length, secondPrice.index);
  const stumpSegment = text.slice(secondPrice.index + secondPrice[0].length, stumpPrice.index);
  const descriptions = [firstSegment, secondSegment, stumpSegment].map((segment) => cleanOptionPhrase(segment));
  if (!descriptions.every((description) => phraseHasWork(description)) || !/\bstump\b/i.test(stumpSegment)) return [];

  return [
    { description: descriptions[0], price: amountFromMatch(firstPrice[1]), preserve_order: true },
    { description: descriptions[1], price: amountFromMatch(secondPrice[1]), preserve_order: true },
    { description: "stump work", price: amountFromMatch(stumpPrice[1]), preserve_order: true },
  ];
}

function phraseHasWork(value) {
  return TREE_SERVICE_PATTERNS.workScope.test(value) ||
    /\b(?:remove|removal|take(?:\s+down)?|cut(?:\s+(?:down|only|tree|trees))?|drop(?:\s+(?:only|tree|trees))?|trim|prune|haul(?:\s+(?:away|off|debris|brush))?|cleanup|clean\s+up|leave\s+(?:wood|logs?|brush|debris)|stack\s+(?:wood|logs?)|stump(?:\s+(?:grind|grinding))?|grind(?:ing)?|chip(?:ping)?)\b/i.test(value);
}
...
function extractImplicitOptionsFromRaw(rawInput) {
  ...
  const threePriceStumpOptions = extractThreePriceStumpOptions(text);
  if (threePriceStumpOptions.length) return threePriceStumpOptions;
...
function extractSidecarOptionPairingsFromRaw(rawInput) {
  ...
  const abPairings = pairings.filter((pairing) => pairing.source === "option_price_sidecar_ab_pair" && pairing.raw_label_token && pairing.price_value);
  ...
  const highConfidencePairings = [...abPairings, ...segmentPairings, ...directPairings, ...implicitPairings]
    .filter((pairing) => (
      pairing.confidence === "high" &&
      pairing.pairing_confidence === "high" &&
      pairing.amount_confidence === "high" &&
  ...
  if (abPairings.length >= 2) {
    usablePairings = abPairings;
  } else if (segmentPairings.length >= 2) {
...
function normalizeOptions(rawJson, rawInput, normalization = {}) {
  const modelOptions = hasNormalizationUncertainty(normalization, ["options", "service_options"])
    ? []
    : collectModelOptions(rawJson);
  const sidecarOptions = extractSidecarOptionPairingsFromRaw(rawInput);
...
export function normalizeToAlphaJsonV14(rawJson = {}, rawInput = "", intakeFields = {}) {
  ...
  const uncertainName = hasNormalizationUncertainty(providedNormalization, ["customer_name", "customer.name", "name"]);
  const uncertainPhone = hasNormalizationUncertainty(providedNormalization, ["phone", "customer.phone"]);
  const uncertainEmail = hasNormalizationUncertainty(providedNormalization, ["email", "customer.email"]);
  const uncertainAddress = hasNormalizationUncertainty(providedNormalization, ["service_address", "job.service_address", "address"]);
  const uncertainTreeCount = hasNormalizationUncertainty(providedNormalization, ["tree_count", "job.tree_details.tree_count"]);
  const uncertainTreeType = hasNormalizationUncertainty(providedNormalization, ["tree_type", "job.tree_details.tree_type", "species"]);
  const uncertainTreeSize = hasNormalizationUncertainty(providedNormalization, ["tree_size", "job.tree_details.tree_size", "size"]);
  const uncertainLocation = hasNormalizationUncertainty(providedNormalization, ["location_on_property", "job.condition_details", "work_location"]);
  const uncertainWorkAction = hasNormalizationUncertainty(providedNormalization, ["work_action", "job.work_action"]);
  const uncertainWorkScope = hasNormalizationUncertainty(providedNormalization, ["work_scope", "job.description", "scope"]);

  const modelProvidedName = uncertainName
    ? ""
    : firstString(client.name, customer.name, modelJson.name, base.customer.name);
  const name = uncertainName
    ? firstCleanCustomerName(intake.name)
    : firstCleanCustomerName(intake.name, modelProvidedName, extractNameFromRaw(sourceRawInput));
  const phone = normalizePhone(firstString(
    intake.phone,
    uncertainPhone ? "" : client.phone,
    uncertainPhone ? "" : customer.phone,
    uncertainPhone ? "" : customer.contact?.phone,
    uncertainPhone ? "" : modelJson.phone,
    uncertainPhone ? "" : modelJson.phone_number,
    uncertainPhone ? "" : base.customer.phone_display,
    uncertainPhone ? "" : extractPhoneFromRaw(sourceRawInput),
  ));
  const email = firstString(
    intake.email,
    uncertainEmail ? "" : client.email,
    uncertainEmail ? "" : customer.email,
    uncertainEmail ? "" : customer.contact?.email,
    uncertainEmail ? "" : modelJson.email,
    uncertainEmail ? "" : modelJson.email_address,
    uncertainEmail ? "" : extractEmailFromRaw(sourceRawInput),
  );
  const address = firstCleanAddress(
    intake.address,
    uncertainAddress ? "" : client.service_address,
    uncertainAddress ? "" : customer.service_address,
    uncertainAddress ? "" : composeAddress(customer.address),
    uncertainAddress ? "" : composeAddress(job.address),
    uncertainAddress ? "" : composeAddress(job.service_address),
    uncertainAddress ? "" : modelJson.service_address,
    uncertainAddress ? "" : modelJson.address,
    uncertainAddress ? "" : extractExplicitCityStateAddress(sourceRawInput),
    uncertainAddress ? "" : extractAddressFromRaw(sourceRawInput),
    uncertainAddress ? "" : base.job.service_address.display,
  );
  const treeCountOverride = intake.treeCountOverride;
  const rawTreeCountExtracted = extractTreeCountFromRaw(sourceRawInput);
  const treeCountModelCandidates = [
    { value: service.tree_count_scope, reasonCode: "service.tree_count_scope" },
    { value: firstService.tree_count, reasonCode: "service[0].tree_count" },
    { value: serviceTree.count, reasonCode: "service[0].tree.count" },
    { value: job.tree_count, reasonCode: "job.tree_count" },
    { value: job.tree_details?.tree_count, reasonCode: "job.tree_details.tree_count" },
    { value: modelJson.tree_count, reasonCode: "model.tree_count" },
    { value: base.job.tree_details.tree_count, reasonCode: "draft.tree_count" },
  ].filter((entry) => asString(entry.value));
  const treeCountCandidate = treeCountOverride === "Unknown" || isTreeCountUnclearOkOverride(treeCountOverride)
    ? ""
    : firstString(
        treeCountOverride,
        uncertainTreeCount ? "" : service.tree_count_scope,
        uncertainTreeCount ? "" : firstService.tree_count,
        uncertainTreeCount ? "" : serviceTree.count,
        uncertainTreeCount ? "" : job.tree_count,
        uncertainTreeCount ? "" : job.tree_details?.tree_count,
        uncertainTreeCount ? "" : modelJson.tree_count,
        uncertainTreeCount ? "" : base.job.tree_details.tree_count,
        rawTreeCountExtracted,
      );
  ...
  const safeExtractedTreeType =
    extractedTreeType && extractTreeTypeFromRaw(extractedTreeTypeEvidence) === extractedTreeType
      ...
      : "";
  const treeType = firstString(
    uncertainTreeType ? "" : serviceTree.type,
    uncertainTreeType ? "" : firstService.tree_type,
    uncertainTreeType ? "" : job.tree_details?.tree_type,
    uncertainTreeType ? "" : base.job.tree_details.tree_type,
    safeExtractedTreeType,
  );
  const treeSize = firstString(
    uncertainTreeSize ? "" : serviceTree.size,
    uncertainTreeSize ? "" : firstService.tree_size,
    uncertainTreeSize ? "" : job.tree_details?.tree_size,
    uncertainTreeSize ? "" : base.job.tree_details.tree_size,
  );
  const location = firstString(
    uncertainLocation ? "" : serviceTree.location,
    uncertainLocation ? "" : firstService.location,
    uncertainLocation ? "" : job.condition_details,
    uncertainLocation ? "" : base.job.condition_details,
  );
  const options = normalizeOptions(modelJson, sourceRawInput, providedNormalization);
  const optionDescriptions = options.map((option) => option.description).filter(Boolean).join("; ");
  const description = firstString(
    uncertainWorkScope ? "" : job.description,
    uncertainWorkScope ? "" : service.description,
    uncertainWorkScope ? "" : firstService.description,
    uncertainWorkScope ? "" : modelJson.scope,
    optionDescriptions,
  ...
  base.job = {
    ...
    work_action: uncertainWorkAction ? "" : job.work_action,
    condition_details: [
      location,
      uncertainLocation ? "" : job.condition_details,
    ].filter(Boolean).join(". "),
  ...
  const claimGraph = buildClaimGraphFromRaw(sourceRawInput);
  const treeCountDecision = buildTreeCountDecisionEnvelope({
    selectedValue: base.job.tree_details.tree_count || "",
    rawInput: sourceRawInput,
    treeCountOverride,
    modelCandidates: treeCountModelCandidates,
    rawExtracted: rawTreeCountExtracted,
    uncertainTreeCount,
    requiresReview: Boolean(treeCountReviewEvidence || treeCountRejectedEvidence),
    abstained: treeCountOverride === "Unknown" || isTreeCountUnclearOkOverride(treeCountOverride),
    claimGraph,
  });
  const selectedTreeCountCandidate = treeCountDecision.candidates.find(
    (candidate) => candidate.id === treeCountDecision.resolution.selectedCandidateId,
  );
  if (treeCountDecision.resolution.status === "selected" && selectedTreeCountCandidate?.value) {
    base.job.tree_details.tree_count = selectedTreeCountCandidate.value;
  }
  base.normalization.corrected_interpretation =
    cleanCorrectedInterpretation(base.normalization.corrected_interpretation, base) ||
    basicNormalization.corrected_interpretation;
  applyOptionScopeInference(base);
  // The initial pass establishes inferred-scope flags; this pass can then
  // recover complete labeled option wording from raw source evidence.
  recoverExplicitSourceOptionScopes(base);
  base.job.description = buildCustomerJobSummary(base);
  // Keep field_evidence.work_scope as source evidence only. Never copy the
  // generated customer-facing summary back into evidence.
  base.normalization.field_evidence = {
    ...base.normalization.field_evidence,
    tree_count: base.job.tree_details.tree_count || base.normalization.field_evidence.tree_count || "",
  };

  attachDecisionEnvelopes(base, { tree_count: treeCountDecision });
  attachClaimGraph(base, claimGraph);
  base.reviewer_decisions = Array.isArray(modelJson.reviewer_decisions)
    ? modelJson.reviewer_decisions
    : [];

  lockFactsAndCustomerWording(base);
