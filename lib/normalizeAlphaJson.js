...
const CUSTOMER_NAME_JOB_TEXT_PATTERN =
  /\b(?:oak|pine|maple|elm|ash|cedar|sycamore|hickory|locust|birch|spruce|walnut|cherry|tree|trees|limb|limbs|branch|branches|stump|brush|garage|garaje|yard|driveway|fence|remove|removal|take\s+down|take|cut|drop|trim|haul|cleanup|clean\s+up|grind|price|quote|option|opt)\b/i;
...
function sortAndRelabelOptions(options) {
  const ordered = options.some((option) => option.preserve_order)
    ? [...options]
...
function treeTextForCount(rawInput) {
  return normalizeTreeServiceText(stripPhoneLikeNumbersForTreeCounts(rawInput))
    .replace(/\s+/g, " ")
    .trim();
}
...
function extractBaseAndStumpPriceOptions(text) {
  ...
  if (!/\bstumps?\b|\bgrind(?:ing)?\b/i.test(betweenPrices)) return [];
  ...
  return [
    ...
    {
      description: /\bgrind|grinding\b/i.test(betweenPrices) ? "stump grinding" : "stump work",
...
function weakScopePriceOption(amount, description = "work scope unclear") {
  return {
    description,
    price: amount,
    preserve_order: true,
...
function parseOptionBody(body) {
  ...
  if (
    ...
    return weakScopePriceOption(amount, description);
  }
  if (
    amount &&
...
function extractLabeledOptionsFromText(text) {
  ...
  for (const regex of patterns) {
    ...
    while ((match = regex.exec(text))) {
      ...
      if (parsed && (phraseHasWork(parsed.description) || parsed.price)) options.push({ ...parsed, preserve_order: true });
    }
...
function extractNameFromRaw(rawInput) {
  ...
  const patterns = [
    { pattern: new RegExp(`\\b(?:note\\s+from|text\\s+from|email\\s+from|message\\s+from|msg\\s+from|voicemail\\s+from|vm\\s+from|call\\s+from|called\\s+by|send\\s+quote\\s+to|customer\\s+is|customer|client|homeowner|lady\\s+named|lady|guy|person)\\s+${namePattern}${stopPattern}`, "i"), source: text },
    ...
    { pattern: new RegExp(`(?:${PHONE_PATTERN.source}|[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,})\\s+(?:or\\s+)?${namePattern}${stopPattern}`, "i"), source: rawText },
  ];
...
export function normalizeToAlphaJsonV14(rawJson = {}, rawInput = "", intakeFields = {}) {
  ...
  const name = cleanCustomerName(firstString(intake.name, modelProvidedName, extractNameFromRaw(sourceRawInput)));
  ...
  const treeCountReviewEvidence = !treeCountOverride && (
    treeCountCandidateNeedsReview(treeCountCandidate)
      ? treeCountCandidate
