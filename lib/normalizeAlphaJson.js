...
function cleanCustomerName(value) {
  let text = stripPhones(stripEmails(value))
    .replace(/\b(?:customer\s+is|customer|client|homeowner|lady\s+named|lady|guy|person|office\s+said\s+call|call|texted|text\s+mess)\b[:\s-]*/gi, " ")
    .replace(/\b(?:maybe|no\s+phone\s+in\s+note|email\s+only|estimate\s+from\s+yesterday|from\s+yesterday)\b.*$/i, "")
    .replace(/\b(?:phone|service|address|job|lives?|wants?|needs?|says?|said|property|place|at|on)\b.*$/i, "")
    .replace(/[,:;.?\s-]+$/g, "")
    .replace(/\s+/g, " ")
    .trim();
...
function textForOptionExtraction(rawInput) {
  let text = rawTextWithoutLeadContact(rawInput).replace(/\s+/g, " ");
...
function extractSlashPriceOptions(text) {
  const match = text.match(/\b([0-9][0-9,]{2,})\s*\/\s*([0-9][0-9,]{2,})\s+(?:with\s+)?([A-Za-z\s]+?)(?:[.;]|$)/i);
  if (!match) return [];
  const addOn = cleanOptionPhrase(match[3]);
  const baseDescription = cleanOptionPhrase(text.slice(0, match.index));
  const base = /\b(tree|oak|pine|maple|limb|brush|stump|cleanup|work)\b/i.test(baseDescription)
    ? baseDescription
    : "basic tree work";
  return [
    { description: base, price: amountFromMatch(match[1]) },
    { description: `${base} and ${addOn}`, price: amountFromMatch(match[2]) },
  ];
}
...
function extractImplicitOptionsFromRaw(rawInput) {
  ...
  prices.forEach((priceMatch, index) => {
    ...
    if (!phrase || !/remove|removal|haul|cut|trim|cleanup|clean|leave|debris|stump|grind|sweep|stack|wood|limb|brush|work|package|cheap|basic|normal|fancy/i.test(phrase)) return;
...
function extractOptionsFromRaw(rawInput) {
  ...
  while ((match = regex.exec(text))) {
    ...
    const description = cleanOptionPhrase(priceMatch ? body.slice(0, priceMatch.index) : body);
    if (/remove|removal|haul|cut|trim|cleanup|leave|debris|stump|grind|grinding|sweep|stack|wood|limb|work|emergency|package|cheap|basic|normal|fancy/i.test(description)) {
      options.push({ description, price: amount });
    }
  }
  return options.length ? options : extractImplicitOptionsFromRaw(rawInput);
}
...
function extractNameFromRaw(rawInput) {
  const explicitCustomerMatches = Array.from(
    asString(rawInput).matchAll(/\bCustomer\s+(?:is\s+)?([A-Za-z\u00c0-\u024f][A-Za-z\u00c0-\u024f\s.'-]{1,50}?)(?=\.|,|\s+Phone\b|\s+Service\b|\s+Address\b)/gi),
  );
  const explicitCustomer = explicitCustomerMatches.at(-1)?.[1];
  if (explicitCustomer) return cleanCustomerName(explicitCustomer);

  const namedMatch = asString(rawInput).match(/\b(?:lady\s+named|guy|person|client)\s+([A-Za-z\u00c0-\u024f][A-Za-z\u00c0-\u024f\s.'-]{1,40}?)(?=\s+maybe\b|\s+\d|\s+says?\b|,|\.|$)/i);
  if (namedMatch) return cleanCustomerName(namedMatch[1]);
