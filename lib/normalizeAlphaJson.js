...
function normalizeLocationForSummary(value) {
  ...
  const match = text.match(/^(near|by|beside|behind|over|along|close to|in)\s+(?:the\s+)?(.+)$/i);
  if (!match) return text;

  const preposition = match[1].toLowerCase();
  const target = match[2].replace(/\b(?:tree|trees)\b.*$/i, "").trim();
  const cleanedTarget = target.replace(/\b(?:remove|removed|removal|trim|trimming|cut|drop|take\s+down)\b.*$/i, "").trim();
...
function locationForSummary(alphaJson) {
  ...
  const match = text.match(/\b(?:near|by|beside|behind|over|along|close to)\s+(?:the\s+)?[^.!?;,$]{2,48}/i);
  if (!match) return "";
