...
function cleanCustomerName(value) {
  const rawCandidate = asString(value);
...
function treeTextBeforeOptions(rawInput) {
  return stripStreetNameSpeciesEvidence(stripTreeCountNonJobNumbers(treeTextForCount(rawInput))).split(/\b(?:Option|Opt)\b/i)[0];
}
...
function normalizeOption(option, index) {
  ...
  const priceIsUnclear = Boolean(option.price_unclear || option.price_is_unclear || option.is_unclear || option.price?.is_unclear);
...
function amountHasNonFirmContext(text, amount) {
  ...
  return Boolean(matches.length) && matches.every((match) => hasNonFirmPriceContext(priceContextText(text, match)));
}
