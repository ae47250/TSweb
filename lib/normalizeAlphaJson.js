...
function cleanCorrectedInterpretation(value) {
  return normalizeTreeServiceText(stripStructuredContactLines(value)
    .replace(/\b(?:Customer\s+name|Customer\s+phone|Customer\s+email|Service\s+address)\s*:\s*/gi, " "));
}
...
function buildBasicNormalizationFromRawInput(rawInput, alphaJson) {
  const normalized = emptyNormalization();
  const correctedText = cleanCorrectedInterpretation(rawTextWithoutLeadContact(rawInput));
...
function rawTextWithoutLeadContact(rawInput) {
  return stripEmails(stripStructuredContactLines(rawInput))
    .replace(/^(?!(?:remove|removal|trim|cut|drop|take|haul|cleanup|clean|grind)\b)[A-Za-z\u00c0-\u024f\s.'-]{2,40}\s+/i, "")
