...
function stripInternalOperationalNotes(value) {
  return asString(value)
    .replace(/\b(?:heads?\s+up[:\s-]*)?(?:aggressive\s+dog|dog|blocked\s+access|no\s+access)[^.!?;]*(?:[.!?;]|$)/gi, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function stripAddressEvidence(value) {
  const addressPattern = new RegExp(
    `\\b\\d+\\s+(?:[A-Za-z0-9.]+\\s+){0,5}${ADDRESS_SUFFIX}\\b(?:\\s*(?:,|-|in)?\\s*(?:Madison|Hanover)\\b(?:,?\\s*(?:Indiana|IN))?)?`,
    "gi",
  );
  return asString(value)
...
function extractTreeCountFromRaw(rawInput) {
  const text = normalizeTreeServiceText(stripPhones(rawInput))
  ...
  const singleTreeWork = text.match(
    /\b(?:remove|removal\s+of|take\s+down|cut\s+down|trim|drop|cut)\s+(?:one|a|an)?\s*(?:dead\s+|large\s+|small\s+|leaning\s+|fallen\s+)?(?:pine|oak|maple|elm|ash|cedar|sycamore|hickory|locust|birch|spruce|walnut|cherry|tree)\b/i,
  );
