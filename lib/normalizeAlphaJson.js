...
function optionAddOns(option) {
  ...
  if (/\b(?:leave\s+wood\s+on\s+site|stack\s+firewood|stack\s+wood)\b/i.test(text)) {
    addOns.push("leave wood on site");
  }
  if (/\b(?:cleanup|clean\s+up|site\s+cleanup|final\s+cleanup|yard\s+protection\s+cleanup|access\s+cleanup|sweep|rake(?:[-\s]+up)?|yard\s+cleanup)\b/i.test(text)) addOns.push("cleanup");
...
export function normalizeToAlphaJsonV14(rawJson = {}, rawInput = "", intakeFields = {}) {
  ...
  attachClaimGraph(base, buildClaimGraphFromRaw(sourceRawInput));
