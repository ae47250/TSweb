...
import { TREE_SERVICE_PATTERNS, TREE_SERVICE_PATTERN_SOURCES } from "./treeServiceLexicon.js";
...
function rawOptionLabelToken(option) {
  const text = firstString(option?.raw_label, option?.label);
  const match = text.match(/^(?:Option\s*)?([A-E])\b/i);
...
function extractSidecarOptionPairingsFromRaw(rawInput) {
  ...
  return pairings
    .filter((pairing) => pairing.source === "option_price_sidecar_segment" && pairing.raw_label_token && pairing.price_value)
    .map((pairing) => ({
