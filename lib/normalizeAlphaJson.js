...
function optionAddOns(option) {
  ...
  const addOns = [];
  if (/\bhaul(?:\s+away|\s+off)?|haul\s+debris|remove\s+(?:all\s+)?debris|haul\s+everything\b/i.test(text)) {
    addOns.push("haul away");
  }
  if (/\bcleanup|clean\s+up|sweep\b/i.test(text)) addOns.push("cleanup");
...
function extractSidecarOptionPairingsFromRaw(rawInput) {
  ...
  const safeDirectPairings = directPairings.length === 2 && directPairings.every((pairing) => (
    ...
    ? directPairings
    : [];
  const usablePairings = segmentPairings.length >= 2 ? segmentPairings : safeDirectPairings;
...
export function normalizeToAlphaJsonV14(rawJson = {}, rawInput = "", intakeFields = {}) {
  ...
  const phone = normalizePhone(firstString(
    ...
    modelJson.phone,
    modelJson.phone_number,
    base.customer.phone_display,
    extractPhoneFromRaw(sourceRawInput),
  ));
