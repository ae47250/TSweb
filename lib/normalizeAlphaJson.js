...
function sanitizeNormalization(value) {
  ...
  normalized.corrected_interpretation = asString(value.corrected_interpretation);
...
function buildBasicNormalizationFromRawInput(rawInput, alphaJson) {
  const normalized = emptyNormalization();
  const correctedText = normalizeTreeServiceText(rawTextWithoutLeadContact(rawInput));
