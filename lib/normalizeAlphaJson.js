...
const LOCAL_CITY_PATTERN =
  "(?:Madison|Hanover|Salem|Corydon|New\\s+Albany|Jeffersonville|Bedford|Scottsburg|Paoli|Seymour)";
...
function locationForSummary(alphaJson) {
  const text = cleanCorrectedInterpretation(alphaJson.job?.description || "", alphaJson);
  const match = text.match(/\b(?:near|by|beside|behind|over|along|close to|in)\s+(?:the\s+)?[^.!?;,$]{2,48}/i);
  if (!match) return "";
  const location = match[0]
    .replace(/\b(?:for|price|quoted|option)\b.*$/i, "")
    .replace(/\s+/g, " ")
    .trim();
  if (!location || /\b(customer|phone|email|address|note lists|tree needing|service option)\b/i.test(location)) return "";
  return location;
}
...
function summaryHasDisplayRisk(value, alphaJson = {}) {
  ...
  if (/;/.test(normalized)) return true;
  if (/\b(?:at|by|in|for|with|and|but|as|is)\s*[,.!?]?\s*$/i.test(normalized.replace(/[.!?]$/, ""))) return true;
  ...
  if (/\b(?:Tree Dude|raw notes?|customer name|customer phone|customer email|parser|evidence|follow-up|internal|note lists|tree needing removal is)\b/i.test(normalized)) return true;
  if (/\b(?:aggressive\s+dog|dog\s+in\s+yard|gate\s+blocked|blocked\s+by\s+trailer|trailer\s+blocking\s+access|power\s*line|service\s+drop|crew\s+caution|call\s+before\s+entering|do\s+not\s+(?:enter|go\s+in))\b/i.test(normalized)) return true;
...
function safeSummaryOrFallback(candidate, alphaJson) {
  return summaryHasDisplayRisk(candidate, alphaJson)
    ? "Tree service work as described in the selected option."
    : candidate;
}
...
export function buildCustomerJobSummary(alphaJson = {}) {
  ...
  if (treeCountWord || treeType) {
    ...
    const core = `${action} ${[quantity, `${type}${noun}`.trim()].filter(Boolean).join(" ")}`;
    const withLocation = ensureSentence(`${core}${location ? ` ${location}` : ""}`);
    if (!summaryHasDisplayRisk(withLocation, alphaJson)) return withLocation;
    return safeSummaryOrFallback(ensureSentence(core), alphaJson);
  }
  ...
  return "Tree service work as described in the selected option.";
}
...
function reconcileParsedFacts(alphaJson, normalization) {
  ...
  if (!alphaJson.job?.tree_details?.tree_type && !hasNormalizationUncertainty(normalization, ["tree_type", "tree type", "species"])) {
    const treeType = firstString(
      ...
      extractTreeTypeFromRaw(summaryEvidence),
    );
...
export function normalizeServiceAddress(value) {
  return cleanServiceAddressCandidate(value) || cleanTypedServiceAddress(value);
}
...
function extractAddressFromRaw(rawInput) {
  const text = normalizeTreeServiceText(stripEmails(stripPhones(rawInput)))
  ...
  if (localNoSuffixMatch && !/\b(tree|trees|limb|limbs|branch|branches|brush|stump|haul|cleanup|clean|remove|removal|trim|cut|drop|option|opt)\b/i.test(localNoSuffixMatch[0])) {
    return localNoSuffixMatch[0].trim().replace(/\s+,/g, ",");
  }
...
function extractTreeCountFromRaw(rawInput) {
  const normalizedRaw = normalizeTreeServiceText(stripPhones(rawInput))
  ...
  const singularLocatedTree = normalizedRaw.match(/\b(?:dead\s+|large\s+|big\s+|small\s+|leaning\s+|fallen\s+)?tree\s+(?:by|near|beside|behind|over|at)\b/i);
  ...
  if (singularLocatedTree && !hasVagueCount && /\b(?:remove|removed|removal|take\s+down|cut\s+down|trim|drop|cut)\b/i.test(normalizedRaw)) return "1 tree";
  return "";
}
