...
import { buildFieldClarificationWarnings, buildStructuredFollowUps } from "./followUpBuilder.js";
...
import { applyCanonicalServiceAssembler } from "./canonicalServiceAssembler.js";
import { validateFinalOptionStructure } from "./finalOptionStructureValidator.js";
import { acceptedIncrementalBundleRelationship, buildSourceFinalFactCoverage } from "./sourceFinalFactCoverage.js";
import { evaluateReadinessSafety } from "./readinessSafety.js";
import { FIELD_RESOLUTION_GATE_VERSION, fieldResolutionBlockingErrors } from "./unifiedFieldResolution.js";
import { readinessOverrideStatus, readinessSafetyBlockingEnabled } from "./readinessReview.js";
...
function followUpSegments(text) {
  const raw = String(text || "");
  const matches = Array.from(raw.matchAll(/\bFollow-up\s+\d+\s*:/gi));
...
function segmentHasExactServiceAddress(text) {
  const segment = String(text || "");
  if (!segment) return false;
  const hasAddressCue =
    /\b(?:service\s+address|exact\s+(?:service\s+)?address|exact\s+location|job\s+(?:address|location)|location\s+(?:is|at)|property\s+(?:is\s+)?at|work\s+(?:is\s+)?at)\b/i.test(segment);
  return Boolean(
    hasAddressCue &&
    /\b\d{1,5}\b/.test(segment) &&
    (hasAddressTownOrCity(segment) || hasLocalIndianaTown(segment) || new RegExp(`\\b${US_STATE_PATTERN}\\b`, "i").test(segment)),
  );
}

function segmentHasGenuineMissingCity(text) {
  const segment = String(text || "");
  if (!/\bno\s+city\b/i.test(segment)) return false;
  if (/\b(?:outside\s+city\s+limits|no\s+city\s+(?:sewer|water|permit|permits|tax|trash|services?))\b/i.test(segment)) return false;
  return (
    new RegExp(`\\b\\d{1,5}\\b.{0,80}\\b${ADDRESS_SUFFIX}\\b.{0,60}\\bno\\s+city\\b`, "i").test(segment) ||
    /\b(?:service\s+address|address)\b.{0,80}\bno\s+city\b/i.test(segment)
  );
}

function segmentHasExplicitAddressGap(text) {
  const segment = String(text || "");
  return /\blot\s+(?:beside|next\s+to|adjacent\s+to|by|near)\b/i.test(segment) || segmentHasGenuineMissingCity(segment);
}

function segmentReopensAddressResolution(text) {
  const segment = String(text || "");
  return (
    /\b(?:city|town|service\s+address|exact\s+address|exact\s+location|location)\b.{0,80}\b(?:still\s+)?(?:unknown|unclear|not\s+known|not\s+confirmed|missing)\b/i.test(segment) ||
    /\b(?:do\s+not|don't)\s+use\b.{0,80}\b(?:city|town|address|location|Madison|Hanover|Salem|Seymour|North\s+Vernon)\b/i.test(segment) ||
    /\b(?:city|town|address|location|Madison|Hanover|Salem|Seymour|North\s+Vernon)\b.{0,80}\b(?:do\s+not|don't)\s+use\b/i.test(segment)
  );
}

function structuredAddressEditResolvesGap(json, address) {
  const flags = json?.job?.service_address?.review_flags || {};
  const currentAddress = String(address || json?.job?.service_address?.display || "").trim();
  const editedValue = flags.service_address_server_verified_td_edit_value || "";
  return Boolean(
    flags.service_address_server_verified_td_edit &&
    currentAddress &&
    sameNormalizedText(editedValue, currentAddress) &&
    segmentHasExactServiceAddress(`service address ${currentAddress}`),
  );
}

function hasUnresolvedExplicitAddressGap(text, json, address) {
  const raw = String(text || "");
  if (!raw) return false;

  let unresolved = segmentHasExplicitAddressGap(originalTextBeforeFollowUps(raw));
  if (structuredAddressEditResolvesGap(json, address)) unresolved = false;
  for (const followUp of followUpSegments(raw)) {
    if (segmentReopensAddressResolution(followUp) || segmentHasExplicitAddressGap(followUp)) {
      unresolved = true;
    } else if (segmentHasExactServiceAddress(followUp)) {
      unresolved = false;
    }
  }
  return unresolved;
...
function optionHasServerVerifiedDescriptionEdit(option) {
  const flags = option?.review_flags || {};
  const verifiedValue = String(flags.description_server_verified_td_edit_value || "").trim();
  return Boolean(
    flags.description_server_verified_td_edit &&
    verifiedValue &&
    sameNormalizedText(verifiedValue, option?.description || ""),
  );
}

function optionScopeReviewText(option) {
  if (optionHasServerVerifiedDescriptionEdit(option)) return String(option?.description || "");
...
function pricedOptionsHaveUnclearScope(options) {
  return options.some((option) => {
    ...
    const text = optionScopeReviewText(option);
...
function blockingSafetyScopeReviewReason(text) {
  ...
  const structureOrUtilityContact =
    /\b(?:tree|limb|limbs|branch|branches)\b.{0,40}\b(?:on|onto|against|touching)\b.{0,24}\b(?:house|garage|service\s+drop|power\s*lines?|utility\s+lines?|wire|wires)\b/i;
  const fenceContact =
    /\b(?:tree|limb|limbs|branch|branches)\b.{0,40}\b(?:on|onto|against)\b.{0,24}\bfence\b/i;
  const treeLeaningOnStructure =
    /\btree\b.{0,30}\bleaning\s+(?:on|against|toward|touching)\b.{0,24}\b(?:structure|house|garage|fence|service\s+drop|power\s*line|wire)\b/i;
  ...
  if (
    ...
    return "Safety, access, or damage detail needs Tree Dude review.";
  }
  if (fenceNeedsReview) {
    return "Safety, access, or damage detail needs Tree Dude review.";
  }
  if (unclearScope.test(normalized) && !confirmedPropertyScope) {
    return "Work scope needs Tree Dude review.";
  }
  return "";
}
...
function extractSafetyOrAccessNote(text) {
  const normalized = String(text || "")
    ...
    .trim();
  if (!normalized) return "";
  const pattern = new RegExp(`[^.!?;]*\\b(?:${SAFETY_ACCESS_TERMS})\\b[^.!?;]*(?:[.!?;]|$)`, "gi");
  const cleanedNote = [...normalized.matchAll(pattern)]
    .map((match) => match[0]?.trim().replace(/^[-,:\s]+/g, "").replace(/\s+/g, " "))
    .filter(Boolean)
    .map(normalizeTreeServiceText)
    .filter(Boolean)
    .slice(0, 3)
    .join(" ");
...
function latestFollowUpText(text) {
  return followUpSegments(text).at(-1) || "";
}

function segmentHasClearWorkScope(text) {
  return /\b(remove|removal|trim|trimming|cut|drop|haul|cleanup|stump|grind|tree|trees|limb|brush)\b/i.test(text || "");
}

function followUpHasClearWorkScope(text) {
  return segmentHasClearWorkScope(latestFollowUpText(text));
}

function segmentHasFirmPrice(text) {
  return /\$?\s*[0-9][0-9,]{2,}\b/i.test(textWithoutPhones(text));
}

function followUpHasFirmPrice(text) {
  return segmentHasFirmPrice(latestFollowUpText(text));
}
...
function explicitSourceOptions(rawText = "") {
  const text = originalTextBeforeFollowUps(String(rawText || ""));
  const labels = [...text.matchAll(/\b(?:Option|Opt)\s*([A-E]|[1-5])\b\s*[:.)-]?/gi)]
...
function explicitTreeCountInSourceOption(segment = "") {
  const match = String(segment || "").match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(?:[a-z]+\s+){0,4}trees?\b/i);
  return match ? countTokenToNumber(match[1]) : null;
}

function explicitStumpCountInSourceOption(segment = "") {
  const match = String(segment || "").match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+stumps?\b/i) ||
    String(segment || "").match(/\bstumps?\s+(?:x\s*)?(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\b/i);
  return match ? countTokenToNumber(match[1]) : null;
}

function explicitSourceOptionQuantityConflictErrors(rawText = "") {
  const sourceOptions = explicitSourceOptions(rawText).filter((option) => option.amount).slice(0, 4);
  if (sourceOptions.length < 2) return [];
  const baseTreeCount = explicitTreeCountInSourceOption(sourceOptions[0].segment);
  if (!baseTreeCount) return [];
  const conflictingOption = sourceOptions.slice(1).find((option) => {
    const stumpCount = explicitStumpCountInSourceOption(option.segment);
    return stumpCount && stumpCount !== baseTreeCount;
  });
  if (!conflictingOption) return [];
  return [
    `Explicit source option quantities conflict; ${sourceOptions[0].label} references ${baseTreeCount} tree${baseTreeCount === 1 ? "" : "s"}, but ${conflictingOption.label} references a different stump count. Confirm the intended scope before PDF.`,
  ];
}

function explicitSourceOptionCompletenessErrors(rawText, options = [], priceReconciliation = {}) {
  const sourceOptions = explicitSourceOptions(rawText).filter((option) => option.amount).slice(0, 4);
  if (sourceOptions.length < 2) return [];

  const finalLabelCounts = new Map();
  const finalAmountsByToken = new Map();
  for (const option of options) {
    const token = rawOptionLabelToken(option);
    if (token) finalLabelCounts.set(token, (finalLabelCounts.get(token) || 0) + 1);
    const amount = optionAmount(option);
    if (token && amount) finalAmountsByToken.set(token, Number(amount));
  }

  const missing = sourceOptions.filter((sourceOption) => {
    if (finalLabelCounts.get(sourceOption.token) !== 1) return true;
    const finalAmount = finalAmountsByToken.get(sourceOption.token);
    if (finalAmount === Number(sourceOption.amount)) return false;
    return !acceptedIncrementalBundleRelationship({
      sourceAmount: sourceOption.amount,
      finalAmount,
      priceReconciliation,
    });
  });
  ...
  const details = missing
    .map((sourceOption) => `${sourceOption.label}${sourceOption.amount ? ` $${sourceOption.amount.toLocaleString("en-US")}` : ""}`)
    .join(", ");
  return [`Explicit source options are incomplete in TD2; missing or mismatched ${details}.`];
}
...
function hasTreeCountConfirmationLanguage(text) {
  const cleaned = stripRouteNumbersAboveTreeAutoMax(textWithoutAddressLikeNumbers(text || ""))
...
function segmentHasSlashPriceScopeAmbiguity(text) {
  const segment = String(text || "");
  return (
    /\bslash\s+price\b.{0,120}\bscope\s+unclear\b/i.test(segment) ||
    /\b\d[\d,]*\s*\/\s*\$?\d[\d,]*\b.{0,120}\bfor\s+tree\?\s*no\s+note\s+on\s+haul\s+or\s+stump\b/i.test(segment) ||
    /\bfor\s+tree\?\s*no\s+note\s+on\s+haul\s+or\s+stump\b.{0,120}\b\d[\d,]*\s*\/\s*\$?\d[\d,]*\b/i.test(segment)
  );
}

function segmentReopensSlashPriceScope(text) {
  const segment = String(text || "");
  return (
    /\b(?:slash\s+prices?|scopes?|option\s+[A-E]|prices?)\b.{0,100}\b(?:still\s+)?(?:unclear|unknown|not\s+confirmed|unconfirmed|wrong|incorrect|may\s+be\s+wrong|do\s+not\s+use|don't\s+use|reopen)\b/i.test(segment) ||
    /\b(?:do\s+not|don't)\s+use\b.{0,100}\b(?:slash\s+prices?|scope|option\s+[A-E]|prices?)\b/i.test(segment)
  );
}

function pricedFirmOptions(options = []) {
  return options.map((option) => ({ option, price: firmOptionPrice(option) })).filter((entry) => entry.price);
}

function segmentContainsAmount(text, amount) {
  const rounded = Math.round(Number(amount || 0));
  if (!rounded) return false;
  const compact = String(rounded);
  const comma = rounded.toLocaleString("en-US");
  return new RegExp(`(?:\\$\\s*)?(?:${escapeRegExp(compact)}|${escapeRegExp(comma)})\\b`, "i").test(text || "");
}

function cleanScopeTextForComparison(text) {
  return normalizedComparableText(
    String(text || "")
      .replace(/\bOption\s+[A-E]\b/gi, " ")
      .replace(/\bFollow-up\s+details\b/gi, " ")
      .replace(/\bscope\s+confirmed\b/gi, " ")
      .replace(/\$?\s*[0-9][0-9,]*\b/g, " "),
  );
}

function descriptionHasResolvedScope(text) {
  const value = String(text || "");
  const normalized = cleanScopeTextForComparison(value);
  return Boolean(
    normalized &&
    !/\b(?:work\s+scope\s+unclear|scope\s+unclear|unclear|unknown)\b/i.test(value) &&
    segmentHasClearWorkScope(value),
  );
}

function tdEditedDescriptionMatchesCurrent(option) {
  const flags = option?.review_flags || {};
  const currentDescription = String(option?.description || "").trim();
  const editedDescription = flags.description_server_verified_td_edit_value || "";
  return Boolean(flags.description_server_verified_td_edit && currentDescription && sameNormalizedText(editedDescription, currentDescription));
}

function structuredSlashPriceScopeResolved(options = []) {
  const pricedOptions = pricedFirmOptions(options);
  if (pricedOptions.length < 2) return false;
  const descriptions = [];
  for (const { option } of pricedOptions) {
    if (option?.scope_unclear || option?.review_flags?.scope_unclear) return false;
    if (!tdEditedDescriptionMatchesCurrent(option)) return false;
    const description = option?.description || option?.title || "";
    if (!descriptionHasResolvedScope(description)) return false;
    descriptions.push(cleanScopeTextForComparison(description));
  }
  return new Set(descriptions).size === pricedOptions.length;
}

function slashResolutionOptionBlocks(text) {
  return Array.from(String(text || "").matchAll(/\bOption\s+[A-E]\b[\s\S]*?(?=\bOption\s+[A-E]\b|$)/gi))
    .map((match) => match[0].trim())
    .filter(Boolean);
}

function segmentResolvesSlashPriceScope(text, options = []) {
  const segment = String(text || "");
  if (!segmentHasFirmPrice(segment) || !segmentHasClearWorkScope(segment)) return false;
  const pricedOptions = pricedFirmOptions(options);
  if (pricedOptions.length < 2) return false;
  const optionBlocks = slashResolutionOptionBlocks(segment);
  if (optionBlocks.length < pricedOptions.length) return false;

  const resolvedScopes = [];
  for (const { price } of pricedOptions) {
    const block = optionBlocks.find((candidate) => segmentContainsAmount(candidate, price.amount));
    if (!block || !descriptionHasResolvedScope(block)) return false;
    resolvedScopes.push(cleanScopeTextForComparison(block));
  }
  return new Set(resolvedScopes).size === pricedOptions.length;
}

function hasUnresolvedSlashPriceScopeAmbiguity(text, options = []) {
  const raw = String(text || "");
  if (!raw) return false;

  let unresolved = segmentHasSlashPriceScopeAmbiguity(originalTextBeforeFollowUps(raw));
  if (structuredSlashPriceScopeResolved(options)) unresolved = false;
  for (const followUp of followUpSegments(raw)) {
    if (segmentReopensSlashPriceScope(followUp) || segmentHasSlashPriceScopeAmbiguity(followUp)) {
      unresolved = true;
    } else if (segmentResolvesSlashPriceScope(followUp, options)) {
      unresolved = false;
    }
  }
  return unresolved;
...
function hasExplicitPropertyResponsibilityUncertainty(text) {
  const normalizedText = normalizeTreeServiceText(text || "");
  return /\b(?:scope\s*\/\s*property\s+responsibility|property\s+responsibility|property\s+ownership|ownership)\b.{0,40}\b(?:unclear|unknown|not\s+sure|needs?\s+clarification|needs?\s+review)\b/i.test(normalizedText) ||
    /\b(?:unclear|unknown|not\s+sure|needs?\s+clarification|needs?\s+review)\b.{0,40}\b(?:scope\s*\/\s*property\s+responsibility|property\s+responsibility|property\s+ownership|ownership)\b/i.test(normalizedText);
}
...
export function validateAlphaJson(input, {
  ...
  applyAssembler = true,
  enforceStructural,
  includeStructuralReadiness = false,
  pipelineName = "",
  env = process.env,
} = {}) {
  const preCanonicalJson = structuredClone(input || {});
  const preCanonicalStructuralValidation = validateFinalOptionStructure(preCanonicalJson, { enforce: false });
  // Direct/offline validation keeps its historical assembled projection for
  // evaluation compatibility. Customer routes pass applyAssembler=false and
  // select the assembler only through lib/customerPipeline.js.
  const json = applyAssembler
    ? applyCanonicalServiceAssembler(preCanonicalJson, { force: true })
    : preCanonicalJson;
  const blocking = [];
  const warnings = [];
  const followUps = [];
  const options = canonicalizeFinalEstimateOptions(json);
  const canonicalAssemblerApplied = Boolean(json.service_options?.canonical_service_assembler_applied);
  ...
  const explicitPropertyResponsibilityUncertainty = hasExplicitPropertyResponsibilityUncertainty(rawText);
  const explicitAddressGap = hasUnresolvedExplicitAddressGap(rawText, json, address);
  const slashPriceScopeAmbiguity = hasUnresolvedSlashPriceScopeAmbiguity(rawText, options);
  const safetyScopeBlock = blockingSafetyScopeReviewReason(rawText);
  const enforceSafetyReviewBlocks = enforceFieldResolution && readinessSafetyBlockingEnabled(env);
  const safetyReviewCodes = [];
  ...
  if (!address || /unknown|tbd|placeholder/i.test(address)) {
    ...
    blocking.push("Service address looks unclear.");
    followUps.push(hasExplicitState(address)
      ? "What town or city belongs with this service address?"
      : "What town or city belongs with this service address? Indiana will be added automatically for a listed local town.");
  }
  if (!phone && !email) {
  ...
  if (enforceSafetyReviewBlocks && explicitPropertyResponsibilityUncertainty) {
    blocking.push("Property responsibility or work scope needs contractor review before generating customer documents.");
    followUps.push("Clarify property responsibility and work scope before generating the estimate.");
    safetyReviewCodes.push("PROPERTY_RESPONSIBILITY_REVIEW");
  }
  if (slashPriceScopeAmbiguity) {
    blocking.push(scopeReviewWarning);
    followUps.push("What does each slash price include?");
  }
  if (optionsHaveUnclearScope && !slashPriceScopeAmbiguity) {
    blocking.push(scopeReviewWarning);
    followUps.push("What does each priced option include?");
  }
  ...
  if (safetyScopeBlock) {
    ...
    if (enforceSafetyReviewBlocks) {
      blocking.push(`${safetyScopeBlock} before generating customer documents.`);
      followUps.push(contractorReviewFollowUp(rawText) || "Confirm safe access and damage scope before generating the estimate.");
      safetyReviewCodes.push("SAFETY_SCOPE_REVIEW");
    }
  }
  ...
  const shouldSortByRawLabel = !canonicalAssemblerApplied &&
    optionsForReview.length <= 4 &&
    optionsForReview.some((option) => rawOptionLabelToken(option));
  ...
  const preserveTrustedOverLimitOptions = Boolean(
    json.normalization?.fact_resolution_locked &&
    json.layout_flags?.over_normal_option_limit,
  );
  const sortedOptions = orderedOptions
    ...
    .slice(0, preserveTrustedOverLimitOptions ? orderedOptions.length : 4)
    .map((option, index) => {
      const { numericPrice, ...rest } = option;
      return {
        ...rest,
        label: shouldSortByRawLabel && rawOptionLabelToken(option)
          ? `Option ${rawOptionLabelToken(option)}`
          : `Option ${String.fromCharCode(65 + index)}`,
        raw_label: canonicalAssemblerApplied ? "" : rest.raw_label,
        sort_order: index + 1,
      };
    });
  ...
  blocking.push(...explicitSourceOptionQuantityConflictErrors(rawText));
  blocking.push(...explicitSourceOptionCompletenessErrors(
    rawText,
    sortedOptions,
    json.normalization?.sidecar_price_reconciliation,
  ));
  const structuralValidation = typeof enforceStructural === "boolean"
    ? validateFinalOptionStructure({
      ...json,
      service_options: {
        ...(json.service_options || {}),
        items: sortedOptions,
      },
    }, { enforce: enforceStructural, env })
    : validateFinalOptionStructure({
      ...json,
      service_options: {
        ...(json.service_options || {}),
  ...
  const sourceFinalCoverage = buildSourceFinalFactCoverage({
    rawText,
    finalOptions: sortedOptions,
    finalJob: json.job,
    finalCustomer: json.customer,
    priceReconciliation: json.normalization?.sidecar_price_reconciliation,
    decisions: json.normalization?.decisions,
    claimGraph: json.normalization?.claim_graph,
  });
  const sourceFinalCoveragePdfBlockingEnabled = Boolean(json.normalization?.fact_resolution_locked);
  if (sourceFinalCoverage.applicable) {
    if (sourceFinalCoveragePdfBlockingEnabled) {
      blocking.push(...sourceFinalCoverage.blocking_messages);
    } else {
      warnings.push(...sourceFinalCoverage.blocking_messages);
    }
    warnings.push(...sourceFinalCoverage.warning_messages);
    const clarificationLabels = new Set(
      sourceFinalCoverage.blocking_results.map((result) => String(result.option_label || "").toUpperCase()),
    );
    sortedOptions.forEach((option) => {
      const optionLabel = String(option.label || "").replace(/^Option\s+/i, "").toUpperCase();
      if (!clarificationLabels.has(optionLabel)) return;
      option.review_flags = {
        ...(option.review_flags || {}),
        source_fact_clarification: true,
      };
    });
  }
  const readinessSafety = evaluateReadinessSafety({
    alphaJson: json,
    options: sortedOptions,
    sourceFinalCoverage,
    structuralValidation: includeStructuralReadiness ? structuralValidation : null,
  });
  const reviewerDecisionErrors = Array.isArray(json?.review?.readiness_decision_errors)
    ? json.review.readiness_decision_errors
    : [];
  if (reviewerDecisionErrors.length) {
    blocking.push(...reviewerDecisionErrors.map((error) => `Reviewer decision rejected: ${error.message || "invalid readiness decision"}`));
    followUps.push("Use the current finding, candidate, option, and reason shown in the review card before retrying.");
  }
  const readinessSafetyPdfBlockingEnabled = readinessSafetyBlockingEnabled(env);
  const enforcedReadinessFindings = readinessSafety.enforced_findings || [];
  const allReadinessSafetyBlockingErrors = readinessSafetyPdfBlockingEnabled
    ? enforcedReadinessFindings.map((finding) => `Readiness check [${finding.finding_id}]: ${finding.reason}`)
    : [];
  const preBlockingReadinessOverrideStatus = readinessOverrideStatus({
    readiness_safety: readinessSafety,
    readiness_safety_blocking_errors: allReadinessSafetyBlockingErrors,
    blocking_errors: allReadinessSafetyBlockingErrors,
  }, {}, json);
  const clearedReadinessFindingIds = new Set(preBlockingReadinessOverrideStatus.clearedFindingIds);
  const readinessSafetyBlockingFindings = enforcedReadinessFindings.filter(
    (finding) => !clearedReadinessFindingIds.has(finding.finding_id),
  );
  const readinessSafetyBlockingErrors = readinessSafetyPdfBlockingEnabled
    ? readinessSafetyBlockingFindings.map((finding) => `Readiness check [${finding.finding_id}]: ${finding.reason}`)
    : [];
  if (readinessSafetyPdfBlockingEnabled) {
    if (readinessSafetyBlockingErrors.length) {
      blocking.push(...readinessSafetyBlockingErrors);
      followUps.push("Review the flagged readiness safety findings before generating customer documents.");
    }
  }
  const fieldResolutionBlocking = enforceFieldResolution
    ? fieldResolutionBlockingErrors(json)
    : [];
  blocking.push(...fieldResolutionBlocking);
  if (fieldResolutionBlocking.length) {
    followUps.push("Resolve the phone, price, tree-count, or service-address review card before generating customer documents.");
  }
  ...
  const nonOverridableStructuralBlocks = (structuralValidation.final_blocking_errors || [])
    .filter((error) => error.code === "CONTAMINATED_OPTION_SCOPE")
    .map((error) => error.message);
  if (nonOverridableStructuralBlocks.length) {
    blocking.push(...nonOverridableStructuralBlocks);
    followUps.push("Remove internal parser or field text from the final customer option descriptions.");
  }
  if (structuralValidation.structural_blocking_errors.length) {
    blocking.push(...structuralValidation.structural_blocking_errors);
    followUps.push("Fix the final customer option structure and revalidate before generating customer documents.");
  }
  if (structuralValidation.structural_warnings?.length) {
    warnings.push(...structuralValidation.structural_warnings);
  }
  json.layout_flags = {
    ...(json.layout_flags || {}),
  ...
  const clarificationWarnings = buildFieldClarificationWarnings({
    alphaJson: json,
  ...
  const allValidationWarnings = [
    ...new Set([
      ...validationWarnings,
      ...clarificationWarnings.map((item) => item.message),
    ]),
  ];
  const structuredFollowUps = buildStructuredFollowUps({
    ...
    follow_ups: treeDudeFollowUps,
    clarification_warnings: clarificationWarnings,
  });
  const readinessOverrideState = readinessOverrideStatus({
    readiness_safety: readinessSafety,
    readiness_safety_blocking_errors: readinessSafetyBlockingErrors,
    blocking_errors: blockingErrors,
  }, {}, json);
  const readinessOverrideOutput = {
    enabled: readinessSafetyPdfBlockingEnabled,
    can_proceed: readinessOverrideState.canProceed,
    cleared_finding_ids: readinessOverrideState.clearedFindingIds,
    remaining_finding_ids: readinessOverrideState.remainingReadinessFindings.map((finding) => finding.finding_id),
    decision_ids: readinessOverrideState.decisions.map((decision) => decision.decisionId).filter(Boolean),
    decision_errors: reviewerDecisionErrors,
  };
  ...
  json.validation = {
    ...
    structural_errors: structuralValidation.structural_errors,
    structural_error_codes: structuralValidation.structural_error_codes,
    source_final_fact_coverage: sourceFinalCoverage,
    source_final_fact_coverage_pdf_blocking_enabled: sourceFinalCoveragePdfBlockingEnabled,
    source_final_fact_coverage_blocking_codes: sourceFinalCoverage.blocking_codes,
    source_final_fact_coverage_blocking_results: sourceFinalCoverage.blocking_results,
    source_final_fact_coverage_warning_results: sourceFinalCoverage.warning_results,
    readiness_safety: readinessSafety,
    readiness_safety_pdf_blocking_enabled: readinessSafetyPdfBlockingEnabled,
    readiness_safety_enforced_findings: enforcedReadinessFindings,
    readiness_safety_blocking_findings: readinessSafetyBlockingFindings,
    readiness_safety_shadow_findings: readinessSafety.shadow_findings || [],
    readiness_safety_blocking_errors: readinessSafetyBlockingErrors,
    safety_review_codes: [...new Set(safetyReviewCodes)],
    readiness_override_status: readinessOverrideOutput,
    readiness_decision_errors: reviewerDecisionErrors,
    readiness_decisions: json?.review?.readiness_decisions || [],
    field_resolution_gate_enabled: enforceFieldResolution,
    field_resolution_gate_version: FIELD_RESOLUTION_GATE_VERSION,
    field_resolution_blocking_errors: fieldResolutionBlocking,
    pre_canonical_diagnostics: preCanonicalStructuralValidation.structural_errors,
    pre_canonical_diagnostic_codes: preCanonicalStructuralValidation.structural_error_codes,
    pre_canonical_warnings: preCanonicalStructuralValidation.structural_warnings || [],
    final_blocking_errors: structuralValidation.final_blocking_errors || [],
    structural_blocking_errors: structuralValidation.structural_blocking_errors,
    structural_warnings: structuralValidation.structural_warnings || [],
    structural_enforcement_enabled: structuralValidation.enforcement_enabled,
    final_option_structural_hash: structuralValidation.final_option_structural_hash,
    final_option_structure_validator_version: structuralValidation.validator_version,
    canonical_option_model: structuralValidation.canonical_option_model,
    pipeline_name: pipelineName || (applyAssembler ? "assembled-validation" : "unassembled-validation"),
    issue_status: blocking.length ? "blocking error + follow-up" : allValidationWarnings.length ? "warning" : "none",
  ...
  return {
    ...
    clarification_warnings: json.validation.clarification_warnings,
    structural_errors: json.validation.structural_errors,
    structural_error_codes: json.validation.structural_error_codes,
    structural_blocking_errors: json.validation.structural_blocking_errors,
    structural_warnings: json.validation.structural_warnings,
    final_option_structural_hash: json.validation.final_option_structural_hash,
    readiness_override_status: readinessOverrideOutput,
    readiness_safety: readinessSafety,
  };
}
