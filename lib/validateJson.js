...
export function validateAlphaJson(input) {
  ...
  const json = applyCanonicalServiceAssembler(preCanonicalJson, { force: true });
  const blocking = [];
  const warnings = [];
  const followUps = [];
  ...
  const sourceFinalCoveragePdfBlockingEnabled = false;
  ...
  // TODO(readiness-safety-enforcement): flipping this to true currently blocks the
  // vast majority of the existing fixture corpus (88/603 unit tests fail), because
  // the invariants were tuned only against the narrow 60-case offline eval cohort.
  // The blocking + override plumbing below is fully wired (see reviewOverrides.js
  // acknowledgedReadinessRisk); enabling this requires a dedicated invariant-tuning
  // pass against the broader test corpus first.
  const readinessSafetyPdfBlockingEnabled = false;
  if (readinessSafetyPdfBlockingEnabled) {
    const readinessOverrideAccepted = Boolean(json?.review?.overrides?.acknowledgedReadinessRisk);
    if (readinessSafety.would_block_pdf && !readinessOverrideAccepted) {
      const readinessMessages = [...new Set(readinessSafety.findings.map((finding) => `Readiness check: ${finding.reason}`))];
      blocking.push(...readinessMessages);
  ...
  const structuralValidation = validateFinalOptionStructure(json);
