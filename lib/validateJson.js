...
const SAFETY_ACCESS_TERMS =
  "power\\s*lines?|service\\s+drop|wire|wires|electric|blocked\\s+access|no\\s+access|aggressive\\s+dog|dog|fence\\s+damage|neighbor\\s+fence|leaning\\s+toward|touching|across\\s+(?:drive|driveway|road|gate)|emergency|same-?day";
...
function hasVagueTreeCount(text) {
  ...
  return /\b(?:several|multiple|some|a\s+few|few)\s+(?:[a-z]+\s+){0,4}trees?\b|\bone\s+tree\s+or\s+(?:several|more|multiple)\b/i.test(text || "");
}
