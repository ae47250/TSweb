...
import { buildSourceFinalFactCoverage } from "./sourceFinalFactCoverage.js";
...
function explicitSourceOptionCompletenessErrors(rawText, options = []) {
  const sourceOptions = explicitSourceOptions(rawText).filter((option) => option.amount).slice(0, 4);
  if (sourceOptions.length < 2) return [];

  const finalLabelCounts = new Map();
  const finalAmounts = new Set();
  for (const option of options) {
    ...
    if (amount) finalAmounts.add(Number(amount));
  }

  const missing = sourceOptions.filter((sourceOption) =>
    finalLabelCounts.get(sourceOption.token) !== 1 ||
    !finalAmounts.has(Number(sourceOption.amount)),
  );
...
export function validateAlphaJson(input) {
  ...
  blocking.push(...explicitSourceOptionCompletenessErrors(rawText, sortedOptions));
  const sourceFinalCoverage = buildSourceFinalFactCoverage({
    rawText,
    finalOptions: sortedOptions,
  });
  const sourceFinalCoveragePdfBlockingEnabled = Boolean(
    sourceFinalCoverage.applicable &&
    sortedOptions.some((option) => option.source === "canonical_final_option_model_shadow" || option.canonical_option),
  );
