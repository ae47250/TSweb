...
import { hasIndianaState, hasLocalIndianaTown } from "./localTowns.js";
...
export function validateAlphaJson(input) {
  ...
  } else if (/^Unknown$/i.test(treeCountOverride)) {
    blocking.push("Tree count is marked unknown.");
    followUps.push("How many trees should be included in this estimate?");
  } else if (!acceptedTreeCountOverride && hasVagueTreeCount(rawText)) {
    blocking.push("Tree count is unclear.");
    followUps.push("How many trees should be included in this estimate?");
  } else if (!acceptedTreeCountOverride && !treeCount && !optionsHaveUnclearScope && !hasClearWorkScope(description, options)) {
