...
const ROUTE_PATTERN = /\b(?:Highway|Hwy|Route|State\s+Road|State\s+Route|County\s+Road|County\s+Rd|Road|Rd|CR)\s+\d+(?:\s+[NSEW])?\b/gi;
...
const PRICE_CONTEXT_PATTERN = new RegExp(
  `\\b(?:option|opt|prices?|priced|quote|quoted|bid|estimate|cost|total|only|plus|add|for|to|hall\\s+off|${TREE_SERVICE_PATTERN_SOURCES.workScope})\\b`,
  "i",
);
const STRONG_BARE_PRICE_CONTEXT_PATTERN = new RegExp(
  `\\b(?:option|opt|prices?|priced|quote|quoted|bid|estimate|cost|total|only|plus|add|hall\\s+off|${TREE_SERVICE_PATTERN_SOURCES.workScope})\\b`,
  "i",
);
...
const BARE_OPTION_LABEL_PATTERN = /(?:^|[\s.;,\/])([A-E])(?=\s*(?:[:.)-]|\b(?:maybe|around|about|roughly|price|quote|cost|for|with|w|and|plus|drop|cut|remove|trim|haul|cleanup|clean|full|stump|grind)\b|\/|\$|\d))/gi;
const OPTIONS_CUE_PATTERN = /\b(?:options?|opts?|optons?)\b\s*[:\-]?\s*/gi;
const BARE_OPTION_AFTER_OPTIONS_CUE_PATTERN =
  /(?:^|[\s.;,\/])([A-E])(?=\s*(?:[:.)-]|\b(?:maybe|around|about|roughly|price|quote|cost|for|with|w|and|plus|drop|cut|remove|trim|take|haul|stump|grind|clean|full|leave|stack|brush|wood|limb|tree)\b|\/|\$|\d))/g;
const OPTION_SEGMENT_SEPARATOR_PATTERN = /\/\/|\r?\n|;|\./g;
const SEGMENT_START_OPTION_LABEL_PATTERN =
  /^\s*([A-E])(?=\s*(?:[:.)/-]|\b(?:maybe|around|about|roughly|price|quote|cost|for|with|w|and|plus|drop|cut|remove|trim|take|haul|stump|grind|clean|full|leave|stack|brush|wood|limb|tree)\b|\$|\d))/i;
...
const GENERIC_WORK_SCOPE_PATTERN =
  /\b(?:remove|removal|rmv|take(?:\s+down)?|cut(?:\s+(?:down|only|tree|trees))?|drop(?:\s+(?:only|tree|trees))?|trim|trimming|prune|pruning|haul(?:\s+(?:away|off|debris|brush|logs?|wood))?|hall\s+off|cleanup|clean\s+up|leave\s+(?:wood|logs?|brush|debris|chips?)|stack\s+(?:wood|logs?)|stump(?:\s+(?:grind|grinding|removal))?|grind(?:ing)?|chip(?:ping)?)\b/i;
const ALTERNATIVE_WORK_PHRASE_PATTERN =
  /\b(?:remove|removed|removal|take(?:\s+down)?|cut(?:\s+(?:down|only))?|drop(?:\s+(?:only|it))?|trim|haul(?:\s+(?:away|off))?|hall\s+off|cleanup|clean\s+up|leave\s+(?:wood|logs?|brush|debris)|stack\s+(?:wood|logs?)|stump(?:\s+(?:grind|grinding|removal))?|grind(?:ing)?)\b[^.;?!/$]{0,72}/gi;
...
function pairingConfidenceFor({ price, description, hasOptionLabel = false, reviewWarning = false }) {
  ...
  const hasScope = pairingDescriptionHasWork(description);
...
function collectMoneyLikeNumbers(text, likelyNonPriceNumbers, protectedSpans = []) {
  return [...text.matchAll(MONEY_LIKE_PATTERN)].flatMap((match) => {
    ...
    const beforeNumber = text.slice(Math.max(0, span.start - 36), span.start);
    const afterNumber = text.slice(span.end, Math.min(text.length, span.end + 16));
    const previousScopeWindow = text.slice(Math.max(0, span.start - 140), span.start);
    const hasTerminalForPriceContext = /\bfor\s*$/i.test(beforeNumber) &&
      /^[\s,.;:)]*$/.test(afterNumber) &&
      PRICE_PAIRING_SCOPE_PATTERN.test(previousScopeWindow);
    if (
      isBarePlainNumber &&
      !hasExplicitMoneyMarker &&
      !hasStrongBarePriceContext &&
      !hasSlashPriceContext &&
      !hasTerminalForPriceContext
    ) return [];
    if (
      isBarePlainNumber &&
      hasContactOrAddressContext &&
      !hasStrongBarePriceContext &&
      !hasSlashPriceContext &&
      !hasTerminalForPriceContext
    ) {
    ...
    const clueReasons = [
      ...
      ...(!hasExplicitMoneyMarker && !hasPriceContext && !hasSlashPriceContext ? ["number has no obvious price or option context"] : []),
    ];
    const confidence = confidenceForCandidate({
      explicitNotCurrentPrice,
      hasExplicitMoneyMarker,
      hasPriceContext: hasPriceContext || hasSlashPriceContext,
      hasAmbiguity,
    });
    ...
    return [{
      ...
      clue_strength: explicitNotCurrentPrice
        ? "weak"
        : hasExplicitMoneyMarker || hasPriceContext || hasSlashPriceContext
          ? "strong"
          : "weak",
...
function isReferencedOptionLabel(text, start = 0) {
  const before = String(text || "").slice(Math.max(0, start - 48), start);
  return /\b(?:includes?|including|included|same\s+as|based\s+on)\s+(?:(?:Option|Opt)\s*)?$/i.test(before);
}
...
function collectOptionBoundaryClues(text) {
  ...
  const explicitLabels = [...text.matchAll(OPTION_LABEL_PATTERN)]
    .filter((match) => !isReferencedOptionLabel(text, match.index ?? 0))
    .map((match) => {
      const span = spanFromMatch(match);
      return {
        raw: span.text,
  ...
  const bareLabels = [...text.matchAll(BARE_OPTION_LABEL_PATTERN)]
    .map((match) => tokenSpanFromMatch(match, match[1]))
    .filter((span) => !isReferencedOptionLabel(text, span.start))
    .map((span) => ({
      raw: span.text,
      kind: "bare_option_label",
      token: span.text.toUpperCase(),
      span: { start: span.start, end: span.end },
      context: contextAround(text, span.start, span.end),
    }));
  ...
  for (const cueMatch of text.matchAll(OPTIONS_CUE_PATTERN)) {
    ...
    for (const match of window.matchAll(BARE_OPTION_AFTER_OPTIONS_CUE_PATTERN)) {
      ...
      if (isReferencedOptionLabel(text, absoluteSpan.start)) continue;
...
function trailingOptionDescription(text, price, pairingEnd) {
  if (!price) return "";
  const trailingText = String(text || "").slice(price.span.end, pairingEnd);
  if (/^\s*[.!?;]/.test(trailingText)) return "";
  return cleanOptionDescription(trailingText);
}

function pairingDescriptionHasWork(value) {
  const normalized = compact(value)
    .replace(/\bstmp\b/gi, "stump")
    .replace(/\bhaull\b/gi, "haul")
    .replace(/\bhall\s+off\b/gi, "haul off")
    .replace(/\bw\b/gi, "with");
  return PRICE_PAIRING_SCOPE_PATTERN.test(normalized) || GENERIC_WORK_SCOPE_PATTERN.test(normalized);
}
...
function implicitDescriptionForPrice(text, price, previousPriceEnd = 0, likelyNonPriceNumbers = []) {
  const nearbyStart = Math.max(previousPriceEnd, price.span.start - 80, 0);
  const evidenceBoundary = likelyNonPriceNumbers
    .filter((clue) => clue.kind === "address" && clue.span.end <= price.span.start && clue.span.end > nearbyStart)
    .sort((left, right) => left.span.end - right.span.end)
    .at(-1);
  const scopedStart = Math.max(nearbyStart, evidenceBoundary?.span.end || 0);
  const beforePrice = text.slice(scopedStart, price.span.start);
...
function cleanAlternativeWorkPhrase(value) {
  return cleanOptionDescription(value)
    .replace(/\bhall\s+off\b/gi, "haul off")
    .replace(/^\s*(?:to|for)\s+/i, "")
    .replace(/\s+(?:plus|and|or|versus|vs\.?)[\s./-]*$/i, "")
    .replace(/\s+(?:service\s+address|address|option|opt|prices?|quote|estimate)\b[\s\S]*$/i, "")
    .replace(/\s+(?:cheap\s+way|basic\s+package|full\s+cleanup\s+package)\s*$/i, "")
    .replace(/\s*(?:--|—)\s*$/g, "")
    .replace(/\s+/g, " ")
    .trim();
}

function alternativeWorkPhrase(value) {
  const matches = [...String(value || "").matchAll(ALTERNATIVE_WORK_PHRASE_PATTERN)];
  return cleanAlternativeWorkPhrase(matches.at(-1)?.[0] || "");
}

function alternativeDescriptionForPrice(text, price, index, prices) {
  const previousPrice = prices[index - 1];
  const nextPrice = prices[index + 1];
  const leadingWindow = text.slice(
    Math.max(0, price.span.start - (previousPrice ? 96 : 180)),
    price.span.start,
  );
  const betweenPrices = previousPrice
    ? text.slice(previousPrice.span.end, price.span.start)
    : "";
  const trailingWindow = text.slice(price.span.end, nextPrice?.span.start || text.length);
  const leadingDescription = alternativeWorkPhrase(leadingWindow);
  const betweenDescription = alternativeWorkPhrase(betweenPrices);
  const trailingDescription = alternativeWorkPhrase(trailingWindow);

  if (index === 0) {
    const firstAlternativeCue = /\b(?:to\s+)?(?:drop|cut|remove|take\s+down)\b[\s\S]{0,80}\b(?:plus|and)\b/i.test(betweenPrices);
    if (firstAlternativeCue) return betweenDescription || trailingDescription || leadingDescription;
    return leadingDescription || trailingDescription || betweenDescription;
  }
  return trailingDescription || betweenDescription || leadingDescription;
}

function collectABPricePairings(text, moneyLikeNumbers) {
  const prices = moneyLikeNumbers
    .filter((price) => Number.isFinite(price.price_value) && price.price_value > 0)
    .sort((left, right) => left.span.start - right.span.start || left.span.end - right.span.end);

  return [...String(text || "").matchAll(/\bA\s*\/\s*B\b/gi)].flatMap((match) => {
    const labelStart = match.index ?? 0;
    const labelEnd = labelStart + match[0].length;
    const firstPrice = prices.find((price) => price.span.start >= labelEnd && price.span.start - labelEnd <= 40);
    if (!firstPrice) return [];
    const secondPrice = prices.find((price) =>
      price.span.start > firstPrice.span.end && price.span.start - firstPrice.span.end <= 32,
    );
    if (!secondPrice) return [];

    const description = alternativeWorkPhrase(text.slice(Math.max(0, labelStart - 180), labelStart)) || "tree work";
    const pairing = (price, token) => ({
      ...
      source_amount_confidence: price.amount_confidence || price.confidence || "low",
      pairing_confidence: "high",
      source: "option_price_sidecar_ab_pair",
      span: { start: labelStart, end: price.span.end },
      context: contextAround(text, labelStart, price.span.end),
      boundary_reason: "explicit_ab_price_pair",
      reason: "A/B shorthand explicitly binds the first two nearby prices as independent options.",
    });

    return [pairing(firstPrice, "A"), pairing(secondPrice, "B")];
  });
}

function collectImplicitOptionPricePairings(text, moneyLikeNumbers, existingPairings = [], likelyNonPriceNumbers = []) {
  const pairedPriceIds = new Set(existingPairings.map((pairing) => pairing.price_id).filter(Boolean));
  const prices = moneyLikeNumbers
    .filter((price) => Number.isFinite(price.price_value) && price.price_value > 0)
    .sort((left, right) => left.span.start - right.span.start || left.span.end - right.span.end);
  const explicitAlternativeSet = hasExplicitIndependentAlternativeSet(text, prices, existingPairings);
  return moneyLikeNumbers.flatMap((price, index) => {
    ...
    const implicitDescription = implicitDescriptionForPrice(
      text,
      price,
      previousPrice?.span?.end || 0,
      likelyNonPriceNumbers,
    );
    const priceIndex = prices.findIndex((candidate) => candidate.price_id === price.price_id);
    const description = explicitAlternativeSet && priceIndex >= 0
      ? alternativeDescriptionForPrice(text, price, priceIndex, prices) || implicitDescription
      : implicitDescription;
    if (!description && price.confidence !== "high") return [];
    const pairingConfidence = explicitAlternativeSet && price.amount_confidence === "high" && pairingDescriptionHasWork(description)
      ? "high"
      : pairingConfidenceFor({ price, description });
    return [{
      price_id: price.price_id,
...
function collectOptionPricePairings(text, optionBoundaryClues, moneyLikeNumbers, likelyNonPriceNumbers = [], emailCandidates = []) {
  const abPairings = collectABPricePairings(text, moneyLikeNumbers);
  const abPriceIds = new Set(abPairings.map((pairing) => pairing.price_id));
  const segmentPairings = collectLabeledOptionSegments(text).flatMap((segment) => {
    ...
    const leadingDescription = cleanOptionDescription(text.slice(segment.labelSpan.end, descriptionEnd));
    const trailingDescription = trailingOptionDescription(text, price, pairingEnd);
    const description = trailingDescription && pairingDescriptionHasWork(trailingDescription) &&
      !pairingDescriptionHasWork(leadingDescription)
      ? trailingDescription
      : leadingDescription;
  ...
  }).filter((pairing) => !abPriceIds.has(pairing.price_id));

  const labels = optionBoundaryClues
    .filter((item) => /(?:option_label|bare_option_label)/i.test(item.kind) && /^[A-E1-5]$/i.test(item.token || ""))
    .sort((left, right) => left.span.start - right.span.start);
  if (!labels.length || !moneyLikeNumbers.length) {
    const existingPairings = [...abPairings, ...segmentPairings];
    return [...existingPairings, ...collectImplicitOptionPricePairings(text, moneyLikeNumbers, existingPairings, likelyNonPriceNumbers)]
  ...
  const fallbackPairings = labels.flatMap((label, index) => {
    ...
    const descriptionEnd = price ? price.span.start : pairingEnd;
    const leadingDescription = cleanOptionDescription(text.slice(label.span.end, descriptionEnd));
    const trailingDescription = trailingOptionDescription(text, price, pairingEnd);
    const leadingHasWork = pairingDescriptionHasWork(leadingDescription);
    const trailingHasWork = pairingDescriptionHasWork(trailingDescription);
    const description = leadingHasWork && trailingHasWork
      ? `${leadingDescription} and ${trailingDescription}`
      : trailingHasWork && !leadingHasWork
        ? trailingDescription
        : leadingDescription;
    if (!description && !price) return [];
  ...
  const seen = new Set();
  const labeledPairings = [...abPairings, ...segmentPairings, ...fallbackPairings]
    .filter((pairing) => {
    ...
    .sort((left, right) => (left.span?.start || 0) - (right.span?.start || 0));
  return [...labeledPairings, ...collectImplicitOptionPricePairings(text, moneyLikeNumbers, labeledPairings, likelyNonPriceNumbers)]
    .sort((left, right) => (left.span?.start || 0) - (right.span?.start || 0));
}
...
function cleanInterpretationScope(value, fallback) {
  const cleaned = cleanOptionDescription(value)
    .replace(/\b(?:includes?|including|included)\s+(?:(?:Option|Opt)\s*)?[A-E1-5]\s+(?:plus|with|and)\s+/gi, " ")
    .replace(/\b(?:includes?|including|included)\s+(?:(?:Option|Opt)\s*)?[A-E1-5]\b/gi, " ")
    .replace(/\b(?:prices?|priced|quote|quoted|estimate|est|bid|cost|total|option|opt)\b/gi, " ")
...
function hasExplicitIndependentAlternativeSet(text, prices = [], optionPricePairings = []) {
  if (prices.length < 2) return false;
  const source = String(text || "");
  const labeledPairings = optionPricePairings
    .filter((pairing) => pairing.price_id && pairing.raw_label_token)
    .sort((left, right) => left.span.start - right.span.start);
  const completeLabeledAlternativeSet = prices.length >= 3 &&
    labeledPairings.length === prices.length &&
    new Set(labeledPairings.map((pairing) => pairing.raw_label_token.toUpperCase())).size === prices.length &&
    labeledPairings.every((pairing) =>
      pairingDescriptionHasWork(pairing.description_raw) &&
      rankedConfidence(pairing.pairing_confidence || pairing.confidence) >= rankedConfidence("high"),
    );
  if (completeLabeledAlternativeSet) return true;

  const labeledServiceFamilies = labeledPairings.map((pairing) => {
    const description = compact(pairing.description_raw).toLowerCase();
    if (/\b(?:stump|stumps|grind|grinding)\b/.test(description)) return "stump_grinding";
    if (/\b(?:trim|trimming|prune|pruning|crown)\b/.test(description)) return "tree_trim";
    if (/\b(?:limbs?|branches?)\b/.test(description)) return "limb_removal";
    if (/\b(?:brush|cleanup|clean\s+up|haul|debris|clear)\b/.test(description)) return "cleanup";
    if (/\b(?:remove|removal|take\s+down|cut\s+down|drop)\b/.test(description)) return "tree_removal";
    return "";
  });
  const hasIndependentCue = /\b(?:different|separate|independent|alternative|alternatively|existing|or|either|versus|vs\.?)\b/i.test(source);
  if (prices.length === 2 &&
      labeledPairings.length === 2 &&
      labeledServiceFamilies.every(Boolean) &&
      new Set(labeledServiceFamilies).size === 2 &&
      hasIndependentCue) {
    return true;
  }

  if (prices.length === 2 &&
      /\b(?:to\s+)?(?:drop|cut|remove|take\s+down)\b[\s\S]{0,100}\b(?:plus|and)\b[\s\S]{0,80}\bto\s+(?:haul|hall|clean(?:up)?|remove)\b/i.test(source)) {
    return true;
  }

  const hasArithmeticCue = /\b(?:add(?:\s*-?on)?|extra|additional|in\s+addition|on\s+top|plus|total|also)\b/i.test(source);
  const labeledOptions = new Set(optionPricePairings
    .map((pairing) => pairing.raw_label_token || "")
    .filter(Boolean));
  const explicitWordLabelCount = [...source.matchAll(/\b(?:option|opt)\s*[A-E1-5]\b/gi)].length;
  const bareLabeledOptions = optionPricePairings
    .filter((pairing) => pairing.raw_label_token && !/^option|^opt/i.test(pairing.raw_label || ""));
  if (bareLabeledOptions.length >= 2 && explicitWordLabelCount < 2 && !hasArithmeticCue) return true;

  if (!hasArithmeticCue && prices.length >= 3) {
    const pairingsByPriceId = new Map(optionPricePairings.map((pairing) => [pairing.price_id, pairing]));
    const pairings = prices.map((price) => pairingsByPriceId.get(price.price_id));
    const hasStandaloneWorkPairings = pairings.length === prices.length && pairings.every((pairing) =>
      pairing &&
      pairingDescriptionHasWork(pairing.description_raw) &&
      rankedConfidence(pairing.pairing_confidence || pairing.confidence) >= rankedConfidence("high"),
    );
    const finalPairingHasStumpScope = /\b(?:stump|stumps|grind|grinding)\b/i.test(pairings.at(-1)?.description_raw || "");
    if (hasStandaloneWorkPairings && finalPairingHasStumpScope) return true;
  }
  if (prices.length !== 2 || hasArithmeticCue) return false;
  if (/\b(?:cut|drop|remove|take\s+down)\s+only\b[\s\S]{0,120}\b(?:vs\.?|versus|\/|or)\b/i.test(source)) return true;
  if (/\b(?:cheap\s+way|basic\s+package|full\s+cleanup\s+package)\b/i.test(source)) return true;
  if (/\bdrop\b[\s\S]{0,100}\b(?:haul|hall\s+off|cleanup|clean\s+up)\b/i.test(source)) return true;
  const betweenPrices = source.slice(prices[0].span.end, prices.at(-1).span.start);
  if (/\//.test(betweenPrices) &&
      /\b(?:remove|take\s+down|cut|drop|haul|hall\s+off|leave|stack|cleanup|clean\s+up)\b/i.test(betweenPrices)) {
    return true;
  }
  return false;
}

function collectIndependentAlternativeRelationships(text, prices = [], optionPricePairings = []) {
  if (!hasExplicitIndependentAlternativeSet(text, prices, optionPricePairings)) return [];
  const labeledPairings = optionPricePairings
    .filter((pairing) => pairing.raw_label_token && pairing.price_id && pairing.price_value)
    .sort((left, right) => left.span.start - right.span.start);
  if (labeledPairings.length !== 2) return [];
  const [first, second] = labeledPairings;
  if (first.raw_label_token === second.raw_label_token) return [];
  return [{
    relationship_id: `alternative_${first.price_id}_to_${second.price_id}`,
    type: "alternative_to",
    option_price_ids: [first.price_id, second.price_id],
    option_amounts: [first.price_value, second.price_value],
    option_displays: [first.price_display, second.price_display],
    confidence: "high",
    reason: "Distinct bare option labels bind two independently described customer choices.",
    source: "option_price_sidecar_independent_alternatives",
  }];
}
...
function collectAddOnPriceInterpretations(text, moneyLikeNumbers = [], optionPricePairings = []) {
  ...
  for (let index = 1; index < prices.length; index += 1) {
    ...
    const slashAlternative = /\/\s*$/.test(between) || /^\s*\//.test(between);
    ...
    const interpretation = slashAlternative
      ? "needs_review"
      : explicitAdditiveIntent && hasAddOnService
      ? "additive_amount"
...
export function buildOptionPriceCandidateView(rawInput = "", protectedSpans = [], options = {}) {
  ...
  const independentAlternativeRelationships = collectIndependentAlternativeRelationships(
    rawCustomerNote,
    moneyLikeNumbers,
    optionPricePairings,
  );
