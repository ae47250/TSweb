...
const ADDRESS_PATTERN =
  /\b\d{1,5}\s+(?:[A-Za-z0-9.]+\s+){0,5}(?:Street|St|Road|Rd|Avenue|Ave|Lane|Ln|Drive|Dr|Court|Ct|Way|Pike|Trail|Trl|Highway|Hwy|Route|Terrace|Parkway)\b/gi;
...
function collectSafetyScopeWarnings(text) {
  return [...text.matchAll(SAFETY_SCOPE_RISK_PATTERN)].map((match) => {
    const span = spanFromMatch(match);
    return {
      warning: "Raw note contains safety, access, damage, or scope language that should be reviewed before PDF generation.",
      raw: span.text,
